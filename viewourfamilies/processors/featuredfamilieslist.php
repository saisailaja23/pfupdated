...
$sql_getAgencyFilter .= ' and `Profiles`. `ID` NOT IN (SELECT  `author_id` FROM  `watermarkimages`) and `Profiles`. `Avatar`!=0';
if ($searchtype == 'ethnicity') {
//    if ($searchvalues == 'other') {
//        $sql_getAgencyFilter .= " AND `Profiles`. `ID` in (select p1.ID from Profiles as p1 left join Profiles as p2  on p1.ID = p2.Couple where (p1.`Ethnicity` IN ('',  'N/A',  'null',  'other') or p2.Ethnicity  IN ('',  'N/A',  'null',  'other') or (p1.Ethnicity  IN ('',  'N/A',  'null',  'other') and (p1.Status = 'Active') and p1.Couple = 0) and (p1.Status = 'Active')) ";
//    } else
//        $sql_getAgencyFilter .= " AND `Profiles`. `ID` in (select p1.ID from Profiles as p1 left join Profiles as p2  on p1.ID = p2.Couple where (p1.`ChildEthnicity` IN  ('$searchvalues' , 'Any') or p2.ChildEthnicity IN  ('$searchvalues' , 'Any')) or (p1.ChildEthnicity IN  ('$searchvalues' , 'Any') and p1.Status = 'Active' and p1.Couple = 0) and p1.Status = 'Active') ";
}
...
$currDate = db_arr('SELECT CURDATE() AS currdate');
$currDate = $currDate[currdate];

switch ($searchtype) {
    case 'Region':
        $AgencyLike_List = "SELECT `Profiles`. `ID`,`Profiles`. `Region`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and Profiles.Region ='$searchvalues' and `Profiles`.`ProfileType` = 2   AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter limit $from , $count";
        if ($from == 0) {
            $totalCount = "select count(`Profiles`. `ID`) FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and Profiles.Region ='$searchvalues' and `Profiles`.`ProfileType` = 2   AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter";
        ...
        break;
    case 'Religion':
        $AgencyLike_List = "SELECT `Profiles`. `ID`,`Profiles`. `Region`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and (Profiles.Religion LIKE '%$searchvalues%' OR Profiles.faith LIKE '%$searchvalues%') and `Profiles`.`ProfileType` = 2   AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter limit $from , $count";
        if ($from == 0) {
            $totalCount = "select count(`Profiles`. `ID`) FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and (Profiles.Religion LIKE '%$searchvalues%' OR Profiles.faith LIKE '%$searchvalues%') and `Profiles`.`ProfileType` = 2   AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter";
            $total_count = mysql_query($totalCount);
            $rowCount = mysql_fetch_array($total_count);
            $tCount = $rowCount[0];
        }
    ...
    case 'Familysize':
        ...
        $AgencyLike_List = "SELECT `Profiles`. `ID`,`Profiles`. `Region`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and ($searchOption) and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter limit $from , $count";
        if ($from == 0) {
            $totalCount = "select count(`Profiles`. `ID`) FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and ($searchOption and Profiles.noofchildren is not null and Profiles.noofchildren <> '') and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter";
            $total_count = mysql_query($totalCount);
            $rowCount = mysql_fetch_array($total_count);
            $tCount = $rowCount[0];
        }
    ...
    case 'Sortby':
        ...
        $AgencyLike_List = "SELECT `Profiles`. `ID`,`Profiles`. `Region`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active')  and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter ORDER BY $orderBy limit $from , $count";
        if ($from == 0) {
            $totalCount = "select count(`Profiles`. `ID`)  FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active')  and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter ORDER BY $orderBy";
            $total_count = mysql_query($totalCount);
            $rowCount = mysql_fetch_array($total_count);
            $tCount = $rowCount[0];
        }
    ...
    case 'ethnicity':
        if ($from == 0) {
            $totalCount = "select count(`Profiles`. `ID`) FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and FIND_IN_SET( '$searchvalues' ,`ChildEthnicity` ) and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter";
            $total_count = mysql_query($totalCount);
            $rowCount = mysql_fetch_array($total_count);
            $tCount = $rowCount[0];
        }
        $AgencyLike_List = "SELECT `Profiles`. `ID`,`Profiles`. `Region`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and FIND_IN_SET( '$searchvalues' ,`ChildEthnicity` ) and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter limit $from , $count";
        break;
    case 'State':
        if ($from == 0) {
            $totalCount = "select count(`Profiles`. `ID`) FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and  (`Profiles`.`Status` = 'Active') and FIND_IN_SET( '$searchvalues' ,`state` ) and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter";
            $total_count = mysql_query($totalCount);
            $rowCount = mysql_fetch_array($total_count);
            $tCount = $rowCount[0];
        }
        $AgencyLike_List = "SELECT `Profiles`. `ID`,`Profiles`. `Region`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and FIND_IN_SET( '$searchvalues' ,`state` ) and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter limit $from , $count";
        break;
    default:
        if ($from == 0) {
            $totalCount = "select count(`Profiles`. `ID`)  FROM `Profiles`  Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter ORDER BY `Profiles`.`DateReg` DESC";
            $total_count = mysql_query($totalCount);
            $rowCount = mysql_fetch_array($total_count);
            $tCount = $rowCount[0];
        }
        // $AgencyLike_List = "SELECT `Profiles`. `ID`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles` WHERE  (`Profiles`.`Status` = 'Active') and `Profiles`.`ProfileType` = 2  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter ORDER BY `Profiles`.`DateReg` , RAND() DESC limit $from , $count";
        $AgencyLike_List = "SELECT `Profiles`. `ID`, `Profiles`.`NickName`, `Profiles`.`Couple`, `Profiles`.`Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles` Join sys_acl_levels_members ON Profiles.ID = sys_acl_levels_members.IDMember WHERE sys_acl_levels_members.IDLevel = 24  and (sys_acl_levels_members.DateExpires > Now() or sys_acl_levels_members.DateExpires IS NULL) and (`Profiles`.`Status` = 'Active') and `Profiles`.`ProfileType` = 2 AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter ORDER BY RAND() limit $from , $count";
        break;
}
...
//$columns = '';
    $Agencydetails = db_res("SELECT  ID,FirstName,Age,State,waiting,noofchildren,faith,ChildEthnicity,ChildAge,Adoptiontype,Avatar,NickName,Occupation,Religion,Education,Ethnicity,Couple from Profiles where ID = " . $agency_id . "");
    $gridData = array();
    while (($row_agency = mysql_fetch_array($Agencydetails, MYSQL_BOTH))) {
        $first_age = '';
        if ($row_agency['Age'] != '')
            $first_age = date("Y") - $row_agency['Age'];
        $gridData[$row_agency['ID']]['Age'] = $first_age ? $first_age : "N/A";
...
//        $gridData[$row_agency['ID']]['Occupation'] = $row_agency['Occupation'] ? $row_agency['Occupation'] : "N/A";
        ...
        $p1Prof_ary = explode(",", $row_agency['Occupation']);
        $p1Prof_sub = array_slice($p1Prof_ary, 0, 3);
        $p1Prof = sizeof($p1Prof_ary) >= 4 ? implode(" ", $p1Prof_sub) . ' <a class = \'toolhelp\' title-text= "' . $row_agency['Occupation'] . '"  href="javascript:void(0)" ><span class="dialog" data-toggle="modal" href="#more">....More</span><span class="nodialog">....More</span></a>' : $row_agency['Occupation'];
//        $childethnicity = htmlentities($childethnicity);
        $gridData[$row_agency['ID']]['Occupation'] = ($p1Prof) ? $p1Prof : "N/A";


        if ($row_agency['Couple'] != 0 && $row_agency['Couple'] != '') {
//            echo "SELECT  ID,FirstName,Age,State,waiting,noofchildren,faith,ChildEthnicity,ChildAge,Adoptiontype,Avatar,NickName,Occupation,Religion,Education,Ethnicity,Couple from Profiles where ID = " . $row_agency['Couple'] . "";
            $Agencydetails_Couple = db_res("SELECT  ID,FirstName,Age,State,waiting,noofchildren,faith,ChildEthnicity,ChildAge,Adoptiontype,Avatar,NickName,Occupation,Religion,Education,Ethnicity,Couple from Profiles where ID = " . $row_agency['Couple'] . "");
            while (($row_agency_couple = mysql_fetch_array($Agencydetails_Couple, MYSQL_BOTH))) {
                $second_age = '';
                if ($row_agency_couple['Age'] != '')
                    $second_age = date("Y") - $row_agency_couple['Age'];
                $gridData[$row_agency_couple['ID']]['Age'] = $second_age ? $second_age : "N/A";
...
//                $gridData[$row_agency_couple['ID']]['Occupation'] = $row_agency_couple['Occupation'] ? $row_agency_couple['Occupation'] : "N/A";
                ...
                $p2Prof_ary = explode(" ", $row_agency_couple['Occupation']);
                $p2Prof_sub = array_slice($p2Prof_ary, 0, 3);
                $p2Prof = sizeof($p2Prof_ary) >= 4 ? implode(" ", $p2Prof_sub) . ' <a class = \'toolhelp\' title-text= "' . $row_agency_couple['Occupation'] . '"  href="javascript:void(0)" ><span class="dialog" data-toggle="modal" href="#more">....More</span><span class="nodialog">....More</span></a>' : $row_agency_couple['Occupation'];
                //        $childethnicity = htmlentities($childethnicity);
                $gridData[$row_agency_couple['ID']]['Occupation'] = ($p2Prof) ? $p2Prof : "N/A";
            }
        }
...
//  $waiting =  $row_agency[4].' years';
        ...
        if ($Couple_age != '') {
            if ($row_agency[2] != '') {
                $first = date("Y") - $row_agency[2];
            }
            if ($Couple_age != '') {
                $second = date("Y") - $Couple_age;
            }
            $profileages = $first . '/' . $second;
        } else {
            if ($row_agency[2] != '') {
                $firstsingle = date("Y") - $row_agency[2];
            }
