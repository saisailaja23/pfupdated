...
define('BX_DOL_ADM_MP_CTL', 'search');
...
$_SESSION['Paramsvoucher'] = $aParams;
...
$_page = array(
    ...
    'js_name' => array('profiles_voucher.js'),
    'header' => _t('Voucher reports')
);
$_page_cont[$iNameIndex] = array(
    ...
    'actions_url' => $GLOBALS['site']['url_admin'] . 'voucherreports.php',
...
function PageCodeControls($sDefault = BX_DOL_ADM_MP_CTL) {
    ...
    return DesignBoxAdmin(_t('please select voucher code usage date'), $oAdmTemplate->parseHtmlByName('mp_controls_voucher.html', $aParams), $aTopMenu);
}
...
//foreach ($aInputkidsprogram as $k => $r) {
    ...
    $aForm = array(
        'form_attrs' => array(
            'id' => 'adm-mp-search',
            'action' => $GLOBALS['site']['url_admin'] . 'voucherreports.php',
            'method' => 'post',
...
function PageCodeMembers($sDefaultCtl = BX_DOL_ADM_MP_CTL, $sDefaultView = BX_DOL_ADM_MP_VIEW) {
    ...
    $aResult = array(
    	'action_url' => $GLOBALS['site']['url_admin'] . 'voucherreports.php',
    ...
    return DesignBoxAdmin(_t('Members Listing'), $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_voucher.html', $aResult), $aTopMenu);
}
    ...
    switch($aParams['ctl_type']) {
        case 'qlinks':
         ...
         // between '2008-12-23'and '2008-12-25';
              if(isset($aParams['ctl_params'][2]) && $aParams['ctl_params'][2] !='0'){
              $memtype = $aParams['ctl_params'][2];
             $sWhereClause .= " AND `tl`.`Name` ='".$memtype."'";
             }
         
              
   if(isset($aParams['ctl_params'][0]) && isset($aParams['ctl_params'][1]) ){

         $start_date = $aParams['ctl_params'][0];
         $format_date = str_replace('-', '/', $start_date);

         $end_date = $aParams['ctl_params'][1];
...
/*if(isset($_SESSION['Paramsvoucher'][2])) {
 $memtype = $_SESSION['Paramsvoucher'][2];
             ...
             $sWhereClause .=  "AND `vt`.`status` = 'Processed'".$memtypes. " AND DATE_FORMAT(`vt`.`Timestamp`,'%Y-%m-%d')  between '".   $new_date."'   AND '". $new_date1. "'";
*/
           break;
    }
...
//print_r($_POST);
    ...
    $oPaginate = new BxDolPaginate(array(
        ...
        'count' => (int)db_value("SELECT COUNT(`tp`.`ID`) FROM `Profiles` AS `tp` ,`bx_groups_main`,`sys_acl_levels_members` AS `tlm`,`sys_acl_levels` AS `tl`,`aqb_membership_vouchers_transactions` AS `vt`

       WHERE `tp`.`AdoptionAgency`=`bx_groups_main`.`id`  And `tp`.`ID`= `tlm`.`IDMember` And `tlm`.`IDLevel`=`tl`.`ID`     	" . $sJoinClause . "
    	AND
    		1 AND (`tp`.`Couple`=0 OR `tp`.`Couple`>`tp`.`ID`) AND `vt`.ProfileID = `tlm`.`IDMember` AND `vt`.`status` = 'Processed' AND `tlm`.`TransactionID` != 'NULL' " . $sWhereClause),
        'per_page' => $aParams['view_per_page'],
        'page_url' => $GLOBALS['site']['url_admin'] . 'voucherreports.php?start={start}',
...
function getMembersGeeky($aProfiles, $sPaginate) {
    ...
    return $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_voucher_geeky.html', array(
