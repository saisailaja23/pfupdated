...
if(isset($_POST['adm-mp-activate']) && (bool)$_POST['members']) {
    ...
    $GLOBALS['MySQL']->query("UPDATE `Profiles` SET `Status`='Inactive' WHERE `ID` IN ('" . implode("','", $_POST['members']) . "')");
    foreach($_POST['members'] as $iId) {
        createUserDataFile((int)$iId);
        reparseObjTags('profile', (int)$iId);
    ...
    $aForm = array(
        ...
        'inputs' => array (
            ...
            'filter_value' => array(
        		'type' => 'select',
        		'name' => 'member_sort',
        		'caption' => _t('Filter by '),
        		'values' => getValuesArray('MemberSort'),
...
function getMembers($aParams)
{
    ...
    switch($aParams['ctl_type']) {
        ...
        case 'search':
          if(isset($aParams['ctl_params'][1]) && $aParams['ctl_params'][1] !='0'){
              
              
             $filtertype = $aParams['ctl_params'][1];
              
               if($filtertype == 'NickName'){
                    $sWhereClause .= " AND (
                `tp`.`NickName` LIKE '%" . $aParams['ctl_params'][0] . "%' 
           )"; 
               }
             else if($filtertype == 'Email') {                   
                  $sWhereClause .= " AND (
                `tp`.`Email` LIKE '%" . $aParams['ctl_params'][0] . "%' 
           )";   
               }
               
            else if($filtertype == 'FirstName') {                   
                  $sWhereClause .= " AND (
                `tp`.`FirstName` LIKE '%" . $aParams['ctl_params'][0] . "%' 
           )";   
               }
               
            else if($filtertype == 'LastName') {                   
                  $sWhereClause .= " AND (
                `tp`.`LastName` LIKE '%" . $aParams['ctl_params'][0] . "%' 
           )";   
               }
           else if($filtertype == 'Membership') {                   
                  $sWhereClause .= " AND (
                `tl`.`Name` LIKE '%" . $aParams['ctl_params'][0] . "%' 
              ...
              $sWhereClause .= " AND (
                `tp`.`ID` LIKE '%" . $aParams['ctl_params']['0'] . "%' OR
                `tp`.`NickName` LIKE '%" . $aParams['ctl_params']['0'] . "%' OR
                `tp`.`Email` LIKE '%" . $aParams['ctl_params']['0'] . "%' OR
                `tp`.`Headline` LIKE '%" . $aParams['ctl_params']['0'] . "%' OR
                `tp`.`DescriptionMe` LIKE '%" . $aParams['ctl_params']['0'] . "%' OR
                `tp`.`Tags` LIKE '%" . $aParams['ctl_params']['0'] . "%' OR
                `tp`.`DateReg` LIKE '%" . $aParams['ctl_params']['0'] . "%' OR
               `tp`.`DateLastLogin` LIKE '%" . $aParams['ctl_params']['0'] . "%'
           )";   
           }   
...
//            )";
  ...
  $sQuery = "
        SELECT
            ...
            " . $sSelectClause . ",
            DATE_FORMAT(`tlm`.`DateStarts`,  '" . $sDateFormat . "' ) AS `date_starts`
        FROM `Profiles` AS `tp`
...
function getMembersGeeky($aProfiles, $sPaginate, $sControls)
{
    ...
    foreach($aProfiles as $aProfile){
        ...
        if($aItems[$aProfile['id']]) {
//            echo $aProfile['id'].'->'.$aItems[$aProfile['id']]['date_starts'] - $aProfile['date_starts'];
            if($aItems[$aProfile['id']]['date_starts'] < $aProfile['date_starts'])
                $aItems[$aProfile['id']] = array(
                    'changepass_icon'=>'<a href='.$GLOBALS['site']['url'] .'changepassword.php?ID='.$aProfile['id'].' ><img src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/icon_member_pass.gif" alt="Change Password" title="Change Password"/> </a>',
                    'id' => $aProfile['id'],
                    ...
                    'ml_id' => !empty($aProfile['ml_id']) ? (int)$aProfile['ml_id'] : 2,
                            'ml_name' => !empty($aProfile['ml_name']) ? $aProfile['ml_name'] : 'Basic',
                            'date_starts' => !empty($aProfile['date_starts']) ? $aProfile['date_starts'] : 'Basic'
                );
        }
        else{
            $aItems[$aProfile['id']] = array(
