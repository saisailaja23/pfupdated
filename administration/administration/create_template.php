...
function actionAllCategories() {
    ...
    if(bx_get('pathes') !== false) {
    	...
    	if(is_array($aPathes) && !empty($aPathes))
	        foreach($_POST['pathes'] as $sValue) {
	            ...
	            if (bx_get('action_disable') !== flase)
    ...
    if (!empty($oCategories->aTagObjects))
    {
        ...
        $sContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('top_block_select.html', array(
            ...
            'location_href' => BX_DOL_URL_ADMIN . 'create_template.php?action=all&module='
        ));
...
function getAddCategoryForm()
{
    ...
    if ($oForm->isSubmittedAndValid())
    {
            ...
            $oForm->getCleanValue('Type') . "'") == 0)
        {
            ...
            header('Location:' . BX_DOL_URL_ADMIN . 'create_template.php?action=all&module=' . $oForm->getCleanValue('Type'));
        }
        else
...
$_page = array(
    ...
    'header' => _t('Upload template'),
    'header_text' => 'Test title'
);
$_page_cont[$iNameIndex]['page_main_code'] = DesignBoxAdmin(_t('Upload template'), getAddCategoryForm()) .
    DesignBoxAdmin($aMenu[$sAction]['title'], $sContent);
