...
function PageCodeControls($sDefault = BX_DOL_ADM_MP_CTL) {
    ...
    return DesignBoxAdmin(_t('Please select registration date'), $oAdmTemplate->parseHtmlByName('mp_controls_user.html', $aParams), $aTopMenu);
}
...
function PageCodeMembers($sDefaultCtl = BX_DOL_ADM_MP_CTL, $sDefaultView = BX_DOL_ADM_MP_VIEW) {
    ...
    return DesignBoxAdmin(_t('Members Listing'), $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_user.html', $aResult), $aTopMenu);
}
...
function getMembersGeeky($aProfiles, $sPaginate) {
    ...
    return $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_user_geeky.html', array(
