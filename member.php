...
class BxDolMember extends BxDolPageView
{
    ...
    function getBlockCode_AccountControl()
    {
        ...
        $sMembersC = ' ' . _t('_Members');
              ...
              if($this->aMemberInfo['Status'] == 'Active') {
        ...
        $sYearsOld = '';
        if ('0000-00-00' != $this->aMemberInfo['DateOfBirth']) {
		    $iYears = age($this->aMemberInfo['DateOfBirth']);
		    $sYearsOld = _t('_y/o', $iYears);
        }

		$sProfileIcon = '';
        switch ($this->aMemberInfo['Sex']) {
            case 'male': $sProfileIcon = '<img src="' . $sMaleIcon . '" />'; break;
            case 'female': $sProfileIcon = '<img src="' . $sFemaleIcon . '" />'; break;
        }
...
if (!$sUrlRelocate = $sRelocate or $sRelocate == $site['url'] or basename($sRelocate) == 'join.php')
	...
	if (strstr($sUrlRelocate,'member.php')) {
		...
		$dbRd = $dbRv;
		$dbRd = str_replace("{nickname}",getUsername((int)$_COOKIE['memberID']),$dbRd);
