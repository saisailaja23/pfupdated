...
$_page = array(
    ...
    'js_name' => array('profiles_expiry.js'),
    'header' => _t('Membership expiry reports')
);
$_page_cont[$iNameIndex] = array(
    ...
    'actions_url' => $GLOBALS['site']['url_admin'] . 'expiryreports.php',
...
function PageCodeControls($sDefault = BX_DOL_ADM_MP_CTL) {
    ...
    return DesignBoxAdmin(_t('please select expiry dates'), $oAdmTemplate->parseHtmlByName('mp_controls_expiry.html', $aParams), $aTopMenu);
}
...
//foreach ($aInputkidsprogram as $k => $r) {
    ...
    $aForm = array(
        'form_attrs' => array(
            'id' => 'adm-mp-search',
            'action' => $GLOBALS['site']['url_admin'] . 'expiryreports.php',
            'method' => 'post',
...
function PageCodeMembers($sDefaultCtl = BX_DOL_ADM_MP_CTL, $sDefaultView = BX_DOL_ADM_MP_VIEW) {
    ...
    $aResult = array(
    	'action_url' => $GLOBALS['site']['url_admin'] . 'expiryreports.php',
    ...
    return DesignBoxAdmin(_t('Members Listing'), $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_expiry.html', $aResult), $aTopMenu);
}
...
//print_r($_POST);
    ...
    $oPaginate = new BxDolPaginate(array(
        ...
        'count' => (int)db_value("SELECT COUNT(`tp`.`ID`) FROM `Profiles` AS `tp` ,`bx_groups_main`,`sys_acl_levels_members` AS `tlm`,`sys_acl_levels` AS `tl`

       WHERE `tp`.`AdoptionAgency`=`bx_groups_main`.`id`  And `tp`.`ID`=`tlm`.`IDMember` And `tlm`.`IDLevel`=`tl`.`ID` AND `tlm`.`DateExpires` != 'NULL'" . $sJoinClause . "
    	AND
    		1 AND (`tp`.`Couple`=0 OR `tp`.`Couple`>`tp`.`ID`)" . $sWhereClause),
        'per_page' => $aParams['view_per_page'],
        'page_url' => $GLOBALS['site']['url_admin'] . 'expiryreports.php?start={start}',
...
function getMembersGeeky($aProfiles, $sPaginate) {
      ...
      //  $_adoptAgency=db_arr("SELECT title FROM bx_groups_main WHERE id=".$aProfile['AdoptionAgency']);
        $aItems[] = array(
            ...
            'last_name' => $aProfile['lastname'],
             'Code' => $aProfile['Expiry date'],
            'dateofcode' => $aProfile['Last login date'],
            'email' => $sEmail,
    ...
    return $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_expiry_geeky.html', array(
