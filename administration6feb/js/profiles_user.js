...
BxManageProfiles.prototype.changeFilterSearch = function () {
    var sValue = $("[name='adm-mp-filter1']").val();
    var sValue_sec = $("[name='adm-mp-filter2']").val();
    var smem = $("[name='adm-mp-filter3']").val();

//alert(sValue);
//alert(sValue_sec);
//alert(smem);
   if(sValue.length <= 0 && sValue_sec.length <= 0)
 
     return;
  // 
        
  //if(sValue_sec.length <= 0)
  // alert("Please select registration end date");
        

    this._oCtlValue['value'] = sValue;
    this._oCtlValue_sec['value'] = sValue_sec;
    this._oCtlValue_mem['value'] = smem;
...
var ctl_value1;
    ...
    $.each(this._oCtlValue, function(sKey, sValue) {
        ...
        //oOptions['ctl_value[]'].push(sKey + '=' + sValue1);
    });
//oOptions['ctl_value[]'] = new Array();
   $.each(this._oCtlValue_sec, function(sKey, sValue_sec) {
        oOptions['ctl_value[]'].push(sKey + '=' + sValue_sec);
        //oOptions['ctl_value[]'].push(sKey + '=' + sValue1);
    });
     $.each(this._oCtlValue_mem, function(sKey, smem) {
        oOptions['ctl_value[]'].push(sKey + '=' + smem);
        //oOptions['ctl_value[]'].push(sKey + '=' + sValue1);
    });
    $.post(
        this._sActionsUrl,
