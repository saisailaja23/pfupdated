...
$_page = array(
    ...
    'js_name' => array('profiles_payment.js'),
    'header' => _t('Payment reports')
);
$_page_cont[$iNameIndex] = array(
    ...
    'actions_url' => $GLOBALS['site']['url_admin'] . 'paymentreport.php',
...
function PageCodeControls($sDefault = BX_DOL_ADM_MP_CTL) {
    ...
    return DesignBoxAdmin(_t('please select payment date'), $oAdmTemplate->parseHtmlByName('mp_controls_payment.html', $aParams), $aTopMenu);
}
...
//foreach ($aInputkidsprogram as $k => $r) {
    ...
    $aForm = array(
        'form_attrs' => array(
            'id' => 'adm-mp-search',
            'action' => $GLOBALS['site']['url_admin'] . 'paymentreport.php',
            'method' => 'post',
...
function PageCodeMembers($sDefaultCtl = BX_DOL_ADM_MP_CTL, $sDefaultView = BX_DOL_ADM_MP_VIEW) {
    ...
    $aResult = array(
    	'action_url' => $GLOBALS['site']['url_admin'] . 'paymentreport.php',
    ...
    return DesignBoxAdmin(_t('Members Listing'), $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_payment.html', $aResult), $aTopMenu);
}
    ...
    switch($aParams['ctl_type']) {
        case 'qlinks':
         ...
         // between '2008-12-23'and '2008-12-25';
            ...
            // }
           //  $sWhereClause .=  $memtypes. " AND  DATE_FORMAT(FROM_UNIXTIME(`vt`.`date`), '%Y-%m-%d')  between '".   $new_date."'   AND '". $new_date1. "'";
...
//print_r($_POST);
    ...
    $oPaginate = new BxDolPaginate(array(
        ...
        'count' => (int)db_value("SELECT COUNT(`tp`.`ID`)
       ...
       AND
       1 AND (`tp`.`Couple`=0 OR `tp`.`Couple`>`tp`.`ID`) AND `vt`.client_id = `tlm`.`IDMember`   AND `vt`.`amount` > 0 AND `vt`.order_id = `tlm`.`TransactionID`  "  . $sWhereClause),
       'per_page' => $aParams['view_per_page'],
       'page_url' => $GLOBALS['site']['url_admin'] . 'paymentreport.php?start={start}',
...
function getMembersGeeky($aProfiles, $sPaginate) {
      ...
      //  $_adoptAgency=db_arr("SELECT title FROM bx_groups_main WHERE id=".$aProfile['AdoptionAgency']);
        $aItems[] = array(
            ...
            'last_name' => $aProfile['lastname'],
             'clientid' => $aProfile['clientid'],
            'paydate' => $aProfile['date'],
            'amount' => $aProfile['amount'],
            'email' => $sEmail,
    ...
    return $GLOBALS['oAdmTemplate']->parseHtmlByName('mp_members_payment_geeky.html', array(
