...
echo "Start <br/><br/>";
$getparents     = "SELECT `ID` ,`ProfileType`, `NickName` FROM `Profiles`  WHERE `Status` = 'Active'  AND `ProfileType` !=8";
...
while($row_parebts = mysql_fetch_assoc($result_parents))
{
    ...
    if(!$userPDFCount)
    {
       echo "Pdf Creating ".$createCounter."<br/>";
       try
       {
		  ...
		  if($fileName == 'no data')
		   {
	    			$log_file   = $GLOBALS['dir']['root']."PDFTemplates/parent_log.txt";
        			ob_start();
				echo "counter ".$createCounter."\n";
        			echo "parent name ".$row_parebts['NickName']."\n";
...
class generatePDF
{
    ...
    public function __construct($pID=NULL,$pType=NULL)
    {
        ...
        echo "coverImage ".$this->coverImage."<br/>";
        echo "blockContPost ".$this->blockContPost."<br/>";
        echo "pictureSelect ".$this->pictureSelect."<br/>";
        //exit();
        //echo "defaultPDF ";
        ...
        if($this->coverImageArray)
        {
            ...
            if($imagepath &&  $this->checkurlFileexists($imagepath))
            {
                    ...
                    if(!$imgtag)
                    {
                        ...
                        $image->resize($containreWidth,$containerHeight);
                        $image->save($this->basepath."PDFTemplates/temp/$sessionID.jpg");
                        $resizeImg    = "PDFTemplates/temp/$sessionID.jpg";
                    $coverimage   = $this->baseurl.$resizeImg;
                    }
                    else
                    {
            ...
            if($coverimage || ($imageResolution))
            {
                if($imgtag)
                {
                    if($specialeffects)
                    {
                        $sp_var       = "width: ".$containreWidth."px;height: ".$containerHeight."px;-moz-border-radius: ".($containreWidth/2)."px / ".($containerHeight/2)."px;border-radius: ".($containreWidth/2)."px / ".($containerHeight/2)."px";
                        $coverimage   = '<img src="'.$imageResolution['profimage'].'" height="'.$imageResolution['imageHeight'].'" width ="'.$imageResolution['imageWidth'].'" style="margin-top:0px;margin-left:0px;'.$sp_var.'"/>';
                    }
                    else
                        $coverimage   = '<img src="'.$imageResolution['profimage'].'" height="'.$imageResolution['imageHeight'].'" width ="'.$imageResolution['imageWidth'].'" style="margin-top:'.$imageResolution['margintop'].'px;margin-left:'.$imageResolution['marginleft'].'px;"/>';
        ...
        for($photocount=0;$photocount<count($photoArray);($photocount=($photocount+$TotalSEtCountArray)))
        {
            ...
            for($phtcnt=0;$phtcnt<$numphotoCount;$phtcnt++)
            {
                ...
                if ($photoArray[$photocount + $phtcnt])
                {
                    ...
                    list($tempvar,$containreWidth,$containerHeight,$rotateAngle,$imgtag) = split('_',$this->phototalbumArray[$phtcnt]);

                    if(!$imgtag)
                    {
                        $image = new SimpleImage();
                        $image->load($photoArray[$photocount + $phtcnt]);
                        $image->resize($containreWidth,$containerHeight);
                        $image->save($this->basepath."PDFTemplates/temp/$sessionID.jpg");
                        $resizeImg    = "PDFTemplates/temp/$sessionID.jpg";
                ...
                else
                {
                        ...
                        $imageResolution    = getUploadImageResolution($imagepath,$containerHeight,$containreWidth);
                        /*echo "image path ".$imagepath."<br/>";
                        echo "container height ".$containerHeight."<br/>";
                        echo "container width ".$containreWidth."<br/>";
                        list($width, $height, $type, $attr) = getimagesize($imagepath);
                        echo "actual height ".$height."<br/>";
                        echo "actual width ".$width."<br/>";
                ...
                else
                {
                    ...
                    //$imgName      = Rotate_img($no_image,$rotateAngle,$sessionID);
                    //$resizeImg    = Resize_temp($no_image,$containreWidth,$containerHeight,$sessionID);
                    list($tempvar,$containreWidth,$containerHeight,$rotateAngle,$imgtag) = split('_',$this->phototalbumArray[$phtcnt]);
                    if(!$imgtag)
                    {
                        $image = new SimpleImage();
                ...
                {
                    if($imgtag)
                        $selImage              = '<img src="'.$imageResolution['profimage'].'" height="'.$imageResolution['imageHeight'].'" width ="'.$imageResolution['imageWidth'].'" style="margin-top:'.$imageResolution['margintop'].'px;margin-left:'.$imageResolution['marginleft'].'px;"/>';
