...
function getAllBlockData($parent_ID)
{
    ...
    $stringletterSQL = "SELECT p1.id,p1.DescriptionMe, p1.agency_letter, p1.letter_aboutThem, p1.DearBirthParent, p2.DescriptionMe AS aboutp2,
                        p1.About_our_home,p1.couple ,p1.firstname as personName, p2.firstname as partnerName, p1.couple AS couple
                        FROM Profiles p1
                        LEFT JOIN Profiles p2 ON(p2.id = p1.couple)
                        WHERE p1.id = '$parent_ID'";
    
    //DescriptionMe - LETTER ABOUT ARUN , agency_letter - AGENCY LETTER, letter_aboutThem - LETTER ABOUT THEM, 
    //DearBirthParent - EXPECTING MOTHER LETTER, DescriptionMe AS aboutp2 - LETTER ABOUT PERSON2,
   // About_our_home - ABOUT OUR HOME  ,couple
    
    $result_letter   = mysql_query($stringletterSQL);
    while($row_letter = mysql_fetch_array($result_letter))
    {
        $letterDetails     = $row_letter;
    }
    //print_r($letterDetails);
    
    //1
    if(trim($letterDetails['DescriptionMe']))
    {
        $profileblock                           = array();
        $profileblock['blockname']              = "DescriptionMe_PRF_1";
        $profileblock['blockcontent']           = $letterDetails['DescriptionMe'];
        $profileblock['blockheading']           = "LETTER ABOUT ".$letterDetails['personName'];
        $profileblockArray[]                    = $profileblock;
    }
    //2
    if(trim($letterDetails['agency_letter']))
    {
        $profileblock                           = array();
        $profileblock['blockname']              = "Agency_PRF_1";
        $profileblock['blockcontent']           = $letterDetails['agency_letter'];
        $profileblock['blockheading']           = "AGENCY LETTER";
        $profileblockArray[]                    = $profileblock;
    }
    //3
    if(trim($letterDetails['letter_aboutThem']))
    {
        $profileblock                           = array();
        $profileblock['blockname']              = "AboutThem_PRF_1";
        $profileblock['blockcontent']           = $letterDetails['letter_aboutThem'];
        $profileblock['blockheading']           = "LETTER ABOUT THEM";
        $profileblockArray[]                    = $profileblock;
    }
    if(trim($letterDetails['DearBirthParent']))
    {
        $profileblock                           = array();
        $profileblock['blockname']              = "Birthparent_PRF_1";
        $profileblock['blockcontent']           = $letterDetails['DearBirthParent'];
        $profileblock['blockheading']           = "EXPECTING MOTHER LETTER";
        $profileblockArray[]                    = $profileblock;
    }
    if(trim($letterDetails['aboutp2']) && $letterDetails['couple'] > 0)
    {
        $profileblock                           = array();
        $profileblock['blockname']              = "AboutPartner_PRF_1";
        $profileblock['blockcontent']           = $letterDetails['aboutp2'];
        $profileblock['blockheading']           = "LETTER ABOUT ".$letterDetails['partnerName'];;
        $profileblockArray[]                    = $profileblock;
    }
    //4
    if(trim($letterDetails['About_our_home']))
    {
        $profileblock                           = array();
        $profileblock['blockname']              = "About_our_home_PRF_1";
        $profileblock['blockcontent']           = $letterDetails['About_our_home'];
        $profileblock['blockheading']           = "ABOUT OUR HOME";
        $profileblockArray[]                    = $profileblock;
    }    
    
    $stringCustomLetterSQL = "SELECT label,description FROM letter WHERE profile_id = '$parent_ID'";
    $resultCustomLetter = mysql_query($stringCustomLetterSQL);
    while($rowCustomLetter = mysql_fetch_array($resultCustomLetter))
    {
        $aqbblock                           = array();
        $aqbblock['blockname']              = $rowCustomLetter['label']."_CUS_1";
        $aqbblock['blockcontent']           = $rowCustomLetter['description'];
        $aqbblock['blockheading']           = $rowCustomLetter['label'];
...
function createXMLFile_add_photo($str)
{

$pid   = getLoggedId();
$myFile = $GLOBALS['dir']['root'].'PDFUser/add_pdf_xml/'."pdfphoto_".$pid.".xml";
$myFile_Name = "pdfphoto_".$pid.".xml";
$fh = fopen($myFile, 'w') or die("can't open file");
fwrite($fh, $str);
fclose($fh);
...
function createXMLFile_add_block($str)
{

$pid   = getLoggedId();
$myFile = $GLOBALS['dir']['root'].'PDFUser/add_pdf_xml/'."pdfblock_".$pid.".xml";
$myFile_Name = "pdfblock_".$pid.".xml";
$fh = fopen($myFile, 'w') or die("can't open file");
fwrite($fh, $str);
fclose($fh);
