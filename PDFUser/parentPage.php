...
.th{ background-color: #6BD3C6; border:1px solid #6BD3C6; font-weight: bold; color: white; padding: 5px; }
...
<div style="position:relative;height:auto;width:1010px;border:0px solid red;">
    ...
    <div style="position:relative;width:auto;border:0px solid red;height:55px;">
        ...
        <nav class="nav" >
            ...
            <li style='margin-left: 682px;' class="last"><a href="javascript:void(0);" id="createpdf">Upload PDF Profile</a></li>
    ...
    <div style="position:relative;width:auto;border:0px solid red;m">
        ...
        <table style="width:100%;">
            <tr class="th"> 
                <td class="th_td" style="width:20px;"></td>
                <td class="th_td" style="width:180px;">Name</td>
                <!-- <td class="th_td" style="width:400px;">Description</td>-->
                <td class="th_td">Uploaded Date (EST)</td>
                <td class="th_td">Actions</td>
            </tr>
            <tbody id="lod_dat"></tbody>
        </table>
        <!-- -->
    </div>
...
$(function(){
    ...
    $('#lod_dat').html("<tr><td colspan='5' style='text-align:center;'><img src='/templates/base/images/loading.gif'></td></tr>");
    $.ajax({
        url: 'PDFUser/getUserPDF.php?apUser=<?php echo $parent_ID; ?>',
        type: "GET",
        cache: false,
        success: function(data) {
            $('#lod_dat').html(data);
            
			$('.createflip').on('click', function(){ 
                            createflip(this.id); });
            $('.user_pdf_view').click( function(){ xProcessView($(this)); });
            $('.user_pdf_mkdflt').click( function () {xProcessMakeDefault($(this));});
            
            $('#createpdf').click(function(){ newpdfWin(); });
            $('#del_PDF').click( function (){ pdfDelete(); }); //xProcessDelete
...
function createflip(filename){         
    var pdfname = filename;
    var poststr = "pdfname=" + pdfname;
    dhtmlx.message({
        type: "confirm",
        text: 'Are you sure you want to make this PDF as your flipbook?',
        callback: function(result) {
            if(result == true){
                dhtmlxAjax.post(siturl+"flip_process.php", poststr, function(loader) {
                    dhtmlx.message({
                        type: "alert",
                        text: "Your flipbook is in conversion process"
                    });
                });
            }
        }
...
userPDFGrid.attachEvent("onXLE", function(){
    ...
    //             dhtmlx.alert({
    //                 text: "The pdf uploaded successfully."
    //             });
    //         } else {
    //             dhtmlx.alert({
    //                 type: "error",
...
function newpdfWin(obj)
{
    uploadController.loadValues({
        ...
        onCallBack      : reload_div
    });
    uploadController.initComponent();
	$('.dhtmlx_wins_body_inner').css('height','200px');
}
...
function xProcessView(item)
{
    ...
    var flexvalues = {
        ...
        ,pdf_old_path: '/var/www/html/pf/PDFTemplates/user/'
