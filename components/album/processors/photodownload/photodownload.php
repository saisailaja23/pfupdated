...
WHERE so.id_album=$aldumid ORDER BY so.obj_order");
...
try {
	...
	$titlename = db_arr("SELECT Caption from sys_albums where ID = '$aldumid'");
	$titlenames = str_replace(' ', '', $titlename['Caption']);

	$zip->close();
	header('Content-disposition: attachment; filename=' . $titlenames . '.' . 'zip');
