...
$.widget("ui.plupload", {
	...
	_initUploader: function() {
		...
		uploader.bind('Error', function(up, err) {			
			var message, details = "";
                  var  fileExtension = err.file.name.replace(/^.*\./, '');  
  
                  if(fileExtension == 'mov' || fileExtension == 'avi' || fileExtension == 'wmv' ||fileExtension == 'mp4' ||fileExtension == 'mpeg' ||fileExtension == 'webm')
                   {
                     var max_file_size = '200mb';
			...
			switch (err.code) {
				...
				case plupload.FILE_SIZE_ERROR:
					details = o.sprintf(_("File: %s, size: %d, max file size: %d"), err.file.name,  plupload.formatSize(err.file.size), max_file_size);
					break;
