...
//echo "sdfsdfsd".$_GET['status'];exit();
...
// water mark to not matched profile Thumbails.
    ...
    $matcheduserthumbnail_agency = mysql_fetch_assoc(mysql_query("SELECT `ID` FROM `Profiles` WHERE `ID` = " . $_GET['id']));
    mysql_query("INSERT INTO `watermarkimages` VALUES ('','" . $_GET['id'] . "', '" . $_GET['id'] . "', 'Matched')");
  ...
  if (file_exists($Source_agency) && !file_exists($Destination_agency)) {
    ...
    mysql_query($sql);
        ...
        // water mark to  matched profile Thumbails to Placed profile Thumbnails.
    $placeduser_agency = mysql_fetch_assoc(mysql_query("SELECT `ID` FROM `Profiles` where `NickName`='" . $_GET['name'] . "'"));
    $placededuserthumbnail_agency = mysql_fetch_assoc(mysql_query("SELECT `ID` FROM `Profiles` WHERE `ID` = " . $placeduser_agency['ID']));
   ...
   if (file_exists($Source_agency) && !file_exists($Destination_agency)) {
    ...
    $notplaceduser_agency = mysql_fetch_assoc(mysql_query("SELECT `ID` FROM `Profiles` where `NickName`='" . $_GET['name'] . "'"));
