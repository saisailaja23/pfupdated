...
if (isset($_GET['profile'])) {
    ...
    if (isset($_GET['search'])) {
        $sql = "SELECT distinct * FROM `Profiles` WHERE (`DescriptionMe` LIKE '%$search%' or `FirstName` LIKE '%$search%' or`LastName` LIKE '%$search%' or`DearBirthParent` LIKE '%$search%' or `About_our_home` LIKE '%$search%') AND `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Status` = 'Active' Limit 12";
        ...
        if (isset($_GET['pareth']) && $_GET['pareth'] != '') {
            ...
            foreach ($pareths as $pareth) {
                $where .= " FIND_IN_SET('$pareth', `BMChildEthnicity`)  or ";
            }
        } 
        if (isset($_GET['beth']) && $_GET['beth'] != '') {
            ...
            foreach ($beths as $beth) {
                $where .= " FIND_IN_SET( '$beth' ,`ChildEthnicity` ) or ";
            }
        }
       if (isset($_GET['family_num_children']) && $_GET['family_num_children'] != '') {
        ...
        $sql = "SELECT distinct * , CHAR_LENGTH( DearBirthParent ) , COUNT( * ) FROM `Profiles`  WHERE ( " . $where . " ) AND `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Status` = 'Active'  GROUP BY ID";
        ...
        $sql = "SELECT distinct * , CHAR_LENGTH( DearBirthParent ) , COUNT( * ) FROM `Profiles`  WHERE `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Status` = 'Active'  GROUP BY ID";
    }
    ...
    $search = $_GET['search'];
    $sql = "SELECT distinct * FROM `Profiles` WHERE (`DescriptionMe` LIKE '%$search%' or `FirstName` LIKE '%$search%' or`LastName` LIKE '%$search%' or`DearBirthParent` LIKE '%$search%' or `About_our_home` LIKE '%$search%') AND `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Status` = 'Active' $limit";
    //echo $sql;
    //exit();
    $result = mysql_query($sql);
    $id = '';
    ...
    //$limit = "LIMIT 0, 70";
    $search = $_GET['search'];
    $sql = "SELECT distinct * FROM `Profiles` LEFT JOIN `bx_avatar_images` ON `Profiles`.`ID` = `bx_avatar_images`.author_id WHERE ( " . $where . " ) AND `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Status` = 'Active'  GROUP BY `Profiles`.`ID` ORDER BY  `bx_avatar_images`.id DESC $limit";
    //echo $sql;
//die;
    $result = mysql_query($sql);
    $id = '';
    ...
    $sql = "SELECT distinct * FROM `Profiles` WHERE `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Status` = 'Active' ";
    $result = mysql_query($sql);
    $id = '';
    while ($row = mysql_fetch_array($result)) {
    ...
    $sql = "SELECT distinct * , CHAR_LENGTH( DearBirthParent ) , COUNT( * ) FROM `Profiles` WHERE `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Status` = 'Active'  GROUP BY ID ORDER BY `Profiles`.`Avatar`";
    ...
    $sql = "SELECT distinct `Profiles`.* , CHAR_LENGTH( DearBirthParent ) , COUNT( * ), `bx_avatar_images`.id FROM `Profiles`  LEFT JOIN
       `bx_avatar_images` ON `Profiles`.`ID` = `bx_avatar_images`.author_id WHERE `AdoptionAgency` = '41' AND find_in_set('$childeth', ChildEthnicity) AND `ProfileType` != '8' AND `Profiles`.`Status` = 'Active' GROUP BY `Profiles`.ID ORDER BY `bx_avatar_images`.id DESC  $limit";
    // echo $sql;exit();
    $result = mysql_query($sql);
    $id = '';
    while ($row = mysql_fetch_array($result)) {
    ...
    $sql = "SELECT distinct `Profiles`.* , CHAR_LENGTH( DearBirthParent ) , COUNT( * ), `bx_avatar_images`.id FROM `Profiles`  LEFT JOIN
       `bx_avatar_images` ON `Profiles`.`ID` = `bx_avatar_images`.author_id WHERE `AdoptionAgency` = '41' AND `ProfileType` != '8' AND `Profiles`.`Status` = 'Active' GROUP BY `Profiles`.ID ORDER BY `bx_avatar_images`.id DESC  $limit";
    // echo $sql;exit();
    $result = mysql_query($sql);
    $id = '';
    while ($row = mysql_fetch_array($result)) {
...
$log = fopen("log/aa_waiting_family".date('Y-m-d').".txt", 'a+');
fwrite($log, "\n".$sql."\n /* ========================== */ \n");
fclose($log);
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            if (file_exists($filename)) {
                ...
                if (file_exists($filename12)) {
                    ...
                    $aData1 = $site['url'].'modules/boonex/avatar/data/resizedphoto/' . $row[author_id] . '.jpg';
...
/*
  ...
  `bx_photos_main` ON `Profiles`.`ID` = `bx_photos_main`.Owner WHERE `AdoptionAgency` = '29' AND `ProfileType` != '8' AND `Profiles`.`Status` = 'Active' GROUP BY `Profiles`.ID ORDER BY  `bx_photos_main`.Hash  DESC $limit";
  ...
  $filename12 = 'D:/xampp/htdocs/www.pf.com/modules/boonex/avatar/data/avatarphotos/'.$row[author_id].'.jpg';
  ...
  $aData1 = 'http://localhost/www.pf.com/modules/boonex/avatar/data/avatarphotos/'.$row[author_id].'.jpg';
