...
//AQB Profile composer	
			require_once(BX_DIRECTORY_PATH_MODULES . 'aqb/pcomposer/classes/AqbPCBuilder.php');
		    	$oBuilder = new  AqbPCBuilder();
			
			if (strpos($_GET['ID'], $oBuilder -> _oMain -> _oConfig -> getCBPrefix()) !== false) 
			{	
				$sContent = $oBuilder -> getRssBlockContent($_GET['ID']);
				if ($sContent !== false) $sCont = $sContent;		
			}	
			else
			{
    $sQuery = "SELECT `Content` FROM `sys_page_compose` WHERE `ID` = " . (int)$_GET['ID'];
