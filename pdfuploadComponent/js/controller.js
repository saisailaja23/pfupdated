    ...
    _window :  function(uid,winWidth,winHeight,winTop,winTitle) {
        ...
        try{
            ...
            if(self.windows[uid] == 'undefined')
                self.windows[uid] = [];
            self.windows[uid] = self.window_manager.createWindow( 'Win'+uid, '0', '0', 450, 400);
        ...
        self.form[ uid ].attachEvent("onFileAdd",function(realName){
            ...
            for(var i=0; i<self.formats.length; i++) {
                if(extnsn[extnsn.length-1].toLowerCase() != self.formats[i].toLowerCase()){
					...
					var des = self.form[ uid ].getItemValue("Description"); 
						var poststr = "temp_description=" + des;
						dhtmlxAjax.post(siteurl + "pdfuploadComponent/insert_pf_deatils.php", poststr, function(loader) {
						self.myUploader.upload();
					});
                }
            }			
