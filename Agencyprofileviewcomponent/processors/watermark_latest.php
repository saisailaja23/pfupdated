...
$User_name = mysql_fetch_assoc(mysql_query("SELECT `NickName` FROM `Profiles` where `ID`='" . $_POST['id'] . "'"));

$ipaddr = $_SERVER['REMOTE_ADDR'];
$stat = ($_POST['status'] == 'none') ? $_POST['stat'] : $_POST['status'];
mysql_query("INSERT INTO `profile_status` (`UserId`, `Status`, `IPaddr`,`AgencyId`) VALUES ('" . $_POST['id'] . "', '" . $stat . "', '" . $ipaddr . "','".getLoggedId()."')");


$agencyId = mysql_fetch_assoc(mysql_query("SELECT bx_groups_main.author_id AS bxid FROM Profiles JOIN bx_groups_main WHERE  Profiles.AdoptionAgency=bx_groups_main.id AND Profiles.ID  IN (SELECT bx_groups_main.author_id FROM Profiles JOIN bx_groups_main WHERE Profiles.ID = " . $_POST['id'] . " AND Profiles.AdoptionAgency=bx_groups_main.id)"));

if ($_POST['status'] == 'Matched') {

	$placeduser = mysql_fetch_assoc(mysql_query("SELECT `author_id` FROM `watermarkimages` where `author_id`='" . $_POST['id'] . "'"));
        mysql_query("UPDATE  `Profiles` SET  `Status` =  'Active' WHERE  `Profiles`.`ID` =".$_POST['id']);
        mysql_query("UPDATE  `Profiles_draft` SET  `Status` =  'Active' WHERE  `Profiles`.`ID` =".$_POST['id']);

    if($placeduser['author_id'] == '') {

		// water mark to not matched profile Thumbails.
		$matcheduserthumbnail = mysql_fetch_assoc(mysql_query("SELECT `Avatar` FROM `Profiles` WHERE `ID` = " . $_POST['id']));
//                echo "INSERT INTO `watermarkimages` (`author_id`,`watermarkimage_id`,`status`) VALUES ('" . $_POST['id'] . "', '" . $matcheduserthumbnail['Avatar'] . "', 'Matched')";
                if($matcheduserthumbnail['Avatar'])
                    mysql_query("INSERT INTO `watermarkimages` (`author_id`,`watermarkimage_id`,`status`) VALUES ('" . $_POST['id'] . "', '" . $matcheduserthumbnail['Avatar'] . "', 'Matched')");
                else
                    mysql_query("INSERT INTO `watermarkimages` (`author_id`,`status`) VALUES ('" . $_POST['id'] . "', 'Matched')");

		$matcheduserthumbnail_agency = mysql_fetch_assoc(mysql_query("SELECT `ID` FROM `Profiles` WHERE `ID` = " . $_POST['id']));
		mysql_query("INSERT INTO `watermarkimages` VALUES ('','" . $_POST['id'] . "', '" . $matcheduserthumbnail['Avatar'] . "', 'Matched')");
	...
	if ($agencyId['bxid'] == 73 || $agencyId['bxid'] == 8117) {
		$overlayimage = BX_DIRECTORY_PATH_BASE . 'images/icons/Matched_Feb_HOA.png';
		$overlayimage_large = BX_DIRECTORY_PATH_BASE . 'images/icons/Matched_HOA.png';
		$overlayimage_agency = BX_DIRECTORY_PATH_BASE . 'images/icons/Matched_HOA.png';
		$overlayimage_favourite = BX_DIRECTORY_PATH_BASE . 'images/icons/Matched_HOA.png';
	}
    ...
    if($placeduser['author_id'] != '') {
		...
		// echo $sql;
		$placeduserthumbnail = mysql_fetch_assoc(mysql_query("SELECT `Avatar` FROM `Profiles` WHERE `ID` = " . $_POST['id']));
                if($placeduserthumbnail['Avatar'])
                    mysql_query("INSERT INTO `watermarkimages` (`author_id`,`watermarkimage_id`,`status`) VALUES ('" . $_POST['id'] . "', '" . $placeduserthumbnail['Avatar'] . "', 'Placed')");
                else
                   mysql_query("INSERT INTO `watermarkimages` (`author_id`,`status`) VALUES ('" . $_POST['id'] . "', 'Placed')"); 
		//  mysql_query($sql);
	...
	if ($agencyId['bxid'] == 73 || $agencyId['bxid'] == 8117) {
		$overlayimage = BX_DIRECTORY_PATH_BASE . 'images/icons/placed_Feb_HOA.png';
		$overlayimage_large = BX_DIRECTORY_PATH_BASE . 'images/icons/placed_HOA.png';
		$overlayimage_agency = BX_DIRECTORY_PATH_BASE . 'images/icons/placed_HOA.png';
		$overlayimage_favourite = BX_DIRECTORY_PATH_BASE . 'images/icons/placed_HOA.png';
	}
