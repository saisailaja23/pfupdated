...
$Avatar = "SELECT AV.`ID`,AV.`author_id`,P.FirstName,P.LastName  FROM `bx_avatar_images` as AV ,`Profiles` as P where  AV.`author_id` = P.`ID`
            and AV.`ID` = P.`Avatar` AND profileType='2'  AND `P`.`Status` = 'Active' and `P`.`ProfileType` = 2 and `P`.`Avatar` != '0'
             AND (`P`.`Couple` = 0 or `P`.`Couple` > `P`.`ID`) ORDER BY  RAND() , `P`.`DateLastLogin` DESC LIMIT 4";
$query = mysql_query($Avatar);
$cmdtuples = mysql_num_rows($query);
$arrRows_Profiles = array();
while (($row = mysql_fetch_array($query, MYSQL_BOTH))) {

  $NickName = $site['url'].'extra_profile_view_17.php?id='.$row[1];
