...
echo $aData;  
	...
	$sql = "SELECT distinct * , CHAR_LENGTH( DearBirthParent ) , COUNT( * ) FROM `Profiles` LEFT JOIN `sys_acl_levels_members` ON `Profiles`.`ID` = `sys_acl_levels_members`.IDMember WHERE `AdoptionAgency` = '29' AND `ProfileType` != '8' AND `Status` = 'Active' AND `sys_acl_levels_members`.IDLevel =5 GROUP BY ID";
    }
...
while($row = mysql_fetch_array($result))
{
  ...
  //echo $id;
   ...
   $where .= "`DescriptionMe` LIKE '1'";
    ...
    $search = $_GET['search'];
    $sql = "SELECT distinct * FROM `Profiles` LEFT JOIN `sys_acl_levels_members` ON `Profiles`.`ID` = `sys_acl_levels_members`.IDMember WHERE ( ".$where." ) AND `AdoptionAgency` = '29' AND `ProfileType` != '8' AND `Status` = 'Active' AND `sys_acl_levels_members`.IDLevel =5 Limit 12";
	//echo $sql; die;
    $result = mysql_query($sql);
    $id = '';
    $i = 0;
  ...
  //echo $id;
   ...
   $sql = "SELECT distinct * FROM `Profiles` LEFT JOIN `sys_acl_levels_members` ON `Profiles`.`ID` = `sys_acl_levels_members`.IDMember WHERE `AdoptionAgency` = '29' AND `ProfileType` != '8' AND `Status` = 'Active' AND `sys_acl_levels_members`.IDLevel =5";
    $result = mysql_query($sql);
    $id = '';
    while($row = mysql_fetch_array($result))
  {
...
elseif(isset($_GET['featuredFamily'])){
	...
	$sql = "SELECT distinct * , CHAR_LENGTH( DearBirthParent ) , COUNT( * ) FROM `Profiles` LEFT JOIN `sys_acl_levels_members` ON `Profiles`.`ID` = `sys_acl_levels_members`.IDMember WHERE `AdoptionAgency` = '29' AND `ProfileType` != '8' AND `Status` = 'Active' AND `sys_acl_levels_members`.IDLevel =5 GROUP BY ID ORDER BY `Profiles`.`Avatar`";
...
if($ffprofile>0)
{
   ...
   $aData .= $aCopB[FirstName].";-";
      ...
      $sql = "SELECT AV.*  FROM `bx_avatar_images` as AV INNER JOIN `Profiles` as P ON AV.`author_id` = P.`ID` WHERE P.`ID` = '$ffprofile'";
      $result = mysql_query($sql);
$aData1='';
      while($row = mysql_fetch_array($result))
       {
