...
EOF;
  ...
  //  $_page['header_text'] = _t("_Member Login1");
...
function mem_expiration_letter( $ID, $membership_name, $expire_days )
{
	...
	// $recipient  = $p_arr['Email'];
       ...
       $recipient  = $p_arr['Email'].", info@cairsolutions.com";
...
function check_login($ID, $passwd, $iRole = BX_DOL_ROLE_MEMBER, $error_handle = true)
{
    ...
    if (strcmp($aProfile['Password'], $passwd) != 0) {
        ...
        /* added this below comment to prevent page from redirecting when password is in correct */
//        if ($error_handle)
  ...
  //  require_once (BX_DIRECTORY_PATH_CLASSES . 'BxDolCmts.php'); 
   ...
   // $oCmts->onAuthorDelete($ID);
	...
	//db_res( "DELETE FROM `Profiles` WHERE `ID` = '{$ID}'" );
         ...
         db_res( "Update `Profiles` SET Status='Rejected' WHERE `ID` = '{$ID}'" );
	
	 //   $iPossibleCoupleID2 = (int)db_value( "SELECT `ID` FROM `Profiles_draft` WHERE `Couple` = '{$ID}'" );
   // if ($iPossibleCoupleID) {
      //  db_res( "DELETE FROM `Profiles_draft` WHERE `ID` = '{$iPossibleCoupleID}'" );
        //delete cache file
       // deleteUserDataFile( $iPossibleCoupleID );
   // }

   // db_res( "DELETE FROM `Profiles_draft` WHERE `ID` = '{$ID}'" );

	 db_res( "Update `Profiles_draft` SET Status='Rejected' WHERE `ID` = '{$ID}'" );
...
$log = fopen($dir['root']."log/Userdelete/userdeletelog".date('Y-m-d').".txt", 'a+');
fwrite($log, "\n /* =============================".'memberID : '.$ID .  'NickName : ' .$sNickName .  ' Couple: ' .$iPossibleCoupleID. ' IP Address: ' .$Ipadd."\n /* ========================== */ \n");
fclose($log);

}


function profile_delete11($ID) {
	//global $MySQL;
	global $dir;
