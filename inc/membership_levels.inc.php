...
{
		$sCacheName = 'arrMemLevel'.$memberID.rand();
...
function checkAction($iMemberId, $actionID, $performAction = false, $iForcedProfID = 0, $isCheckMemberStatus = true)
{
    ...
    if($arrMembership['ID'] != MEMBERSHIP_ID_NON_MEMBER || $logged['admin'] || $logged['moderator']) {
        ...
        if ($isCheckMemberStatus) {
            ...
            if ($active['Status'] != 'Active' && $active['Status'] != 'Inactive') {
...
function setMembership($iMemberId, $iMembershipId, $iDays = 0, $bStartsNow = false, $sTransactionId = '', $isSendMail = true)
{
    ...
    $iDateStarts = time();
    if ($latestMembership['ID'] != $membershipID)$bStartsNow=true;
    if(!$bStartsNow) {
        /**
        ...
        if(!is_null($aMembershipLatest['DateExpires'])) {
 		...
 		// if membership already exists then it's unlimited - just shift or delete it
			$res = db_res("
				SELECT	IDMember
				FROM	`sys_acl_levels_members`
				WHERE	IDMember = $iMemberId
						AND UNIX_TIMESTAMP(DateStarts) = $iDateStarts
						AND IDLevel = $iMembershipId");
			...
			if($res[0])
			{
				...
				else
				{
					db_res("UPDATE	`sys_acl_levels_members`
							SET		DateStarts = FROM_UNIXTIME(". ((int)$iDateStarts + $iDays * $SECONDS_IN_DAY) .")
							WHERE	IDMember = $iMemberId
									AND UNIX_TIMESTAMP(DateStarts) = $iDateStarts
								AND IDLevel = $iMembershipId");
				}
		...
		//delete previous profile's membership level
                db_res("INSERT INTO sys_acl_levels_members_history SELECT * FROM `sys_acl_levels_members` WHERE `IDMember` = {$iMemberId}");
   ...
   db_res("
		INSERT `sys_acl_levels_members` (IDMember, IDLevel, DateStarts, DateExpires, TransactionID,Upgrade)
		VALUES ($iMemberId, $iMembershipId, FROM_UNIXTIME($iDateStarts), FROM_UNIXTIME($iDateExpires), '$sTransactionId', NOW())");
...
if(db_affected_rows() <= 0)
 ...
 // Added by prashanth status changes on membership upgrade   
    
$Profiledet = getProfileInfo($iMemberId);   
    
if(@mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE id=".$Profiledet['AdoptionAgency']." AND author_id=".$iMemberId))){
$mFlag=false;
}else
{
if(@mysql_num_rows(mysql_query("SELECT GroupId FROM bx_groups_moderation WHERE GroupId =".$Profiledet['AdoptionAgency']."  AND ApproveStatus= 'on' AND Type = 'profiles'"))){
$mFlag=true;
 }
else {
$mFlag=false;
...
if($Profiledet['Status'] == 'Unconfirmed')
{
  db_res("Update Profiles SET Status ='$sStatusText' where ID = '$iMemberId'");  
  db_res("Update Profiles_draft SET Status ='$sStatusText' where ID = '$iMemberId'");  
  // Sending user notification
 // usernotification_upgrade($iMemberId);
 // sendNewUserNotify_upgrade($iMemberId);
...
function usernotification_upgrade($iMemID) {
...
$sEmails    = $aMember['Email'].','.$agencymail.','.'info1@cairsolutions.com';
 // $sEmail    = $aMember['Email'].','.'info1@cairsolutions.com';
 return sendMail( $sEmails, $aTemplate['Subject'],$sMessageBody, $iMemID , 1 );
}

function sendNewUserNotify_upgrade( $iMemID )
{
        $iMemID = (int)$iMemID;
        $aMember = getProfileInfo( $iMemID );
        if( !$aMember )
        ...
        // $sEmail    = 'info1@cairsolutions.com'.','.'cairs1@cairsolutions.com';
         ...
         return sendMail($sEmail, $aTemplate['Subject'], $aTemplate['Body'], $iMemID, 1 );
}
/**
