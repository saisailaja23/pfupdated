window.IDICommon=window.IDICommon||(function(){return{getHash:function(a){var b=a.indexOf("#")+1;return b?a.substr(b):""},htmlEscape:function(a){return/[&<>\"]/.test(a)?a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;"):a},makeIframeTag:function(a){var b="<iframe";for(var d in a)b+=" "+d+'="'+IDICommon.htmlEscape(a[d])+'"';return b+"></iframe>"},getIframe:function(a,b){try{return a.frames[b]}catch(d){return null}},makeIframeNode:function(a){var b=document.createElement("iframe");
for(var d in a)b.setAttribute(d,a[d]);return b},appendHiddenIframe:function(a,b){setTimeout(function(){document.body.appendChild(IDICommon.makeIframeNode({id:a,name:a,src:b,width:0,height:0,frameBorder:0}))},0)},writeHiddenIframe:function(a,b){document.write(IDICommon.makeIframeTag({id:a,name:a,src:b,width:0,height:0,frameBorder:0}))},splitURIComponent:function(a,b){var d=[],j=a.length,g=0;while(g<j){var k=a.substr(g,b),l=k.length;if(g+l<j)for(var m=1;m<3;++m)if(k.charAt(l-m)=="%")k=k.substr(0,l-=
m);d.push(k);g+=l}return d},MAX_URL_LENGTH:4095,IDI_DEFAULT_POLLING_INTERVAL:1000}})();window.IDIModule=window.IDIModule||(function(){var a=0,b=0,d="",j=0,g="",k={};function l(f){if(window.parent!=window){var c;while(c=IDICommon.getIframe(window.parent,window.name+"_"+b)){try{if(c.location.href=="about:blank")break}catch(n){break}d+=IDICommon.getHash(c.location.href);++b}var i=d.split("$"),h=i.length-1;if(h>0){d=i[h];for(var e=0;e<h;++e)f(decodeURIComponent(i[e]))}}}function m(){var f=window.location.href,c=IDICommon.getHash(f);if(!c&&f.charAt(f.length-1)=="#"&&window.parent!=window){var n;
while(n=IDICommon.getIframe(window.parent,window.name+"_"+b)){c+=IDICommon.getHash(n.location.href);++b;if(c.charAt(c.length-1)=="$")break}if(c.charAt(c.length-1)=="$")c=c.substr(0,c.length-1)}if(c){var i=c.split("&");for(var h=0;h<i.length;++h){var e=i[h].split("=");if(e.length==2){var o=decodeURIComponent(e[0]),p=decodeURIComponent(e[1]);if(o.indexOf("up_")==0||o.indexOf("msg_")==0)k[o]=p;else if(o=="idi_hr")g=p}}}}m();function q(){return k}function r(f){g=f}function s(f){if(g){var c=encodeURIComponent(f)+
"$",n=IDICommon.MAX_URL_LENGTH-1-g.length,i=IDICommon.splitURIComponent(c,n),h=i.length;for(var e=0;e<h;++e)IDICommon.appendHiddenIframe(window.name+"_"+(a+e),g+"#"+i[e]);a+=h}}function t(f,c){j&&window.clearInterval(j);j=window.setInterval(function(){l(f)},typeof c=="object"&&c.pollingInterval||IDICommon.IDI_DEFAULT_POLLING_INTERVAL)}function u(){window.clearInterval(j);j=0}return{getPrefs:q,setHostRelayUrl:r,postMessageToHost:s,registerListener:t,unregisterListener:u}})();
