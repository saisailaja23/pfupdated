	...
	function MailBox()
	{
			...
			if ( sMessagesId )
			{
				if ( confirm(this.sSurecaption) )
				{
					...
					$('#'+this.sResponceBlock).load(sPageUrl, { 'action' : 'hide_deleted' });
                       ...
                       // Changed into normal alert box to dhtmlx alert box by prashanth 
                           
                             dhtmlx.message({ type: "alert-error", text: this.sErrorMessage                                                              
                            })            
				//alert(this.sErrorMessage);
		}
	   ...
	   var sComposeSubject =  $.trim($('#' + htmlSelectors[1]).attr('value'));
         ...
         var iChars = "!@#$%^&*()+=-[]\\\';,./{}|\":<>?";
              for (var i = 0; i < sRecipientNick.length; i++) {
           if (iChars.indexOf(sRecipientNick.charAt(i)) != -1) {
           sErrorMessage = 'Please enter valid recipient'; 
           }
              }
			...
			else
			{
                           ...
                           // Changed into normal alert box to dhtmlx alert box by prashanth 
                            //alert(sErrorMessage);
                             dhtmlx.message({ type: "alert-error", text: sErrorMessage                                                              
                            })
			}
		}
	}
