...
/* Deanos Per Page Head Injections Source Mod Part 1 of 4 Start */
var $_aHeadData;
var $_bCalled;
/* Deanos Per Page Head Injections Source Mod Part 1 of 4 End */
...
/* Deanos Per Page Head Injections Source Mod Part 2 of 4 Start */
        if ($this->_bCalled == false) {
          if(!empty($GLOBALS[$this->_sPrefix . 'PageTitle']))
            $sHeadTitle = $GLOBALS[$this->_sPrefix . 'PageTitle'];
          else if(isset($_page['header']))
            $sHeadTitle = $_page['header'];
            if($sHeadTitle != '') {
                $this->_aHeadData = BxDolService::call('head_injections', 'get_injection',array($sHeadTitle));
                $this->_bCalled = true;
            }
        }
/* Deanos Per Page Head Injections Source Mod Part 2 of 4 End */
...
/* Deanos Per Page Head Injections Source Mod Part 3 of 4 Start */
if($this->_aHeadData['content'] != '') $sRet .= $this->_aHeadData['content'];
/* Deanos Per Page Head Injections Source Mod Part 3 of 4 End */
                break;
            case 'page_header':
                if(!empty($GLOBALS[$this->_sPrefix . 'PageTitle']))
...
/* Deanos Per Page Head Injections Source Mod Part 4 of 4 Start */
if($this->_aHeadData['title'] != '') $sRet = $this->_aHeadData['title'];
/* Deanos Per Page Head Injections Source Mod Part 4 of 4 End */
                break;
            ...
            case 'main_splash':
             ...
             case 'cart_item':
				...
				break;				
             case 'top_status':
				$sRet = getProfileStatus();
				break;
             case 'top_profile':
				$sRet = getProfile();
				break;   
             case 'top_login_form_part_account':
				$sRet = getTopLoginPartaccount();
				break;
             case 'top_deano':
				 $sRet = getDeano();
				break;
             ...
             case 'top_membership_ID':
				 $sRet = getMembershipID();
				break;  
             case 'top_current_agency':
				 $sRet = getCurrentAgency();
				break;   
             case 'top_NickName':
				 $sRet = getUsername();
				break;                
             case 'top_Email':
				 $sRet = getProfileEmail();
				break;            
             case 'top_CreationDate':
				 $sRet = getProfileCreationDate();
				break;              
             ...
             case 'top_pdfcount':
				 $sRet = pdfprofilecount();
				break;             
            case 'load_from':
                $sRet = isset($_GET['loadFrom'])?$_GET['loadFrom']:'';
            break;
            case 'load_rows':
                $sRet = isset($_GET['rows'])?$_GET['rows']:3;
            break;            
            case 'load_columns':
                $sRet = isset($_GET['columns'])?$_GET['columns']:3;
            break;
            case 'load_agencyid':
                $sRet = isset($_GET['agencyId'])?$_GET['agencyId']:0;
            break;
            case 'profile_agencyid':
                $sRet = getAdoptionAgency();
            break;       
            case 'cache_control':
                $sRet = BX_DOL_CACHE_CONTROL_NUMBER;
            break;       
            case 'profile_id':
                $sRet = BX_DOL_PROFILE_ID;
            break;      
            case 'journal_id':
                $sRet = BX_DOL_JOURNAL_ID;
            break;      
            case 'profile_badge':
                $sRet = BX_DOL_PROFILE_BADGE;
            break;      
            case 'profile_nickname':
                $sRet = BX_DOL_PROFILE_NickName;
            break;      
           case 'flip_count':
                $sRet = !isAllowedAdd(false);
            break;        
    ...
    {
        return "<script language=\"javascript\" type=\"text/javascript\" src=\"" . $sFile . '?cache='. BX_DOL_CACHE_CONTROL_NUMBER . "\"></script>";
    }
    /**
    ...
    function _wrapInTagCss($sFile)
    {
        if (!$sFile)
            return '';
        return "<link href=\"" . $sFile. '?cache='. BX_DOL_CACHE_CONTROL_NUMBER . "\" rel=\"stylesheet\" type=\"text/css\" />";
    }
    /**
