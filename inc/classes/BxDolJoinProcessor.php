...
class BxDolJoinProcessor
{
                            ...
                            switch( $aItem['Type'] ) {
                                ...
                                case 'system':
                                    switch( $aItem['Name'] ) {
										...
										case 'LCaptcha': // AndrewP Logical captcha mod changes
                                        case 'Captcha':
    ...
    function registerMember()
    {
        ...
        if(!$iId1) {
	        ...
	        if($iId1) {
				$iPrType = (int)$this->aValues[0]['ProfileType'];
				$iAgencyId = (int)$this->aValues[0]['AdoptionAgency'];
				switch ($iPrType) {
					case 8:
						if (isset($this->aValues[0]['PromoCode'])) {
							$sPromoCode = $this->aValues[0]['PromoCode'];
							if (BxDolService::call('groups', 'check_code', array($sPromoCode, $iAgencyId)) === true) {
								BxDolService::call('groups', 'activate_with_admin', array($iAgencyId, $iId1));
							}
							else {
								bx_import('BxDolEmailTemplates');
								$oEmailTemplate = new BxDolEmailTemplates();
								$aTemplate = $oEmailTemplate->getTemplate('bx_groups_owner_failure') ;
					
								ob_start();
								echoDbg($this->aValues[0]);
								$sParameters = ob_get_clean();
								$aPlus = array(
								    'Parameters' => $sParameters,
								);					
					            sendMail(getParam('site_email_notify'), $aTemplate['Subject'], $aTemplate['Body'], '', $aPlus);
								return array(false, 'Fail');
							}
						}
						break;
					...
					default:
						if ($iAgencyId > 0)
							BxDolService::call('groups', 'add_fan', array($iAgencyId, $iId1, true));
				}
			}
        }
