          ...
          if($watermark['status'] == 'Matched')
		   {
		    ...
		    $matched = '<img  class="tipz" src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/fr_lst.gif" width="18" height="18" onClick="openbox(\''.$aProfile['NickName'].'\', 1)"  alt="Matched" id="Matched"/>';
		                  ...
		                  // $matched = '<img  class="tipz"   src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/placed_icon.png" width="18" height="18"   alt="Placed"  onClick="openbox_placed(\''.$aProfile['NickName'].'\')" />';
		                 ...
		                 //  $matched = '<img  class="tipz"   src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/placed_icon.png" width="18" height="18"   alt="Placed"  onClick=\'window.location = "'.$site['url'].'watermark_old.php?status=notplaced&name='.$aProfile['NickName'].'"\' />';
		                     $matched = '<img  class="tipz"   src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/placed_icon.png" width="18" height="18"   alt="Placed"  onClick=\'window.location = "'.$site['url'].'watermark_latest.php?status=notplaced&name='.$aProfile['NickName'].'"\' />';
		     }
		   else {
		               //  $matched = '<img  class="tipz"  src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/notmatched.png"  alt="Not Matched"  onClick="openbox_matched(\''.$aProfile['ID'].'\')" />';
		               //  $matched = '<img  class="tipz"  src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/notmatched.png"  alt="Not Matched"  onClick=\'window.location = "'.$site['url'].'watermark.php?status=matched&id='.$aProfile['ID'].'"\' />';
		                // $matched = '<img  class="tipz"  src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/notmatched.png"  alt="Not Matched"  onClick=\'window.location = "'.$site['url'].'watermark_old.php?status=matched&id='.$aProfile['ID'].'"\' />';
                                 $matched = '<img  class="tipz"  src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/notmatched.png"  alt="Not Matched"  onClick=\'window.location = "'.$site['url'].'watermark_latest.php?status=matched&id='.$aProfile['ID'].'"\' />';
                     }
...
//print_r($aProfile);exit();
       if($aProfile['DateLastLogin'] == '0000-00-00') {

        $aProfile['DateLastLogin'] = '';
	    ...
	    'profile_icon'=>'<a href='.$GLOBALS['site']['url'].'extra_profile_view_12.php?id=' .$aProfile['ID'].'><img src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/user.png" alt="visit profile"  class="tipz" /> </a>',
	    ...
	    'avatar_icon'=>'<a href='.$GLOBALS['site']['url'] .'modules/?r=avatar&ID='.$aProfile['ID'].' ><img src= "'.$GLOBALS['site']['url'].'templates/base/images/icons/cmt-male.gif" class="tipz" alt="change profile photo" /> </a>',
          //  'edit_link' => $GLOBALS['site']['url'] . 'pedit.php?ID=' . $aProfile['ID'],
            'edit_link' => $GLOBALS['site']['url'] . 'extra_profile_view_12.php?id=' . $aProfile['ID'].'&approve=1',
         ...
         // 'last_login' => date("m/d/Y", strtotime($aProfile['DateLastLogin'])),
            // 'last_login' => !empty($aProfile['DateLastLogin']) ?date("m/d/Y", strtotime($aProfile['DateLastLogin'])) : '',
           //  'last_login' => !empty($aProfile['DateLastLogin']) ?date("m/d/Y", strtotime($aProfile['DateLastLogin'])) : 'Never Visited',
              'last_login' => $aProfile['DateLastLogin'] == '0000-00-00 00:00:00' ? 'Never Visited' : date("m/d/Y", strtotime($aProfile['DateLastLogin'])),

            //'last_login' => if($aProfile['DateLastLogin'] != '' || $aProfile['DateLastLogin'] != '0000-00-00') ? '' : ("m/d/Y", strtotime($aProfile['DateLastLogin'])),
 	     'match' => $matched ,
        ...
        if ($_POST['action_activate'] && is_array($_POST['entry'])) {
            ...
            foreach ($_POST['entry'] as $iId) {
                if ($this->_oDb->activateEntry($iId)) {
                    ...
                    $aDataEntry = $this->_oDb->getEntryByTtile($iId);
                    
                   $agency_title = $aDataEntry[$this->_oDb->_sFieldTitle];
                    
                   $sqlQuery = "INSERT INTO `sys_pre_values` (`Key`, `Value`, `Order`, `LKey`) VALUES('AdoptionAgency', '$iId', '$iId', '__$agency_title')";
                   db_res($sqlQuery);   
                   $sqlQuery_keys = "INSERT INTO `sys_localization_keys` (`IDCategory`, `Key`) VALUES('1','__$agency_title')";
                   db_res($sqlQuery_keys);
                   $lang_id = db_last_id();
                   ...
                   $profilileDet = getProfileInfo($aDataEntry[$this->_oDb->_sFieldAuthorId]);
                   
                   $sqlQuery_statusupdate = "Update Profiles SET Status='Active' where ID = '$profilileDet[ID]'";
                   mysql_query($sqlQuery_statusupdate);
                   
                   $sqlQuery_statusupdate_draft = "Update Profiles_draft SET Status='Active' where ID = '$profilileDet[ID]'";
                   mysql_query($sqlQuery_statusupdate_draft);
                   
                   $mem_type = 7;
                   setMembership($profilileDet['ID'], $mem_type, $iDays = 0, $bStartsNow = false, $sTransactionId = '', $isSendMail = false);                                                       
                   
				   
                   // To trigger agency approval email
                   $rEmailTemplate = new BxDolEmailTemplates();
                   $aTemplate = $rEmailTemplate -> getTemplate( 't_Agencyapproved' ) ;
                   
                   sendMail( $profilileDet['Email'], $aTemplate['Subject'],$aTemplate['Body'], $profilileDet['ID'] );
                   
				   $sql = "select p.NickName ,p.State , p.Region , bxm.title from  bx_groups_main as bxm , Profiles as p where p.AdoptionAgency = bxm.id   and p.ID ='$profilileDet[ID]' limit 0 , 1";
				$agencyrofile_sql_res = mysql_query($sql);
				$agencyrofile_sql_array = mysql_fetch_row($agencyrofile_sql_res);
 
				   $agency_title = str_replace('&','' ,$agencyrofile_sql_array[3]);
				   $UserName = $agencyrofile_sql_array[0];
				   $State = $agencyrofile_sql_array[1];
				   $Region = $agencyrofile_sql_array[2];
					$xml = '<Accounts>
					<row no="1">
					<FL val="Account Name">'.$agency_title.'</FL>
					<FL val="Agency Contact">'.$UserName.'</FL>
					<FL val="CAIRS Products Used">ParentFinder</FL>
					<FL val="Billing State">'.$State.'</FL>
					<FL val="Billing Street">'.$Region.'</FL>
					</row>
					</Accounts>';
					$curl_url = 'https://crm.zoho.com/crm/private/xml/Accounts/insertRecords';
					$param= "authtoken=b3d6ecff0dadb8095202d088f62a4196&scope=crmapi&newFormat=1&xmlData=".$xml;
						$ch = curl_init();
						...
						xml_parser_free($p);
					$sql	= "update Profiles set  ZOHO_ID ='".$vals[4]['value']."'  where ID ='".$profilileDet['ID']."'";
...
//	    'manage_blocks'=> array('href' => BX_DOL_URL_ROOT  . 'm/aqb_pcomposer/approve_blocks','title' => _t('_manage_blocks'))
         ...
         'photos' => array('href' => BX_DOL_URL_ROOT . 'extra_agency_approve_30.php?type=photo', 'title' => _t('_moderate_photos'), 'onclick' => '', 'active' => $bSimAct),
         'videos' => array('href' => BX_DOL_URL_ROOT . 'extra_agency_approve_30.php?type=video', 'title' => _t('_moderate_videos'), 'onclick' => '', 'active' => $bAdvAct),
         'journals' => array('href' => BX_DOL_URL_ROOT . 'extra_agency_approve_30.php?type=blog', 'title' => _t('_moderate_journals'), 'onclick' => '', 'active' => $bQuiAct)     
