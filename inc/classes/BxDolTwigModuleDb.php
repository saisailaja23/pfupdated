...
class BxDolTwigModuleDb extends BxDolModuleDb
{
    ...
    function getEntryByTtile ($sid)
    {
        return $this->getRow ("SELECT * FROM `" . $this->_sPrefix . $this->_sTableMain . "` WHERE `{$this->_sFieldId}` = '$sid' LIMIT 1");
    }
 ...
 if(@mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE id=".$iEntryId." AND author_id=".$_COOKIE['memberID'])) || $GLOBALS['logged']['admin']) // Check if Viewer is an agency admin
       {
     
          if(!empty($_GET['OrderBy']))
               {
                   $_OrderVal="`p`.".$_GET['OrderBy'];
               }else{
                   $_OrderVal="`f`.`when`";
               }
		if( $_GET['OrderBy'] == 'DateLastNav' ||  $_GET['OrderBy'] == 'DateReg' ){
                 $aProfiles = $this->getAll ("SELECT SQL_CALC_FOUND_ROWS `p`.* , IF(ISNULL(`tl`.`Name`),'', `tl`.`Name`) AS `ml_name` FROM `Profiles` AS `p` INNER JOIN `" . $this->_sPrefix . $this->_sTableFans . "` AS `f` ON (`f`.`id_entry` = '$iEntryId' AND `f`.`id_profile` = `p`.`ID` AND `f`.`confirmed` = $isConfirmed AND `ProfileType` !=8 AND `p`.`Status` <> 'Rejected' AND `p`.`Status` <> 'Unconfirmed' $sFilter)     LEFT JOIN `sys_acl_levels_members` AS `tlm` ON `p`.`ID`=`tlm`.`IDMember` AND `tlm`.`DateStarts` < NOW() AND (`tlm`.`DateExpires`>NOW() || ISNULL(`tlm`.`DateExpires`))
                 LEFT JOIN `sys_acl_levels` AS `tl` ON `tlm`.`IDLevel`=`tl`.`ID` GROUP BY `p`. `ID` ORDER BY $_OrderVal DESC LIMIT $iStart, $iMaxNum");
           }
          else{
           $aProfiles = $this->getAll ("SELECT SQL_CALC_FOUND_ROWS `p`.* , IF(ISNULL(`tl`.`Name`),'', `tl`.`Name`) AS `ml_name` FROM `Profiles` AS `p` INNER JOIN `" . $this->_sPrefix . $this->_sTableFans . "` AS `f` ON (`f`.`id_entry` = '$iEntryId' AND `f`.`id_profile` = `p`.`ID` AND `f`.`confirmed` = $isConfirmed AND `ProfileType` !=8 AND `p`.`Status` <> 'Rejected' AND `p`.`Status` <> 'Unconfirmed' $sFilter)     LEFT JOIN `sys_acl_levels_members` AS `tlm` ON `p`.`ID`=`tlm`.`IDMember` AND `tlm`.`DateStarts` < NOW() AND (`tlm`.`DateExpires`>NOW() || ISNULL(`tlm`.`DateExpires`))
            LEFT JOIN `sys_acl_levels` AS `tl` ON `tlm`.`IDLevel`=`tl`.`ID` GROUP BY `p`. `ID` ORDER BY $_OrderVal ASC LIMIT $iStart, $iMaxNum");


          }      
		 }else{
           if($this->_sPrefix . $this->_sTableFans =='bx_events_participants')
           $aProfiles = $this->getAll ("SELECT SQL_CALC_FOUND_ROWS `p`.* FROM `Profiles` AS `p` INNER JOIN `" . $this->_sPrefix . $this->_sTableFans . "` AS `f` ON (`f`.`id_entry` = '$iEntryId' AND `f`.`id_profile` = `p`.`ID` AND `f`.`confirmed` = $isConfirmed AND `ProfileType` !=8 $sFilter AND `p`.`Status` <> 'Unconfirmed') ORDER BY `f`.`when` DESC LIMIT $iStart, $iMaxNum");
           else{
               $aProfiles = $this->getAll ("SELECT SQL_CALC_FOUND_ROWS `p`.* FROM `Profiles` AS `p` INNER JOIN `" . $this->_sPrefix . $this->_sTableFans . "` AS `f` ON (`f`.`id_entry` = '$iEntryId' AND `f`.`id_profile` = `p`.`ID` AND `f`.`confirmed` = $isConfirmed AND `ProfileType` !=8 AND `p`.`Status` = 'Active' $sFilter AND `p`.`Status` <> 'Unconfirmed') ORDER BY `f`.`when` DESC LIMIT $iStart, $iMaxNum");
           }
