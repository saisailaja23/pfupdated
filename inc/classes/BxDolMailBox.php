         ...
         */
         function BxDolMailBox( $sPageName, &$aMailBoxSettings )
         {
            ...
            (
               ...
               (
                   // Based on new requirement there is no Friends and Faves.
               ...
               $this -> iSendMessageStatusCode = BX_MAILBOX_SEND_FAILED;
                ...
                if ( !$isSimulateSending && $bAllowToSend ) {
                    ...
                    if ( db_res($sQuery ) ) {
                           ...
                           $profileDetails = getProfileInfo($iRecipientID);
                            
                            if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
                            $ProfileLink = $site['url'].'extra_profile_view_17.php?id='.$iRecipientID; 
                            }
                            else {
                            $ProfileLink = $site['url'].'extra_agency_view_29.php?id='.$iRecipientID;  
                            } 
                        
                      //  $sReturnMessage = MsgBox( _t('_MESSAGE_SENT', $sComposeUrl, getProfileLink($iRecipientID), $aRecipientInfo['NickName']));
                          $sReturnMessage = MsgBox( _t('_MESSAGE_SENT', $sComposeUrl, $ProfileLink, $aRecipientInfo['NickName']));
                        ...
                        if ( $aComposeSettings['send_copy'] ) {
                          ...
                          // Prashanth added for profile link change
                          
                         $profileDetails = getProfileInfo($this -> aMailBoxSettings['member_id']);
                            
                         if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
                         $aPlus['ProfileUrl'] = $GLOBALS['site']['url'].'extra_profile_view_17.php?id='.$this -> aMailBoxSettings['member_id']; 
                          }
                         else {
                         $aPlus['ProfileUrl'] = $GLOBALS['site']['url'].'extra_agency_view_29.php?id='.$this -> aMailBoxSettings['member_id'];  
                         } 
                         // Prashanth added for profile link change                          
