...
//        echo "before modifying it: " . $sMailHeader;
        
        //-------------------------------------------------------------------------        
        //-------------------------------------------------------------------------
        $sMailHeader = "{$sEmailNotify}"; 

        $ch = curl_init();
//        echo "receipent email :" . $sRecipientEmail;
//        echo "Mail Subject : ". $sMailSubject;
//        echo "Mail Body : ". $sMailBody;
//        echo "Mail Header : ". $sMailHeader;
//        echo "Mail Parameters : ". $sMailParameters;
//        exit;
        curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
        curl_setopt($ch, CURLOPT_USERPWD, 'api:key-4au3zfrreixafi-p2row0gw2r1zxw3w7');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);

        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
        curl_setopt($ch, CURLOPT_URL, 'https://api.mailgun.net/v3/parentfinder.com/messages');
        curl_setopt($ch,
                    CURLOPT_POSTFIELDS,
                     array('from' => $sMailHeader,
                          'to' => $sRecipientEmail,
...
function _format_when_group ($iSec) {
	$s = '';
...
function defineTimeIntervalGroup($iTime) {
    $iTime = time() - (int)$iTime;
    $sCode = _format_when_group($iTime);
    return $sCode;
}
...
function bx_encode_url_params ($a, $aExcludeKeys = array (), $aOnlyKeys = false)
{
  ...
  function pre_checkAction($User,$viewMem=0){
    $proType=getProfileTypeMem($User['ProfileType']);
    $sQuery="SELECT Profiles.ID
FROM Profiles
JOIN bx_groups_main
WHERE Profiles.ID =".$User['ID']."
AND bx_groups_main.author_id =".$viewMem."
AND bx_groups_main.id = Profiles.AdoptionAgency";
$sRow=mysql_num_rows(mysql_query($sQuery));
if($sRow >0)
   return true ;
else
    return false;
}
function getProfileTypeMem($sPtID){

 $sQuery="SELECT LKey FROM sys_pre_values JOIN Profiles 
     WHERE sys_pre_values.Key='ProfileType' AND sys_pre_values.Value=".$sPtID."
     AND Profiles.ProfileType=sys_pre_values.Value LIMIT 1";
    $sRow1= mysql_query($sQuery);
    $sRes= mysql_fetch_assoc($sRow1);
    return substr($sRes['LKey'], 2);
}

function isDefaultMemship($User){
    $sQuery="SELECT IDLevel FROM sys_acl_levels_members WHERE IDMember = ".$User['ID'];
   
    if(mysql_num_rows(mysql_query($sQuery))){
        return false;
    }else{
        return true;
    }
}
function is_Agencyadmin($ID){
    if(@mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE  author_id=".$ID." LIMIT 0,1")))
         return true;
    else
         return false;
}
...
function getRpxAlbumPicsUrl($_albumId){
$albumIdRes = db_res("SELECT concat(Hash,'.',Ext) As ImgName,Uri FROM bx_photos_main JOIN sys_albums_objects WHERE sys_albums_objects.id_album='$_albumId' AND sys_albums_objects.id_object=bx_photos_main.ID");
$CountPics=  mysql_num_rows($albumIdRes);
        $i=1;
global $site;

while ($row = mysql_fetch_assoc($albumIdRes)) {
    if($i < 6){
        $r_code[0].=$site['url']."m/photos/get_image/thumb/".$row['ImgName'];
        $r_code[1].=$site['url']."m/photos/view/".$row['Uri'];
        if($CountPics > 1 && $CountPics > $i++){
            $r_code[0].= "##";
            $r_code[1].= "##";
