...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$new_maxwidth =  (int) abs ($this->param('mxw', 0));
		$new_maxheight = (int) abs ($this->param('mxh', 0));
		...
		if ($new_width>$new_maxwidth) {
			$new_width=$new_maxwidth;
			$new_height = floor ($height * ($new_width / $width));
			}
		if ($new_height>$new_maxheight) {
			$new_height=$new_maxheight;
			$new_width = floor ($width * ($new_height / $height));
			}
