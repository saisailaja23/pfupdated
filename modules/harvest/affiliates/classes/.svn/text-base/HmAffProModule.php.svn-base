...
function affilates_class_import($sClassPostfix, $aModuleOverwright = array()) {
    ...
    if (!$a || $a['uri'] != 'memberships') {
        $oMain = BxDolModule::getInstance('HmAffProModule');
        $a = $oMain->_aModule;
    }
...
class HmAffProModule extends BxDolModule {
    var $_iProfileId;
    function HmAffProModule(&$aModule) {      
   	...
   	function actionHome () {
		...
		$this->_oTemplate->pageStart();
        	...
        	$oPage = new HmAffProPageMain($this);
			if(getParam('dol_aff_make_private') == 'on'){
				if($this->_oDb->isAffiliate($iId)){		
					$sCode = $oPage->getCode();	
				}else{	
	...
	function actionCampaigns(){
        ...
        $this->_oTemplate->addAdminCss('jquery-ui-1.7.3.custom.css');
        $this->_oTemplate->addAdminJs('jquery-ui-1.7.3.custom.min.js');		
        $this->_oTemplate->pageCodeAdmin(_t('_dol_aff_campaigns'));
	}
	function actionEditCampaign(){
		$sCampaignsUrl = BX_DOL_URL_ROOT.'m/affiliates/campaigns/';
		$iCid = $_GET['cid'];
		$sCode = $this->_oTemplate->getEditCampaignsAdminPage($iCid);
		$this->_oTemplate->pageStart();
			echo $this->_oTemplate->parseHtmlByName('back_button', array('url' => $sCampaignsUrl, 'button_text' => 'Back to Campaigns'));
			echo $sCode;	
        $this->_oTemplate->addAdminCss('admin.css');	
        $this->_oTemplate->pageCodeAdmin(_t('_dol_aff_edit_campaign'));
	}
