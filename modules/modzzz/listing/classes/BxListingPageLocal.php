...
class BxListingPageLocal extends BxDolTwigPageMain {
    ...
    function BxListingPageLocal(&$oMain, $sCountry, $sState) {
    ...
    function getBlockCode_StateListings() {
		...
		if($this->sState){ 
			...
			$sListings = $this->ajaxBrowse('local_state', $this->oDb->getParam('modzzz_listing_perpage_main_recent'), array(), $this->sState);
		}elseif($this->sCountry){
			$this->sUrlStart = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'local/'. $this->sCountry . '?';

			$sListings = $this->ajaxBrowse('local_country', $this->oDb->getParam('modzzz_listing_perpage_main_recent'), array(), $this->sCountry);
		} 
