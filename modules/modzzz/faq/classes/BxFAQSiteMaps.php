...
/**
 * Sitemaps generator for FAQ
 */
class BxFAQSiteMaps extends BxDolSiteMaps
{
    protected $_oModule;
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aQueryParts = array (
            ...
            'table' => "`modzzz_faq_main`", // table name
            'join' => "", // join SQL part
            'where' => "AND `status` = 'approved' AND `allow_view_faq_to` = '" . BX_DOL_PG_ALL . "'", // SQL condition, without WHERE
            'order' => " `created` ASC ", // SQL order, without ORDER BY
        );

        $this->_oModule = BxDolModule::getInstance('BxFAQModule');
    }
