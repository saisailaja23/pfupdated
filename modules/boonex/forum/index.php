...
switch ($action)
{
    ...
    case 'lock_topic':
        $orca_admin = new Admin ();		
		transCheck ($orca_admin->lock ((int)$_GET['topic_id']), '', 0);
        break;
...
*/
	...
	case 'show_hidden_topics':
		transCheck ($f->getHiddenTopicsXML(false), $gConf['dir']['xsl'] . 'forum_topics.xsl', $_GET['trans']);
		break;
    ...
    case 'goto':                
		switch (true)
		{
            ...
            case (isset($_GET['my_flags'])):
                transCheck ($f->getMyFlagsXML(true), $gConf['dir']['xsl'] . 'forum_topics_main.xsl', $_GET['debug'] ? 0 : 1);
                break;                
            case (isset($_GET['my_threads'])):
                transCheck ($f->getMyThreadsXML(true), $gConf['dir']['xsl'] . 'forum_topics_main.xsl', $_GET['debug'] ? 0 : 1);
                break;                                
            case (isset($_GET['hidden_topics'])):
                transCheck ($f->getHiddenTopicsXML(true), $gConf['dir']['xsl'] . 'forum_topics_main.xsl', $_GET['debug'] ? 0 : 1);
                break;
