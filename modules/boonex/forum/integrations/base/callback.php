...
function onPostReply ($aTopic, $sPostText, $sUser)
{
    ...
    if (BX_ORCA_INTEGRATION == 'dolphin' && !isAdmin($iProfileId)) {
     ...
     // Profile view link change by prashanth                                                                                  
       ...
       $profileDetails = getProfileInfo($iProfileId);
                            
       if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
       $ProfileLink = $site['url'].'extra_profile_view_17.php?id='.$iProfileId; 
        }
       else {
       $ProfileLink = $site['url'].'extra_agency_view_29.php?id='.$iProfileId;  
        }                           
    ...
    $aPlusOriginal = array (
       // 'PosterUrl' => $iProfileId ? getProfileLink($iProfileId) : 'javascript:void(0);' ,
        'PosterUrl' => $iProfileId ? $ProfileLink : 'javascript:void(0);' ,
...
function getUserInfo ($sUser)
{
    ...
    // Profile view link change by prashanth                                                                                  
     ...
     $profileDetails = getProfileInfo($oProfile->_iProfileID);
                            
     if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
     $ProfileLink = $site['url'].'extra_profile_view_17.php?id='.$oProfile->_iProfileID; 
      }
     else {
     $ProfileLink = $site['url'].'extra_agency_view_29.php?id='.$oProfile->_iProfileID;  
      }                           
  ...
  //  $aRet['profile_url'] = $oProfile->_iProfileID ? getProfileLink($oProfile->_iProfileID) : '';
    $aRet['profile_url'] = $oProfile->_iProfileID ? $ProfileLink : '';
