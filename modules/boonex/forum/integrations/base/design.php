...
if (isset($_page['extra_js']))
    $_page['extra_js'] .= $GLOBALS['oSysTemplate']->addJs ($gConf['url']['editor'] . 'tiny_mce_gzip.js', true);
else
    $_page['extra_js'] = $GLOBALS['oSysTemplate']->addJs ($gConf['url']['editor'] . 'tiny_mce_gzip.js', true);
...
if (isLogged()) {
    ...
    if ($oEditor)
        $sEditor .= $oEditor->attachEditor ('#fakeEditor', BX_EDITOR_FULL, true) . '<div id="fakeEditor" style="display:none;"></div>';
}
