...
Forum.prototype.editPost = function (id) {
  ...
  var settings = tinyMCE.activeEditor['settings'];
    ...
    if (document.getElementById('tinyEditor_'+id)) {
        new tinymce.Editor('tinyEditor_'+id, settings, tinymce.EditorManager).render();
        tinymce.editors['tinyEditor_'+id].setContent(postText);
    }
	}	
...
Forum.prototype.editPostCancel = function (id) {
    ...
    try {
	    tinymce.editors['tinyEditor_'+id].remove();
    } catch(err) {};
...
Forum.prototype.newTopic = function (id) { 
  ...
  var settings = tinyMCE.activeEditor['settings'];
	if ($('#tinyEditor').size()) {
        try {
		    tinymce.editors['tinyEditor'].remove();
        } catch(err) {};
	}
            ...
            function (r) {
                if (document.getElementById('tinyEditor')) {
                    if (0 < $('#tinyEditor').val().length)
                        $('#tinyEditor').val('');
                    new tinymce.Editor('tinyEditor', settings, tinymce.EditorManager).render();
                }
            } 
    );
}
...
Forum.prototype.insertImageToPost = function (sEditorId, iFileId) {
    ...
    eEditor.insertContent('<img class="forum-post-img-' + iFileId + ' forum-post-img bx-def-shadow bx-def-round-corners" src="' + sImgUrl + '" />');
}
