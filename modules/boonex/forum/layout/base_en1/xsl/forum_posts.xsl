    ...
    <xsl:variable name="menu_links">
        <xsl:if test="forum/id != 0 and topic/id != 0">
            ...
            <xsl:if test="1 = /root/logininfo/admin">
                <xsl:if test="0 = topic/locked">
                    <btn href="javascript:void(0);" onclick="return orca_admin.lock('{topic/id}', '{topic/locked}');" icon="{/root/urls/img}btn_icon_unlocked.gif">Lock</btn>
                </xsl:if>
                <xsl:if test="1 = topic/locked">
                    <btn href="javascript:void(0);" onclick="return orca_admin.lock('{topic/id}', '{topic/locked}');" icon="{/root/urls/img}btn_icon_locked.gif">Unlock</btn>
                </xsl:if>                    
            </xsl:if>                
        ...
        <xsl:with-param name="content">
            <xsl:choose>
                <xsl:when test="count(post)">
                    ...
                    <xsl:variable name="tmp_path" select="'se/images/icons/more.png'" /> <!-- workaround to not compile /base_en/ in this path -->
    ...
    <div class="forum_post_actions">        
        ...
        <span class="forum_stat">
            ...
            <xsl:if test="(not((points &lt; min_point) or (vote_user_point = -1) or (1 = hidden))) and 0 = ../topic/locked">
                <a href="javascript:void(0);" onclick="return f.postReplyWithQuote({../forum/id}, {../topic/id}, {@id});">Quote</a>
                &#8226;
            </xsl:if>
                ...
                <span class="forum_post_actions_rate_buttons">
                    <xsl:choose>
                        <xsl:when test="'' = vote_user_point and user/@name != /root/logininfo/username">
                            <a href="javascript:void(0);" onclick="return f.voteGood({@id});" style="margin-right:3px;" title="Like"><img alt="Like" class="vote_good" src="{/root/urls/img}vote_good.png" /></a>
                        </xsl:when>
                        <xsl:otherwise>					
                            <a href="javascript:void(0);" style="margin-right:3px;" title="Like"><img alt="Like" class="vote_good" src="{/root/urls/img}vote_good_gray.png" /></a>
                        </xsl:otherwise>
                    </xsl:choose>
                </span>
            </span>	
