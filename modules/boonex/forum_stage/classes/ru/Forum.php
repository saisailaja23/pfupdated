    ...
    function getSearchResultsXML ($text, $type, $forum, $u, $disp, $start = 0)
    {
        ...
        switch ($type) {
            ...
            case 'tlts':
                ...
                break;
            default:
                return '<error>Неверный тип поиска</error>';
        }
...
EOS;
    ...
    function getSearchXML ($wp = false)
    {
        ...
        if ($wp) {
            $this->setTitle ('<![CDATA[Поиск по форуму]]>');
...
EOF;
    ...
    {
        return $this->getXxxTopicsXML ($wp, 'Спрятанные темы', '_Hidden topics', 'getHiddenTopics', $start);
    }
    ...
    {
        return $this->getXxxTopicsXML ($wp, 'Мои темы', 'Темы, в которых Вы участвуете', 'getMyThreadsTopics', $start);
    }
    ...
    {
        return $this->getXxxTopicsXML ($wp, 'Отмеченные темы', 'Темы, отмеченные Вами', 'getMyFlaggedTopics', $start);
    }
...
<script language="javascript" type="text/javascript">
    if (!$isUploadSuccess)
        window.parent.alert ('Не удалось загрузить один или более файлов');
...
EOF;
    ...
    function topicAutoContinue ($t)
    {
        ...
        $sNewTopicSubj = ($aTopicFirst ? $aTopicFirst['topic_title'] : $t['topic_title']) . sprintf(' - часть %d', $iTopicIndex);
...
EOF;
    ...
    function getRssUpdatedTopics ()
    {
        ...
        while ( list (,$r) = each ($a) ) {
            ...
            $td = orca_mb_replace('/#/', $r['count_posts'], '# сообщений') . ' &#183; ' . orca_mb_replace('/#/', $ui[$r['last_post_user']]['title'], 'последний ответ от #') . ' &#183; ' . $r['cat_name'] . ' &#187; ' . $r['forum_title'];
        ...
        return $this->_rssFeed ('Обновленные темы', '', 'Обновленные темы', $lastBuildDate, $items);
    }
    ...
    function getRssUser ($user, $sort)
    {
        ...
        $sTitle = sprintf("%s сообщения", $aUser['title']);
    ...
    function getRssAll ($sort)
    {
        ...
        return $this->_rssFeed ('Cообщения', '', 'Cообщения', $lastBuildDate, $items);
    }
...
EOF;
    ...
    function _format_when ($iSec)
    {
        ...
        if ($iSec < 3600) {
            $i = round($iSec/60);
            if (0 == $i || 1 == $i) $s .= '1</b> Минуту назад';
            else $s .= $i . '</b> Минут назад';
        } else if ($iSec < 86400) {
            $i = round($iSec/60/60);
            if (0 == $i || 1 == $i) $s .= '1</b> Час назад';
            else $s .= $i . '</b> Часов назад';
        } else {
            $i = round($iSec/60/60/24);
            if (0 == $i || 1 == $i) $s .= '1</b> День назад';
            else $s .= $i . '</b> Дней назад';
        }
        return $s;
    }
    ...
    function setTitle ($s)
    {
        ...
        $GLOBALS['gConf']['title'] = '<![CDATA[' . $s . ' :: Форум ' . ']]>';
    }
