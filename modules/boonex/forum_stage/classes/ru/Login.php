...
class Login extends ThingPage
{
    ...
    var $f_join =  array (
        'username' => array (
            ...
            'err' => 'Имя пользователя должно быть от 4 до 12 символов',
        ...
        'email' => array (
            ...
            'err' => 'Электронный адрес должен быть действительным',
            'attributes' => array ('class' => 'sh'),
            ),
    );
    ...
    var $f_login =  array (
        ...
        'pwd' => array (
            ...
            'title' => 'Пароль',
            'regexp' => '/^[A-Za-z0-9_]+$/',
            'err' => 'Пароль должен соответствовать требованиям',
            'attributes' => array ('class' => 'sh'),
            ),
    );
    ...
    function joinSubmit ($p)
    {
            ...
            switch ($name) {
                case 'username':
                    if (!$this->_checkUniqUser($p[$name]))
                        $js .= $this->_getJsErr($name, 'Имя пользователя должно быть уникальным');
                    break;
                case 'email':
                    if (!$this->_checkUniqEmail($p[$name]))
                        $js .= $this->_getJsErr($name, 'Электронный адрес должен быть уникальным');
                    break;
            }
        }
        ...
        if (!$js) {
            ...
            if (!$db->insertUser($p)) {
                $js .= $this->_getJsErr('email', 'Ошибка базы данных');
            }

            // send activation mail
            $mail = new BxMail ();
            if (!$mail->sendActivationMail($p)) {
                $js .= $this->_getJsErr('email', 'Не удалось послать письмо');
            }
        }
    ...
    function loginSubmit ($p)
    {
        ...
        if (!$js) {
            ...
            if (!$db->checkLogin($p)) {
                $js .= $this->_getJsErr('pwd', 'неправильный пароль или имя');
            } else {
                setcookie ('orca_user', $p['username']);
                if (!setcookie ('orca_pwd', $p['pwd']))
                    $js .= $this->_getJsErr('pwd', 'Куки должны быть включены');
            }
        }
