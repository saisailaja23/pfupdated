...
EOF;
	...
	function getHiddenPostXML ($post_id, $force_show)
	{
		...
		if ($this->_checkUserPerm ('', '', 'hide_posts', 0))
			$gl_allow_hide_posts = 1;

		if ($this->_checkUserPerm ('', '', 'unhide_posts', 0))
			$gl_allow_unhide_posts = 1;

		if ($this->_checkUserPerm ('', '', 'download'));
            $gl_allow_download = 1;
		...
		{
			if ($this->_checkUserPerm ($r['user'], 'own', 'edit', $forum_id) && !$this->_isEditTimeout($r['post_id']))
				$allow_edit = 1;
		}			

		if (!$allow_del && $r['user'] == $this->_getLoginUserName())
		{
			if ($this->_checkUserPerm ($r['user'], 'own', 'del', $forum_id) && !$this->_isEditTimeout($r['post_id']))
				$allow_del = 1;
		}			
...
EOF;
	...
	function getPostsXML ($topic_uri, $wp)
	{
		...
		if ($this->_checkUserPerm ($u, '', 'hide_topics', 0))
			$gl_allow_hide_topics = 1;

		if ($this->_checkUserPerm ($u, '', 'unhide_topics', 0))
		...
		if ($this->_checkUserPerm ($u, '', 'hide_posts', 0))
			$gl_allow_hide_posts = 1;
			
		if ($this->_checkUserPerm ('', '', 'unhide_posts', 0))
			$gl_allow_unhide_posts = 1;
	
		if ($this->_checkUserPerm ($u, '', 'move_topics', 0))
			$gl_allow_move_topics = 1;

		if ($this->_checkUserPerm ($u, '', 'del_topics', 0))
...
EOF;
	...
	{
        return $this->getXxxTopicsXML ($wp, '[L[My Topics]]', '[L[Topics you participate in]]', 'getMyThreadsTopics');
	}
	...
	{
        return $this->getXxxTopicsXML ($wp, '[L[Flagged topics]]', '[L[Topics you have flagged]]', 'getMyFlaggedTopics');
	}
...
EOS;
	...
	function editPost ($post_id, $topic_id, $text)
	{
		...
		if ($no_access && $user == $this->_getLoginUserName())
			if ($this->_checkUserPerm ('', 'own', 'edit', $t['forum_id']) && !$this->_isEditTimeout((int)$post_id)) 
				$no_access = false;
...
EOF;
	...
	function deletePostXML ($post_id, $topic_id, $forum_id)
	{
		...
		if ($no_access && ($user = $this->fdb->getPostUser((int)$post_id)) == $this->_getLoginUserName())
			if ($this->_checkUserPerm ('', 'own', 'del', $f['forum_id']) && !$this->_isEditTimeout((int)$post_id)) 
				$no_access = false;
...
EOF;
	...
	{
        $user = $this->_getLoginUserName();
		if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm ($user, '', 'hide_posts', 0))
			return '<ret>0</ret>';
...
EOS;
    ...
    function moveTopicSubmit ($topic_id, $forum_id, $old_forum_id, $goto_new_location) {
        $user = $this->_getLoginUserName();
		if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm ($user, '', 'move_topics', 0) || !$topic_id || !$forum_id || !$old_forum_id)
			return '<ret>0</ret>';
	...
	function delTopic ($topic_id) {
        $user = $this->_getLoginUserName();
		if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm ($user, '', 'del_topics', 0))
			return '<ret>0</ret>';
	...
	{
        $user = $this->_getLoginUserName();
		if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm ($user, '', $is_hide ? 'hide_topics' : 'unhide_topics', 0))
			return '<ret>0</ret>';
...
EOF;
	...
	function getRssUpdatedTopics ()
	{
		...
		while ( list (,$r) = each ($a) )
		{
			$td = orca_mb_replace('/#/', $r['count_posts'], '[L[# posts]]') . ' &#8226; ' . orca_mb_replace('/#/', $r['last_post_user'], '[L[last reply by #]]') . ' &#8226; ' . $r['cat_name'] . ' &#187; ' . $r['forum_title'];
...
EOF;
    ...
    function _handleUpload (&$p, $post_id) {
        ...
        foreach ($attachments as $file) {
            if (false !== array_search ($file['att_hash'], $p['existing_file'])) 
                continue;                
