...
<xsl:template match="topics">
    ...
    <xsl:variable name="menu_links">
        ...
        <xsl:if test="count(cat)">            
            <btn href="{$rw_forum}{forum/uri}{$rw_forum_page}0{$rw_forum_ext}" onclick="" icon="{/root/urls/img}btn_icon_plink.gif">Постоянная ссылка</btn>
            <btn href="{$rw_rss_forum}{forum/uri}{$rw_rss_forum_ext}" onclick="" icon="{/root/urls/img}btn_icon_rss.gif">RSS-канал</btn>
        </xsl:if>
    </xsl:variable>
