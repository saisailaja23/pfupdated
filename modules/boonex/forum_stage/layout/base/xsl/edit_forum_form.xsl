...
<xsl:template match="forum">
	...
	<div class="wnd_box">
        ...
        <div class="wnd_title">
            <h2>
                <xsl:if test="@forum_id &gt; 0">Редактировать топик</xsl:if>
                <xsl:if test="0 = @forum_id">Новый форум</xsl:if>
            </h2>
        </div>			
		...
		<div class="wnd_content bx-def-padding">
            <form name="orca_edit_forum" onsubmit="var x=document.getElementById('js_code').innerHTML; eval(x); return false;">
				...
				<fieldset class="form_field_row">
                    <input type="submit" name="submit_form" value="Послать" onclick="var x=document.getElementById('js_code').innerHTML; eval(x); return false;" class="bx-btn" />
                    <input type="reset" value="Отменить" onclick="f.hideHTML(); return false;" class="bx-btn bx-def-margin-sec-left"  />
