...
<xsl:template match="posts">
    ...
    <xsl:variable name="menu_links">
        <xsl:if test="forum/id != 0 and topic/id != 0">
            ...
            <btn href="javascript:void(0);" onclick="return f.newTopic('{forum/uri}')" icon="{/root/urls/img}btn_icon_new_topic.gif">Новая тема</btn>

            <xsl:if test="0 = topic/locked">
                <btn href="javascript:void(0);" onclick="return f.postReply('{forum/id}','{topic/id}')" icon="{/root/urls/img}btn_icon_reply.gif">Ответить</btn>				
            </xsl:if>

            <xsl:if test="0 = topic/flagged">
                <btn href="javascript:void(0);" onclick="return f.flag({topic/id});" icon="{/root/urls/img}btn_icon_flag.gif">Подписаться</btn>
            </xsl:if>
            <xsl:if test="1 = topic/flagged">
                <btn href="javascript:void(0);" onclick="return f.flag({topic/id});" icon="{/root/urls/img}btn_icon_flag.gif">Отписаться</btn>
            </xsl:if>

            <xsl:if test="1 = topic/allow_lock_topics">
                <xsl:if test="0 = topic/locked">
                    <btn href="javascript:void(0);" onclick="return f.lock('{topic/id}', '{topic/locked}');" icon="{/root/urls/img}btn_icon_unlocked.gif">Блокировать</btn>
                </xsl:if>
                <xsl:if test="1 = topic/locked">
                    <btn href="javascript:void(0);" onclick="return f.lock('{topic/id}', '{topic/locked}');" icon="{/root/urls/img}btn_icon_locked.gif">Разблокировать</btn>
                </xsl:if>
            </xsl:if>

            <xsl:if test="1 = topic/allow_stick_topics">
                <xsl:if test="0 = topic/sticky">
                    <btn href="javascript:void(0);" onclick="return f.stick('{topic/id}');" icon="{/root/urls/img}btn_icon_unsticky.gif">Прикрепить</btn>
                </xsl:if>
                <xsl:if test="1 = topic/sticky">
                    <btn href="javascript:void(0);" onclick="return f.stick('{topic/id}');" icon="{/root/urls/img}btn_icon_sticky.gif">Открепить</btn>
                </xsl:if>
            </xsl:if>

            <xsl:if test="1 = topic/allow_hide_topics and 0 = topic/hidden">
                <btn href="javascript:void(0);" onclick="return f.hideTopic(1, '{topic/id}')" icon="">Спрятать тему</btn>				
            </xsl:if>   
            <xsl:if test="1 = topic/allow_unhide_topics and 1 = topic/hidden">
                <btn href="javascript:void(0);" onclick="return f.hideTopic(0, '{topic/id}')" icon="">Показать тему</btn>				
            </xsl:if>   

            <xsl:if test="1 = topic/allow_move_topics">
                <btn href="javascript:void(0);" onclick="return f.moveTopicForm('{topic/id}')" icon="">Переместить тему</btn>
            </xsl:if>   

            <xsl:if test="1 = topic/allow_del_topics">
                <btn href="javascript:void(0);" onclick="return f.delTopic('{topic/id}', '{forum/uri}', true)" icon="">Удалить тему</btn>
            </xsl:if>   

            <btn href="{$rw_topic}{topic/uri}{$rw_topic_ext}" onclick="" icon="{/root/urls/img}btn_icon_plink.gif">Постоянная ссылка</btn>

            <btn href="{$rw_rss_topic}{topic/uri}{$rw_rss_topic_ext}" onclick="" icon="{/root/urls/img}btn_icon_rss.gif">RSS-канал</btn>
                ...
                <xsl:when test="count(post)">
                    ...
                    <xsl:if test="forum/id != 0 and topic/id != 0 and 0 = topic/locked">
                        <div class="paginate bx-def-padding-left bx-def-padding-right">
                            <div class="view_all">
                                <a href="javascript:void(0);" onclick="return f.postReply('{forum/id}','{topic/id}')" icon="{/root/urls/img}btn_icon_reply.gif">Ответить</a>
                            </div>
                        </div>
                    </xsl:if>
                        ...
                        <xsl:for-each select="attachments/file">
                            <div>
                                ...
                                <span class="forum_stat">
                                    ...
                                    <xsl:if test="0 = @image">
                                        <xsl:call-template name="replace_hash">
                                            <xsl:with-param name="s" select="string('# завгрузок')"/>
                                            <xsl:with-param name="r" select="@downloads" />
                                        </xsl:call-template>
                                    </xsl:if>
                                    <xsl:if test="1 = @image">
                                        <xsl:call-template name="replace_hash">
                                            <xsl:with-param name="s" select="string('# просмотров')"/>
                                            <xsl:with-param name="r" select="@downloads" />
                                        </xsl:call-template>
                                    </xsl:if>
                                </span>
    ...
    <div class="forum_post_actions">        
        ...
        <span class="forum_stat bx-def-font-grayed">
            ...
            <xsl:if test="1 = allow_clear_report">
                <a href="javascript:void(0);" onclick="orca_admin.clearReport({@id})">Очистить жалобу</a>
                <span class="bullet"> &#183; </span>
            </xsl:if>

            <xsl:if test="1 = allow_hide_posts and 0 = hidden">
                <a href="javascript:void(0);" onclick="f.hidePost(true, {@id}, {../forum/id}, {../topic/id})">Спрятать</a>
               <span class="bullet"> &#183; </span> 
            </xsl:if>

            <xsl:if test="1 = allow_unhide_posts and 1 = hidden">
                <a href="javascript:void(0);" onclick="f.hidePost(false, {@id}, {../forum/id}, {../topic/id})">Показать</a>
                <span class="bullet"> &#183; </span>
            </xsl:if>

            <xsl:if test="allow_del = 1">
                <a href="javascript:void(0);" onclick="f.deletePost({@id}, {../forum/id}, {../topic/id}, true)">Удалить</a>
                <span class="bullet"> &#183; </span>
            </xsl:if>

            <xsl:if test="allow_edit = 1">
                <a onclick="f.editPost({@id});" href="javascript:void(0);">Редактировать</a>
                <span class="bullet"> &#183; </span>
            </xsl:if>

            <xsl:if test="(not((points &lt; min_point) or (vote_user_point = -1) or (1 = hidden))) and 0 = ../topic/locked">
                <a href="javascript:void(0);" onclick="return f.postReplyWithQuote({../forum/id}, {../topic/id}, {@id});" onmousedown="f.processSelectedText()">Цитировать</a>
                <span class="bullet"> &#183; </span>
            </xsl:if>
            ...
            <span id="report_{@id}" class="forum_post_actions_report_button">
                <xsl:if test="'' = vote_user_point and user/@name != /root/logininfo/username">
                    <a title="пожаловаться на сообщение" href="javascript:void(0);" onclick="f.report({@id}); return f.voteBad({@id});">Пожаловаться</a>
                    <span class="bullet"> &#183; </span>
                </xsl:if>
            </span>

            <xsl:if test="'' = vote_user_point and user/@name != /root/logininfo/username">
                <a href="javascript:void(0);" onclick="return f.voteBad({@id});">Отстой</a>
                <span class="bullet"> &#183; </span>
            </xsl:if>
            ...
            <span class="forum_post_actions_rate" id="rate_{@id}">
                <span class="forum_post_actions_rate_text">
                        ...
                        сообщение спрятано (
                            <xsl:choose>
                                <xsl:when test="1 = @force_show">
                                    <a href="javascript:void(0);" onclick="f.hideHiddenPost({@id})">спрятать сообщение</a>
                                </xsl:when>		
                                <xsl:otherwise>							
                                    <a href="javascript:void(0);" onclick="f.showHiddenPost({@id})">показать сообщение</a>
                                </xsl:otherwise>																
                            </xsl:choose>
                            )
                    </xsl:if>
                    ...
                    <span id="points_{@id}">
                        <xsl:call-template name="replace_hash">
                            <xsl:with-param name="s" select="string('# людям понравилось')"/>
                ...
                <span class="forum_post_actions_rate_buttons">
                    <xsl:choose>
                        <xsl:when test="'' = vote_user_point and user/@name != /root/logininfo/username">
                            <a href="javascript:void(0);" onclick="return f.voteGood({@id});" style="margin-right:3px;" title="Нравится"><i title="Нравится" class="vote_good sys-icon thumbs-up"></i></a>
                        </xsl:when>
                        <xsl:otherwise>					
                            <a href="javascript:void(0);" style="margin-right:3px;" title="Нравится"><i title="Нравится" class="vote_good sys-icon thumbs-up bx-def-font-grayed"></i></a>
                        </xsl:otherwise>
                    </xsl:choose>
                </span>
            </span>	
