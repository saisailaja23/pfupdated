	...
	<div class="wnd_box">
        ...
        <div class="wnd_content bx-def-padding">
            <form name="move_topic" onsubmit="var x=document.getElementById('js_code').innerHTML; eval(x); return false;">
                ...
                <fieldset class="form_field_row bx-def-margin-bottom"><legend>Переместить тему в форум:</legend>
                    <select name="forum">
                ...
                <fieldset class="form_field_row bx-def-margin-bottom"><legend>После перемещения перейти к:</legend>
                    <select name="goto_new_location">
                        <option value="0" selected="selected">Остаться тут</option>
                        <option value="1">Форум, куда была перемещена тема</option>
                    </select>
                </fieldset>
