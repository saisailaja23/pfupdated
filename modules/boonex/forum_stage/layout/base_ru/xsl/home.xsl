...
<xsl:template match="page">
    ...
    <xsl:variable name="menu_links">
        ...
        <btn href="javascript:void(0);" onclick="return f.newTopic('0')" icon="{/root/urls/img}btn_icon_new_topic.gif">Новая тема</btn>
<!--
		<xsl:if test="1 = /root/logininfo/admin">
            <btn href="javascript:void(0);" onclick="orca_admin.editCategories();" icon="">Форум Управление</btn>
            <btn href="javascript:void(0);" onclick="orca_admin.reportedPosts();" icon="">Сообщения, отмеченные жалобами</btn>
            <btn href="javascript:void(0);" onclick="orca_admin.hiddenPosts();" icon="">Спрятанные сообщения</btn>
            <btn href="javascript:void(0);" onclick="orca_admin.hiddenTopics();" icon="">Спрятанные темы</btn>
        </xsl:if>
-->
    </xsl:variable>

    <xsl:call-template name="box">
        <xsl:with-param name="title">Форумы Указатель</xsl:with-param>
