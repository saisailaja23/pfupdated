...
<xsl:template match="cat">
	...
	<div class="wnd_box">
        ...
        <div class="wnd_title">
            <h2>
                <xsl:if test="@cat_id &gt; 0">Редактировать группу</xsl:if>
                <xsl:if test="0 = @cat_id">Новая группа</xsl:if>
            </h2>
        </div>			
        ...
        <div class="wnd_content bx-def-padding">
            <form name="orca_edit_cat" onsubmit="var x=document.getElementById('js_code').innerHTML; eval(x); return false;">
                ...
                <fieldset class="form_field_row bx-def-margin-bottom"><legend>Название группы:</legend>
                    <input class="forum_form_field bx-def-font-large bx-def-round-corners-with-border" type="text" name="cat_name" value="{cat_name}" />
                </fieldset>

                <fieldset class="form_field_row bx-def-margin-bottom"><legend>Развернуть группу по умолчанию:</legend>
                ...
                <fieldset class="form_field_row bx-def-margin-bottom"><legend>Порядок группы:</legend>
