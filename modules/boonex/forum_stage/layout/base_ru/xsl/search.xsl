...
<xsl:template match="search">
    ...
    <xsl:variable name="menu_links">		
        ...
        <btn href="javascript:void(0);" onclick="return f.showSearch()" icon="">Новый поиск</btn>
    </xsl:variable>

    <xsl:call-template name="box">
        <xsl:with-param name="title">Резудьтаты поиска для: '<xsl:value-of select="search_text" />'</xsl:with-param>
        <xsl:with-param name="content">

            <xsl:if test="0 = count(sr)">
                <div style="text-align:center;" class="bx-def-font-large bx-def-bc-padding">
                    Результатов нет. <br />
                    Пожалуйста, <a href="javascript:void(0);" onclick="return f.showSearch()" >попробуйте снова</a>.
                </div>
            </xsl:if>
            <xsl:if test="0 != count(sr)">
