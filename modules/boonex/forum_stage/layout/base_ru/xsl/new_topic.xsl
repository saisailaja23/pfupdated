            ...
            <div class="bx-def-bc-padding">
                ...
                <form action="{/root/urls/base}" enctype="multipart/form-data" name="new_topic" method="post" target="post_new_topic" onsubmit="return f.checkPostTopicValues(this.topic_subject, this.topic_text, this.signature, this.forum, true);">
                    ...
                    <div class="bx-def-margin-bottom">
                        Форум:
                        <div class="forum_field_error_message" style="display:none" id="err_forum">Пожалуйста, выберите форум для размещения темы</div>
                        <div>
                            <select class="forum_form_field bx-def-font-large bx-def-round-corners-with-border" name="forum">
                                <option value="0">Выбрать Форум</option>
                    ...
                    <div class="bx-def-margin-bottom">
                       Название темы: 
                        <div class="forum_field_error_message" style="display:none" id="err_topic_subject">Пожалуйста, введите от 5 до 50 символов</div>
                        <div>
					...
					<xsl:if test="1 = @sticky">
                        <div class="bx-def-margin-bottom">
					        <span class="sticky"><input type="checkbox" name="topic_sticky" id="sticky" /><label for="sticky">Прикрепленные</label></span>
                        </div>
					</xsl:if>

                    Текст темы: 
                    <div class="forum_field_error_message" style="display:none" id="err_topic_text">Пожалуйста, введите от 5 до 128000 символов</div>
					...
					<div class="bx-def-margin-top">
                        <input type="submit" name="post_submit" value="Послать" onclick="tinyMCE.triggerSave(); if (!f.checkPostTopicValues(document.forms['new_topic'].topic_subject, document.forms['new_topic'].topic_text, document.forms['new_topic'].signature, document.forms['new_topic'].forum, true)) return false;" class="bx-btn" />
                        <input type="reset" name="cancel" value="Отменить" onclick="return f.cancelNewTopic('{forum/uri}', 0)" class="bx-btn" />
