...
<xsl:template match="page">
    ...
    <xsl:variable name="menu_links">
        <!-- <btn href="{/root/urls/base}" onclick="return f.selectForumIndex()" icon="">Форумы</btn> -->
        <btn href="{/root/urls/base}" onclick="" icon="">Форумы</btn>
        <btn href="javascript:void(0);" onclick="orca_admin.newCat()" icon="{/root/urls/img}btn_icon_new_cat.gif">Новая Группа</btn>        
        <xsl:for-each select="../langs/lang">
            <btn href="javascript:void(0);" onclick="return orca_admin.compileLangs('{.}')" icon="">Компилировать язык:<xsl:value-of select="." /></btn>
        </xsl:for-each>
    </xsl:variable>

    <xsl:call-template name="box">
        <xsl:with-param name="title">Форум Управление</xsl:with-param>
            ...
            <div style="position:relative;">
                ...
                <span class="forum_stat bx-def-font-grayed"> 
                    ...
                    <xsl:call-template name="replace_hash">
                        <xsl:with-param name="s" select="string('# форумов')"/>
                        <xsl:with-param name="r" select="@count_forums"/>
                    </xsl:call-template>
                    <span class="bullet"> &#183; </span>
                    <xsl:call-template name="replace_hash">
                        <xsl:with-param name="s" select="string('# топиков')"/>
                ...
                <div style="position:absolute; right:0px; top:4px; width:90px;">			
                    ...
                    <div title="редактировать" class="icn" onmouseover="this.style.backgroundPosition='0 24px'" onmouseout="this.style.backgroundPosition='0 0'" >
                    ...
                    <div title="удалить" class="icn" onmouseover="this.style.backgroundPosition='0 24px'" onmouseout="this.style.backgroundPosition='0 0'" >
                    ...
                    <div title="новый форум" class="icn" onmouseover="this.style.backgroundPosition='0 24px'" onmouseout="this.style.backgroundPosition='0 0'" >
