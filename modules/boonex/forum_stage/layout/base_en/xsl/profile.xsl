...
<xsl:template match="profile">
	...
	<div id="f_tbl">
		<ul class="tbl_hh">
			<li class="tbl_hh_profile">Информация о пользователе</li>
		</ul>
		<div class="profile">
			...
			<div style="margin-left:57px;">
				Зарегистрирован: <xsl:value-of select="join_date" />
				<br />
				Последние в сети: <xsl:value-of select="last_online" />
				<br />
				<br />
				Сообщения: <xsl:value-of select="posts" />
				<br />
				Последнее сообщение:
				<xsl:choose>
				...
				<br />	
				<xsl:if test="posts &gt; 0">
					<a href="javascript:void(0);" onclick="return f.search ('', 'tlts', 0, '{username}', 'topics');">Найти все сообщения по <xsl:value-of select="username" /></a>
				</xsl:if>
			</div>				
		</div>			
	</div>
