...
<title>
	<xsl:choose>
		<xsl:when test="string-length(/root/page/posts/topic/title) &gt; 0">
			<xsl:value-of select="/root/page/posts/topic/title" /> :: Форум
		</xsl:when>
		<xsl:when test="string-length(/root/page/topics/forum/title) &gt; 0">
			<xsl:value-of select="/root/page/topics/forum/title" /> :: Форум
		</xsl:when>
		<xsl:otherwise>
						...
						<div id="f_hello">
							...
							<xsl:choose>
								<xsl:when test="string-length(logininfo/username) &gt; 0">									
										Вошел как <b><xsl:value-of select="logininfo/username" /></b> 
                                        &#160;&#160;&#160;
                                        (
										<xsl:if test="1 = /root/logininfo/admin">
											<a href="javascript:void(0);" onclick="return orca_admin.editCategories()">Форум Управление</a>,&#160;
                                            <a href="javascript:void(0);" onclick="return orca_admin.reportedPosts()">Сообщения, отмеченные жалобами</a>,&#160;
                                            Компилировать языки:
                                        ...
                                        <a href="javascript:void(0);" onclick="return orca_login.logout()">Выйти</a>
                                        )
								</xsl:when>
								<xsl:otherwise>
								</xsl:otherwise>
                           ...
                           <xsl:choose>
							<xsl:when test="string-length(logininfo/username) &gt; 0">									
                                <span style="background-image:url({/root/urls/img}btn_icon_flags.gif)">
                                    <a href="javascript:void(0);" onclick="return f.showMyFlags()">Мои Подписки</a>
                                </span>
                                <span style="background-image:url({/root/urls/img}btn_icon_topics.gif)">
                                    <a href="javascript:void(0);" onclick="return f.showMyThreads()">Мои темы</a>
                                </span>
                            </xsl:when>
                            <xsl:otherwise>
                                <span style="background-image:url({/root/urls/img}btn_icon_join.gif)">
                                    <a href="javascript:void(0);" onclick="orca_login.showJoinForm()">Регистрация</a>
                                </span>
                                <span style="background-image:url({/root/urls/img}btn_icon_login.gif)">
                                    <a href="javascript:void(0);" onclick="orca_login.showLoginForm()">Войти</a>
                                </span>
                            </xsl:otherwise>
                            </xsl:choose>
                                <span style="background-image:url({/root/urls/img}btn_icon_search.gif)">
                                    <a href="javascript:void(0);" onclick="return f.showSearch()">Поиск</a>
                                </span>                        
                        </div>						
