...
Forum.prototype.setWindowTitle = function (s) {
    ...
    if (!s || !s.length) 
    {
        var html = $("#orca_main > .disignBoxFirst:first").html();
        var d = document.createElement('div');        
        d.innerHTML = html;        
        if (html && html.length && d.firstChild && d.firstChild.getElementsByTagName('div').length) {
            d.firstChild.removeChild(d.firstChild.getElementsByTagName('div')[0]);
            var sContentTitle = d.firstChild.innerHTML;
            if (sContentTitle.length)
                s = sContentTitle;
        }
    }
