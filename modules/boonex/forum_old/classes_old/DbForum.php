...
class DbForum extends BxDb
{
    ...
    function renameUser ($sUserOld, $sUserNew)
    {
        ...
        $aTables2Field = array (
            TF_FORUM_ACTIONS_LOG => 'user_name',
            TF_FORUM_FLAG => 'user',
            TF_FORUM_POST => 'user',
            TF_FORUM_SIGNATURES => 'user',
            TF_FORUM_TOPIC => 'first_post_user',
            TF_FORUM_USER_ACT => 'user',
            TF_FORUM_USER_STAT => 'user',
            TF_FORUM_VOTE => 'user_name',
        );

        if ($sUserNew == $gConf['anonymous']) {
            unset($aTables2Field[TF_FORUM_ACTIONS_LOG]);
            $iAffectedRows += $this->query ("DELETE FROM  " . TF_FORUM_FLAG . " WHERE `user` = '" . $this->escape($sUserOld) . "'");
            $iAffectedRows += $this->query ("DELETE FROM  " . TF_FORUM_SIGNATURES . " WHERE `user` = '" . $this->escape($sUserOld) . "'");
            $iAffectedRows += $this->query ("DELETE FROM  " . TF_FORUM_USER_ACT . " WHERE `user` = '" . $this->escape($sUserOld) . "'");
            $iAffectedRows += $this->query ("DELETE FROM  " . TF_FORUM_USER_STAT . " WHERE `user` = '" . $this->escape($sUserOld) . "'");
            $iAffectedRows += $this->query ("DELETE FROM  " . TF_FORUM_VOTE . " WHERE `user` = '" . $this->escape($sUserOld) . "'");
        }

        foreach ($aTables2Field as $sTable => $sField)
            $iAffectedRows += $this->query ("UPDATE " . $sTable . " SET `" . $sField . "` = '" . $this->escape($sUserNew) . "' WHERE `" . $sField . "` = '" . $this->escape($sUserOld) . "'");
        $iAffectedRows += $this->query ("UPDATE " . TF_FORUM_TOPIC . " SET `last_post_user` = '" . $this->escape($sUserNew) . "' WHERE `last_post_user` = '" . $this->escape($sUserOld) . "'");
