...
function getLoginUser ()
{
    ...
    if ($logged['member'] || $logged['admin'])
        return getUsername(getID((int)$_COOKIE['memberID']));
...
function forumAlert ($sAction, $iObjectId, $iSenderId, $aExtrax)
{
    $sAlertUnitPostfix = BX_ORCA_INTEGRATION == 'dolphin' ? '' : '_' . BX_ORCA_INTEGRATION;
    $oAlert = new BxDolAlerts('bx_forum' . $sAlertUnitPostfix, $sAction, $iObjectId, $iSenderId);
    $oAlert->alert();
}
