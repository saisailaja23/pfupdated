...
class BxVideosTemplate extends BxDolFilesTemplate
{
    ...
    function getFileConcept ($iFileId, $aExtra = array())
    {
        ...
        $iFileId = (int)$iFileId;
        if(empty($aExtra['ext']))
            $sPlayer = getApplicationContent('video','player',array('id' => "_".$iFileId, 'user' => $this->iViewer, 'password' => clear_xss($_COOKIE['memberPassword'])),true);
        else {
        ...
        //Aravind
        //Replaceing defalt boonex player with jwplayer.....
         $sPlayer='<div id="myElement">Loading the player...</div>
                        <script src="//ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
                        <script type="text/javascript">
                        $(function(){
                             $.getScript(site_url+"components/album/js/jwplayer/jwplayer.js",function(){
                                      jwplayer.key = "TFY8XJrqkQZEPwil4aTGwJmtN3tA+5VcSuvtE7lSeWI=";
                                  jwplayer("myElement").setup({
                                        flashplayer: site_url+"components/album/js/jwplayer/jwplayer.flash.swf",
                                        file: site_url+"flash/modules/video/files/_'. $iFileId .'.flv",
                                        image: site_url+"flash/modules/video/files/_'. $iFileId .'.jpg",
                                        width: 640,
                                        height: 480
                                    });
                           });
