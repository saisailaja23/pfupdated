        ...
        if ($iPostID > 0) {
            ...
            $oZ->alert();
                       ...
                       $rEmailTemplate = new BxDolEmailTemplates();
                                    $aTemplate = $rEmailTemplate -> getTemplate( 'Blog_delete_notification' ) ;
                                    $aPlus = array ('PostTitle' => $iPosttitle);
...
EOF;
        ...
        return DesignBoxContent(_t('_bx_blog_post_view'), $sPostStringVal, 1, '', $sSPaginate);
    }
...
EOF;
        ...
        return DesignBoxContent ($sCategoriesC, $sCategories, 1);
    }
...
EOF;
        ...
        $sMyPostsBox = DesignBoxContent ($sMyPostsC, $sMyPosts, 1, false, $sMyPostsPagination);

        return $sAdministrationUnitsSect . $sMyPostsBox;
    }

	  function GenMyPageAgencyAdmin($sMode = '') {
		$this->CheckLogged();
        ...
        $sBlogManageLink = "{$sLink}?action=agency_admin&mode=manage";
        $sBlogManageLink =  "{$sLink}?action=agency_admin&mode=manage";
        $sBlogPendingLink = "{$sLink}?action=agency_admin&mode=pending";
		...
		if (bx_get('action_delete') && is_array(bx_get('bposts'))) {
			foreach (bx_get('bposts') as $iBPostID) {
                ...
                if (bx_get('action_deactivate') && is_array(bx_get('bposts'))) {
			foreach (bx_get('bposts') as $iBPostID) {
				if ($this->_oDb->setPostStatus((int)$iBPostID)) {}
			}
		}
                if (bx_get('action_activate') && is_array(bx_get('bposts'))) {
			foreach (bx_get('bposts') as $iBPostID) {
				if ($this->_oDb->setPostStatus((int)$iBPostID, 'approval')) {}
			}
		...
		switch($sMode) {
			...
			case 'manage':
				$sManageTabClass = 1;
	$oTmpBlogSearch->aCurrent['restriction']['postStat'] = array(
                                        'value'=>'', 'field'=>'PostStatus', 'operator'=>'=', 'paramName'=>'postStat', 'table'=>'bx_blogs_posts'
                                    );
                                 $oTmpBlogSearch->aCurrent['restriction']['postStat']['value'] = 'approval';
                                   $oTmpBlogSearch->aCurrent['restriction']['activeStatus'] = '';
                            if(is_Agencyadmin((int)$_COOKIE['memberID'])){
                                 $aAgencyIdRes=mysql_query("SELECT id FROM bx_groups_main WHERE  author_id=".$_COOKIE['memberID']." LIMIT 0,1");
                                 $aAgencyIdRow=mysql_fetch_assoc($aAgencyIdRes);
                                $oTmpBlogSearch->aCurrent['restriction']['owner']['value'] = '';
                                 //Add extra join to query-> fetch only agency fan's photos
                                 $oTmpBlogSearch->aCurrent['join']['group_fans'] = array(
                                        'type' => 'left',
                                        'table' => 'bx_groups_fans',
                                        'mainField' => 'OwnerId',
                                        'onField' => 'id_profile',
                                        'joinFields' => '');
                                 // Add restrictions for fetching Agency subscribers
                                  $oTmpBlogSearch->aCurrent['restriction']['groupfan'] = array(
                                        'value'=>'', 'field'=>'id_entry', 'operator'=>'=', 'paramName'=>'groupfan', 'table'=>'bx_groups_fans'
                                    );
                                $oTmpBlogSearch->aCurrent['restriction']['groupfan']['value'] = $aAgencyIdRow['id'];
                               	$oTmpBlogSearch->bShowCheckboxes = true;
				$sBlogPostsVal = $oTmpBlogSearch->displayResultBlock();
				$sActivePosts = ($oTmpBlogSearch->aCurrent['paginate']['totalNum'] == 0) ? MsgBox(_t('_Empty')) : $sBlogPostsVal;
				$sAdmPanel = $oTmpBlogSearch->showAdminActionsPanel('bposts_box', array('action_deactivate' => '_Deactivate','action_delete' => '_Delete'), 'bposts');
...
EOF;
			...
			case 'pending':
				...
				$oTmpBlogSearch->aCurrent['restriction']['postStat'] = array(
                                 ...
                                 $oTmpBlogSearch->aCurrent['restriction']['postStat']['value'] = 'disapproval';
                                   $oTmpBlogSearch->aCurrent['restriction']['activeStatus'] = '';
				if(is_Agencyadmin((int)$_COOKIE['memberID'])){
                                 $aAgencyIdRes=mysql_query("SELECT id FROM bx_groups_main WHERE  author_id=".$_COOKIE['memberID']." LIMIT 0,1");
                                 $aAgencyIdRow=mysql_fetch_assoc($aAgencyIdRes);
                                $oTmpBlogSearch->aCurrent['restriction']['owner']['value'] = '';
                                 ...
                                 $oTmpBlogSearch->aCurrent['restriction']['groupfan']['value'] = $aAgencyIdRow['id'];
                               	...
                               	$oTmpBlogSearch->bShowCheckboxes = true;
				$sBlogPostsVal = $oTmpBlogSearch->displayResultBlock();
				$sDeActivePosts = ($oTmpBlogSearch->aCurrent['paginate']['totalNum'] == 0) ? MsgBox(_t('_Empty')) : $sBlogPostsVal;
				$sAdmPanel = $oTmpBlogSearch->showAdminActionsPanel('bposts_box', array('action_activate' => '_activate','action_delete' => '_Delete'), 'bposts');
				$sBpostsActiveUnitsForm = <<<EOF
<form id="bx_blogs_user_form" method="post">
	<div id="bposts_box" class="blog_posts_wrapper">
		{$sDeActivePosts}
...
EOF;
       ...
       $sAdmPost = BxDolPageView::getBlockCaptionMenu(mktime(), array(
		...
		$sAdministrationUnitsSect = DesignBoxContent ($sAdministrationC, $sAdmContent, 1, $sAdmPost);
		return $sAdministrationUnitsSect ;
	}
    /**
...
EOF;
        ...
        if ($oForm->isSubmittedAndValid()) {
          ...
          //  $sAutoApprovalVal = (getParam('blogAutoApproval')=='on') ? "approval" : "disapproval";
			$mFlag='';
                        if(@mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE id=".$iEntryId." AND author_id=".$_COOKIE['memberID']))){
                              $mFlag=false;
                          }else{
                               if(@mysql_num_rows(mysql_query("SELECT ID FROM  Profiles JOIN bx_groups_moderation WHERE Profiles.ID= ".(int)$_COOKIE['memberID']." AND Profiles.AdoptionAgency = bx_groups_moderation.GroupId AND bx_groups_moderation.ApproveStatus= 'on' AND bx_groups_moderation.Type = 'journal'"))){
                                  $mFlag=true;
                            }else{
                                $mFlag=false;
                            }
			...
			$sAutoApprovalVal = (getParam('blogAutoApproval')=='on' || $mFlag ) ? "approval" : "disapproval";
            ...
            if ($iPostID>0) {
                ...
                $oForm->update($iPostID, $aValsAdd);
                                /*Notification Start*/
                                if($aValsAdd['PostStatus']=="disapproval")
                                {
                                    $rEmailTemplate = new BxDolEmailTemplates();
                                    $aTemplate = $rEmailTemplate -> getTemplate( 'Blog_Edit_approval' ) ;
                                    $aPlus = array ('PostTitle' => $sPostCaption,'clickhere' =>$aBlogPost['PostUri']);
                                    $aAgencyEmail=db_arr("SELECT Profiles.* FROM Profiles JOIN bx_groups_main WHERE  Profiles.AdoptionAgency=bx_groups_main.id AND Profiles.ID  IN (SELECT bx_groups_main.author_id FROM Profiles JOIN bx_groups_main WHERE Profiles.ID =".$_COOKIE['memberID']." AND Profiles.AdoptionAgency=bx_groups_main.id )");
                                    If (!(mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE  author_id=".$_COOKIE['memberID'])))){}
                                    sendMail( $aAgencyEmail['Email'], $aTemplate['Subject'], $aTemplate['Body'], $_COOKIE['memberID'],$aPlus );
                                }else{
                                    $rEmailTemplate = new BxDolEmailTemplates();
                                    $aTemplate = $rEmailTemplate -> getTemplate( 'Blog_Edit_notification' ) ;
                                      $aPlus = array ('PostTitle' => $sPostCaption,'clickhere' =>$aBlogPost['PostUri']);
                                    $aAgencyEmail=db_arr("SELECT Profiles.* FROM Profiles JOIN bx_groups_main WHERE  Profiles.AdoptionAgency=bx_groups_main.id AND Profiles.ID  IN (SELECT bx_groups_main.author_id FROM Profiles JOIN bx_groups_main WHERE Profiles.ID =".$_COOKIE['memberID']." AND Profiles.AdoptionAgency=bx_groups_main.id )");
                                    If (!(mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE  author_id=".$_COOKIE['memberID'])))){}
                                    sendMail( $aAgencyEmail['Email'], $aTemplate['Subject'], $aTemplate['Body'], $_COOKIE['memberID'],$aPlus );
                ...
                $iBlogPostID = $oForm->insert($aValsAdd);
                                ...
                                {
                                    $rEmailTemplate = new BxDolEmailTemplates();
                                    $aTemplate = $rEmailTemplate -> getTemplate( 'Blog_addition_approval' ) ;
                                    $aPlus = array ('PostTitle' => bx_get('PostCaption'),'clickhere' =>$aValsAdd['PostUri']);
                                    $aAgencyEmail=db_arr("SELECT Profiles.* FROM Profiles JOIN bx_groups_main WHERE  Profiles.AdoptionAgency=bx_groups_main.id AND Profiles.ID  IN (SELECT bx_groups_main.author_id FROM Profiles JOIN bx_groups_main WHERE Profiles.ID =".$aValsAdd['OwnerID']." AND Profiles.AdoptionAgency=bx_groups_main.id )");
                                    If (!(mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE  author_id=".$aValsAdd['OwnerID'])))){}
                                    sendMail( $aAgencyEmail['Email'], $aTemplate['Subject'], $aTemplate['Body'], $aValsAdd['OwnerID'],$aPlus );
                                }else{
                                    $rEmailTemplate = new BxDolEmailTemplates();
                                    $aTemplate = $rEmailTemplate -> getTemplate( 'Blog_add_notification' ) ;
                                    $aPlus = array ('PostTitle' => bx_get('PostCaption'),'clickhere' =>$aValsAdd['PostUri']);
                                    $aAgencyEmail=db_arr("SELECT Profiles.* FROM Profiles JOIN bx_groups_main WHERE  Profiles.AdoptionAgency=bx_groups_main.id AND Profiles.ID  IN (SELECT bx_groups_main.author_id FROM Profiles JOIN bx_groups_main WHERE Profiles.ID =".$aValsAdd['OwnerID']." AND Profiles.AdoptionAgency=bx_groups_main.id )");
                                    If (!(mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE  author_id=".$aValsAdd['OwnerID'])))){}
                                    sendMail( $aAgencyEmail['Email'], $aTemplate['Subject'], $aTemplate['Body'], $aValsAdd['OwnerID'],$aPlus );
