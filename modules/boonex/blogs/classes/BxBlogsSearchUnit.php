...
class BxBlogsSearchUnit extends BxTemplSearchResultText
{
    ...
    function displaySearchUnit($aResSQL)
    {
      ...
      //  $sAuthorLink = getProfileLink($aProfileInfo['ID']);     
          ...
          $sAuthorLink = $site['url'].'extra_profile_view_17.php?id='.$aProfileInfo['ID']; 
           }
          else {
          $sAuthorLink = $site['url'].'extra_agency_view_29.php?id='.$aProfileInfo['ID'];  
          }                           
...
EOF;
		...
		$RpxPublishHref='<a href="Javascript:void(0);" onClick="return PrepareRpxQueryBlog('."'".$sPostCaption."'".','."'".$sPostLink."'".');">Share</a>';
        $aUnitReplace = array(
            'checkbox' => $sAdminCheck,
            'post_caption' => $sPostCaptionHref,
'rpx_publish_link'  => $RpxPublishHref,
       ...
       if($aResSQL['ownerId']!=$_COOKIE['memberID'])
       {	
         $aUnitReplace['rpx_publish_link']='';
          }

/////////////////////////////////
  defineMembershipActions( array('RPX Janrain') );
		$aCheck = checkAction((int)$_COOKIE['memberID'], BX_RPX_JANRAIN, $isPerformAction);
