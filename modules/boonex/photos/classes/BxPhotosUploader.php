	...
	function initPhotoFile($iPhotoID, $sTitle, $sCategories, $sTags, $sDesc) {
		if(@mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE id=".$iEntryId." AND author_id=".$_COOKIE['memberID']))){
                              $mFlag=false;
                               ...
                               if(@mysql_num_rows(mysql_query("SELECT ID FROM  Profiles JOIN bx_groups_moderation WHERE Profiles.ID= ".(int)$_COOKIE['memberID']." AND Profiles.AdoptionAgency = bx_groups_moderation.GroupId AND bx_groups_moderation.ApproveStatus= 'on' AND bx_groups_moderation.Type = 'photo'"))){
                                $mFlag=true;
                            }else{
                                $mFlag=false;
                            }
                          }
		$sStatus = (getParam('bx_photos_activation') == 'on' || $mFlag )? 'approved' : 'pending';
...
/*Notification Start*////////////////////////////////////////////////////
                        $albumCap=db_arr("SELECT Caption FROM sys_albums WHERE Uri = '".$sAlbum."'");
                                if($sStatus=="pending")
                                {
