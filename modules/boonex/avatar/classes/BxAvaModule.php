...
class BxAvaModule extends BxDolModule
{
    ...
    function _setAvatar ($iProfileId, $sImg, $isSetAvatarForCouple = false)
    {
        ...
        $aDataEntry = $this->_oDb->getAvatarByIdAndOwner ($iAvatar, $iProfileId, $this->isAdmin());
		   if(!is_Agencyadmin($_COOKIE['memberID'])){
        if (!$aDataEntry) {
            echo _t('_bx_ava_msg_access_denied');
            exit;
        }
	   }
		 $fetch_id=explode('/', $_GET['r']);
         $iProfileId = $iProfileIdOrig =$fetch_id[2];
    ...
    function _addAvatar ($sFullPath, $isMove = false, $isUpdateProfile = true)
    {
        ...
        if (!$isOk) {
            ...
            if ($this->_oDb->updateProfile ($iAvatar, $this->_iProfileId, $this->isAdmin())) {
               $AgencyId=getProfileInfo($this->_iProfileId);
                $avtapproval=db_arr("SELECT ApproveStatus FROM bx_groups_moderation WHERE Type='Photo' AND GroupId=".$AgencyId['AdoptionAgency']);

                if(!empty($avtapproval)) {

                if($avtapproval['ApproveStatus']=='on')
                $avtFlag=false;
                else
                $avtFlag=true;
                $this->onEventChanged ($this->_iProfileId, $iAvatar,0,$avtFlag);
    ...
    function _uploadImage ($iProfileId = 0)
    {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $sImagePath)) {
		 ...
		 copy($sImagePath, BX_DIRECTORY_PATH_MODULES.'boonex/avatar/data/avatarphotos/'.($iProfileId ? $iProfileId : $this->_iProfileId) . '.jpg');
			
           if($_FILES['image']['type'] ==  'image/jpeg') {             
                require_once ('resize.php');
                $work = new ImgResizer(BX_DIRECTORY_PATH_MODULES.'boonex/avatar/data/avatarphotos/'.($iProfileId ? $iProfileId : $this->_iProfileId) . '.jpg'); // me.jpg (800x600) is in directory �img� in the same path as this php script.
                $work -> resize(400, BX_DIRECTORY_PATH_MODULES.'boonex/avatar/data/avatarphotos/'.($iProfileId ? $iProfileId : $this->_iProfileId) . '.jpg'); // the old me.jpg (800x600) is now replaced and overwritten with a smaller me.jpg (400x300).
              }


            chmod(BX_DIRECTORY_PATH_MODULES.'boonex/avatar/data/avatarphotos/'.($iProfileId ? $iProfileId : $this->_iProfileId) . '.jpg', 0777);
            return IMAGE_ERROR_SUCCESS == imageResize($sImagePath, '', BX_AVA_PRE_RESIZE_W, BX_AVA_PRE_RESIZE_H, true) ? true : false;
        }
        return false;
    }
