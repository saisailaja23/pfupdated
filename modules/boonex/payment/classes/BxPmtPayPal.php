...
class BxPmtPayPal extends BxPmtProvider
{
    ...
    function initializeCheckout($iPendingId, $aCartInfo, $bRecurring = false, $iRecurringDays = 0)
    {
        ...
        switch($this->getOption('prc_type')) {
         ...
         $this->logData('Posting Data to Paypal Payment', $aFormData,$sActionURL);
        Redirect($sActionURL, $aFormData, 'post', $this->_sCaption);
        exit();
    }
    function finalizeCheckout(&$aData)
    {	  $this->logData('Posting Data to Paypal Payment', $aData,'');
        ...
        {
            global $dir;
            $qb_log_file = $dir['root']."log/paymentlogPaypalNet_".date('Y-m-d').".txt";
            ob_start();
            echo "\n  ============================= Payment Log =================\n ";
            echo " Action :".$Action."  \n";
    ...
    function _validateCheckout(&$aData) {
        //Below two lines added by Joseph to skipp the payment process
        if($aData['bypass']=='true')
