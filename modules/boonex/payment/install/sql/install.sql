...
CREATE TABLE IF NOT EXISTS `[db_prefix]transactions_pending` (
  ...
  `client_id` int(11) NOT NULL default '0',
  `seller_id` int(11) NOT NULL default '0',
  `items` text character set utf8 collate utf8_unicode_ci NOT NULL default '',
  `amount` float NOT NULL default '0',
...
('_payment_tbar_item_caption', 'bx_pmt_cart', 'modules/boonex/payment/|tbar_item_cart.png', 'modules/?r=payment/cart/', '', '', 'return BxDolService::call(''payment'', ''get_cart_items'');', 0, '1', 0, 0, '', 'top_extra', 'link', 0, '$aRetEval = BxDolService::call(''payment'', ''get_cart_item_count'', array({ID}, {iOldCount}));', '_payment_tbar_item_description');
