...
class BxGroupsTemplate extends BxDolTwigTemplate
{
    ...
    function unit ($aData, $sTemplateName, &$oVotingView, $isShort = false)
    {
        ...
        $aVars = array (
           ...
           // 'group_url' => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'view/' . $aData['uri'],
            'group_url' => BX_DOL_URL_ROOT . 'extra_agency_view_29.php?ID=' . $aData['author_id'],
    ...
    {
       $query = "SELECT `Email` FROM `Profiles` LEFT JOIN `bx_groups_main` ON `Profiles`.`id` = `bx_groups_main`.`author_id` WHERE `author_id` = ".$aDataEntry['author_id'];
        $emailadress =  $GLOBALS['MySQL']->getOne($query);
        $sRet = '<table class="bx_groups_fields">
                <tr>
                <td class="bx_groups_field_name">Email Id </td>
                <td></td>
                <td class="bx_groups_field_value">'. $emailadress .'</td>
                </tr>';
