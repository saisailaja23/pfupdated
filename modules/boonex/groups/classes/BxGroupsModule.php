...
class BxGroupsModule extends BxDolTwigModule
{
    ...
    function actionBrowseFans ($sUri)
    {
	 ...
	 function actionBrowseFansList ($sUri) {
        
         if(@mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE uri='".$sUri."' AND author_id=".$_COOKIE['memberID'])) || $GLOBALS['logged']['admin']) // Check if Viewer is an agency admin
             parent::_actionBrowseFansList ($sUri, 'isAllowedViewFans', 'getFansBrowse', $this->_oDb->getParam('bx_groups_perpage_browse_fans'), 'browse_fans_list/', _t('_bx_groups_page_title_fans'));
         else
               parent::_actionBrowseFans ($sUri, 'isAllowedViewFans', 'getFansBrowse', $this->_oDb->getParam('bx_groups_perpage_browse_fans'), 'browse_fans/', _t('_bx_groups_page_title_fans'));
    }

   function actionBrowseTemplateList ($sUri) {

    if(@mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE uri='".$sUri."' AND author_id=".$_COOKIE['memberID'])) || $GLOBALS['logged']['admin']) // Check if Viewer is an agency admin
     parent::_actionBrowseTemplateList($sUri, 'isAllowedViewFans', 'getFansBrowse', $this->_oDb->getParam('bx_groups_perpage_browse_fans'), 'browse_template_list/', _t('List of templates'));
       //  else
             //  parent::_actionBrowseFans ($sUri, 'isAllowedViewFans', 'getFansBrowse', $this->_oDb->getParam('bx_groups_perpage_browse_fans'), 'browse_fans/', _t('_bx_groups_page_title_fans'));
    }
    ...
    function actionAdministrationSettings ()
    {
	    ...
	    function actionManageCodes () {
    	if (!$this->isAdmin()) {
            $this->_oTemplate->displayAccessDenied ();
            return;
        }
    	$sContent = '';
        $aEntries = $this->_oDb->getCodesList();
        if ($aEntries) {
    		$aData = array();
    		$sUrl = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri();
    		foreach ($aEntries as $aEntry) {
    			$aEntry['bx_if:code_link'] = array(
    				'condition' => mb_strlen($aEntry['code']) == 0,
    				'content' => array(
    					'url' => $sUrl,
    					'id' => $aEntry['id']
    				)
    			);
    			$aData['bx_repeat:entries'][] = $aEntry;
    		}
        	$sContent = $this->_oTemplate->parseHtmlByName('codes_list', $aData);
        }
        else
    ...
    function actionGenCode ($iEntryId) {
    	if (!$this->isAdmin()) {
            $this->_oTemplate->displayAccessDenied ();
            return;
        }
        $this->_oTemplate->pageStart();
        if ($this->_oDb->generateEntryCode($iEntryId))
        	$sMessage = '_Saved';
    	else
    		$sMessage = '_Error';
    	echo $this->_oTemplate->adminBlock (MsgBox(_t($sMessage)), _t('_bx_groups_page_title_administration'), $aMenu);
        $this->_oTemplate->addCssAdmin ('admin.css');
        $this->_oTemplate->addCssAdmin ('unit.css');
        $this->_oTemplate->addCssAdmin ('main.css');
        $this->_oTemplate->addCssAdmin ('forms_extra.css'); 
    ...
    function serviceCheckCode ($sCode, $iEntryId) {
    	return $this->_oDb->checkCode($sCode, $iEntryId);
    }
    
    function serviceCheckControl ($iMaster, $iDepend) {
    	return $this->_oDb->checkControl($iMaster, $iDepend);
    }
    
    function serviceActivateWithAdmin ($iEntryId, $iProfileId) {
    	$iEntryId = (int)$iEntryId;
    	$iProfileId = (int)$iProfileId;
    	//if ($this->_oDb->changeGroupOwner($iEntryId, $iProfileId)) {
    	//	return $this->_oDb->activateEntry($iEntryId);
    	//}
    	return false;
    }
    
    function serviceAddFan ($iEntryId, $iProfileId, $bConfirm = true) {
    	$iEntryId = (int)$iEntryId;
    	$iProfileId = (int)$iProfileId;
    	if ($this->_oDb->joinEntry($iEntryId, $iProfileId, $bConfirm) > 0)
    		return true;
		return false;
    }
    
    function actionTest ($iMaster, $iDepend) {
    	$bRes = $this->_oDb->checkControl($iMaster, $iDepend);
    	var_dump($bRes);
    }
    ...
    // ================================== events
        ...
        $this->_oDb->generateEntryCode($iEntryId);
        $this->_oDb->addEntryToPre($aDataEntry);
		$this->_oDb->cleanCache('sys_prevalues_keys');
		addStringToLanguage('__' . $aDataEntry['title'], $aDataEntry['title']);
	...
	function onEventDeleted ($iEntryId, $aDataEntry = array()) {
		...
		$this->_oDb->removedEntryToPre($iEntryId);
		$this->_oDb->cleanCache('sys_prevalues_keys');
		
		deleteStringFromLanguage('__' . $aDataEntry['title']);
    ...
    {
		  if($aDataEntry['id'] && $this->_iProfileId)
        if(!@mysql_num_rows(mysql_query("SELECT * FROM bx_groups_fans WHERE id_entry=".$aDataEntry['id']." AND id_profile=".$this->_iProfileId)))
            return false;
        if (!$this->_iProfileId)
            return false;
    ...
    function _browseMy (&$aProfile)
    {
	 ...
	 function isAllowedAgencySettings($aDataEntry) {
        return $this->isEntryAdmin($aDataEntry);
    }
    function actionApprovalSettings ($sUri) {
        parent::_actionApprovalSettings ($sUri, 'isAllowedAgencySettings', 'approvalsettings', array ('approvalsettings_photo', 'approvalsettings_video', 'approvalsettings_journal', 'approvalsettings_letter'), _t('_bx_groups_page_title_approval_setting'));
    }
