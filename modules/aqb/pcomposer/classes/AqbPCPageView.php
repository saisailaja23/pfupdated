...
class AqbPCPageView extends BxDolPageView {
	...
	function genBlock( $iBlockID, $aBlock, $bStatic = true, $sDynamicType = 'tab' ) {
		 if(!is_numeric($iBlockID)) // AQB block ID are in string format
            {
                $_PCblockId=explode('_', $iBlockID);
                $_pcApprovedFlag=db_arr("SELECT approved FROM aqb_pc_members_blocks WHERE id={$_PCblockId[2]}"); // fetch approved flag fro DB
                if(!$_pcApprovedFlag['approved'])
                    return; // skip generating block if not approved
            }
