   ...
   $aBlocks = $this -> getProfileBlocksAsArray($iPId);
	   ...
	   $aB = array();
		  ...
		  foreach($v as $key => $value)
			{  
			  ...
			  if ($sType == 'shared' &&  $k == 'c') $aB[] = $key;
                          if ( $k == 'c') $aAQB[] = $key;//code for AQB by Joseph
			}  
		...
		if ($sWhere) $sWhere = "AND `c`.`ID` NOT IN {$sWhere}";
		        ...
		        //Coded for AQB
                if (count($aAQB)){
			$sWhereAQB = implode("','", $aAQB);
			$sWhereaAQB = "('".$sWhereAQB."')";
		}

		if ($sWhereaAQB) $sWhereaAQB = "AND `c`.`ID` NOT IN {$sWhereaAQB}";

                //Coded fro AQB by Joseph
		...
		if ($sType == 'standard')
		{
                    /* echo $sql = "SELECT COUNT(*)
										...
										WHERE `c`.`Page` = 'profile' AND `c`.`Column` != 0 {$sWhere}";exit(); */
		  ...
		  else 
		{
	     ...
	     if($sType == 'standard')// this is for taking the AQB block for the users
		{ 
			$iCount = $iCount + $this -> getOne("SELECT COUNT(*)
									FROM `{$this->_sPrefix}members_blocks` as `c`
									WHERE `c`.`approved` = '1' AND `c`.`owner_id` = '{$iPId}' {$sWhereaAQB}");
			$iCount = !(int)$iCount ? 1 : $iCount;
			...
			$aRes1 = $this -> getAll("SELECT `c`.`id` as `ID`
									 FROM `{$this->_sPrefix}members_blocks` as `c`
									 WHERE `c`.`approved` = '1' AND `c`.`owner_id` = '{$iPId}'  {$sWhereaAQB} ORDER BY `c`.`id` ");
		...
		{	
			$aResult[] = array(
                ...
                if($sType == 'standard')// this is for taking the AQB block for the users
                foreach($aRes1 as $k => $v) // for the AQB block part
		{
			$aResult[] = array(
					'id' => ($sPrefix.$v['ID']) ,
		                        'params' =>  $this -> getCustomBlockInfo($v['ID'], array())
					);
		}
	...
	//	return $this -> query("DELETE FROM `{$this->_sPrefix}profiles_info` WHERE `member_id` <> '0'");
		 $blockstring = db_arr("SELECT `page` FROM `{$this->_sPrefix}profiles_info` WHERE `member_id` = '0' ");
            $defaltblocks = explode("'c' =>",$blockstring['page']);
                       
            $members = mysql_query("SELECT `member_id` FROM `{$this->_sPrefix}profiles_info`");
                     while($row = mysql_fetch_array($members))
                     {
                         $profileblocks = db_arr("SELECT `page` FROM `{$this->_sPrefix}profiles_info` WHERE `member_id` = ".$row['member_id']);
                         $s = explode("'c' =>",$profileblocks['page']);       
                         $s[0] = $defaltblocks[0];             
                         $finalblocks = $s[0]."'c' =>".$s[1];            
                         $sBlocks = str_replace("'", "\'", $finalblocks);
                         mysql_query("UPDATE `{$this->_sPrefix}profiles_info` SET `page`= '".$sBlocks."' WHERE `member_id` = ".$row['member_id']);                
                     }  
	}
	...
	function getColumnBlocks($iCol, $iProfile, $iMGroup = AQB_PC_GUEST){
		...
		foreach($aCol as $key => $value){
		    if ($value['type'] == 's') {
				$aArray = $this -> getStandardBlockInfo($value['id'], $value);
				if ($aArray === false) continue; 
				$aResult[$value['id']] = $aArray;
			}	
			else{
				$aArray = $this -> getCustomBlockInfo($value['id'], $value);
				if ($aArray === false || !(int)$aArray['approved']) continue; 
				$aResult[$sPrefix.$value['id']] = $aArray;
			}		
		}		
	...
	function createCustomBlock($iPId, &$aValues, $iUBlockId = 0){
		if (call_user_func(array($this -> _oConfig, 'is' . strtoupper($aValues['type']) . 'BlockAllowed'))) 
		{
			...
			if (call_user_func(array($this -> _oConfig, 'isAutoApprove' . strtoupper($aValues['type']) . 'Block'))) $sAutoApprove = ",`approved` = '1'"; else $sAutoApprove = ",`approved` = '0'";
			  if($mFlag)	
                               $sAutoApprove = ",`approved` = '1'"; 
			else
				$sAutoApprove = ",`approved` = '0'"; 
	...
	function withPointsIntegration(){
		...
		if ($iId) {	
			...
			$this -> query("REPLACE INTO `aqb_points_actions`
															 SET `title` = '_aqb_points_action_aqb_pcomposer_edit',
															`handler` = 'edit',
															`alerts_unit` = 'aqb_pcomposer',
															`points` = '1',
															`day_limit` = '100',
															`active` = 'true',
															`module_uri` = 'aqb_pcomposer',
															`param` = 'first',
															`group` = 'aqb_pcomposer'");
			
			$this -> query("REPLACE INTO `aqb_points_actions`
															 SET `title` = '_aqb_points_action_aqb_pcomposer_add',
															`handler` = 'add',
															`alerts_unit` = 'aqb_pcomposer',
															`points` = '1',
															`day_limit` = '100',
			...
			$this -> query("REPLACE INTO `aqb_points_actions`
															 SET `title` = '_aqb_points_action_aqb_pcomposer_remove',
															`handler` = 'remove',
															`alerts_unit` = 'aqb_pcomposer',
															`points` = '-1',
															`day_limit` = '100',
			...
			$this -> query("REPLACE INTO `aqb_points_actions`
															 SET `title` = '_aqb_points_action_aqb_pcomposer_approved',
															`handler` = 'approved',
															`alerts_unit` = 'aqb_pcomposer',
															`points` = '1',
															`day_limit` = '100',
			...
			$this -> query("REPLACE INTO `aqb_points_actions`
															 SET `title` = '_aqb_points_action_aqb_pcomposer_disapproved',
															`handler` = 'disapproved',
															`alerts_unit` = 'aqb_pcomposer',
															`points` = '-1',
															`day_limit` = '100',
