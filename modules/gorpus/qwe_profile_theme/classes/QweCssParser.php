...
class QweCssParser {
  ...
  function QweCssParser($html = true) {
  ...
  function Add($key, $codestr) {
    ...
    if(!isset($this->css[trim($key)])) {
      $this->css[trim($key)] = array();
    }
    ...
    if(count($codes) > 0) {
      foreach($codes as $code) {
		...
		$sep_pos = strpos($code, ":");
		$codekey = substr($code, 0, $sep_pos);
		$codevalue = substr($code, $sep_pos + 1);

        if(strlen($codekey) > 0) {
          $this->css[trim($key)][trim($codekey)] = trim($codevalue);
        }
      }
    }
  }

	function GetRaw($key, $property)
	{
  ...
  function GetSection($key) {
	...
	function GetSectionCss($key)
	{
		$section = $this->GetSection($key);
		if($section && is_array($section)) {
			$result = $key . $this->GetSectionStyle($key);
			return $result;
		}
		else {
			return '';
	...
	function GetSectionStyle($key, $exclude_props = FALSE)
	{
		$section = $this->GetSection($key);
		...
		if($section && is_array($section)) {
			...
			if($exclude_props && is_array($exclude_props)) {
				foreach($section as $key => $value) {
					$use_prop = TRUE;

					foreach($exclude_props as $prop) {
