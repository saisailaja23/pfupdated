...
class TinyMCE_Compressor {
	...
	public function handleRequest() {
		...
		$plugins = preg_split("/[, ]/", $this->settings["plugins"]);

		$themes = self::getParam("theme");
		if ($themes)
			$this->settings["theme"] = $themes;
		$themes = explode(',', $this->settings["theme"]);
...
//				$buffer .= "\n//-FILE-$tinymceDir/$file (". strlen($fileContents) . " bytes)\n";
		...
		$buffer .= 'tinymce.each("' . implode(',', $allFiles) . '".split(","),function(f){tinymce.ScriptLoader.markDone(tinyMCE.baseURL+"/"+f);});';
	...
	public static function renderTag($tagSettings, $return = false) {
		...
		if (isset($settings["theme"]))
			$scriptSrc .= "&theme=" . (is_array($settings["theme"]) ? implode(',', $settings["theme"]) : $settings["theme"]);
