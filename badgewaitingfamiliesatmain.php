...
echo $aData;
    ...
    $sql = "SELECT distinct * FROM `Profiles` WHERE (`DescriptionMe` LIKE '%$search%' or `FirstName` LIKE '%$search%' or`LastName` LIKE '%$search%' or`DearBirthParent` LIKE '%$search%' or `About_our_home` LIKE '%$search%') AND `AdoptionAgency` = '47' AND `ProfileType` != '8' AND `Status` = 'Active' Limit 24";
    }
      ...
      /* while($row = mysql_fetch_array($result))
       {
        ...
        if (file_exists($filename)) {
          $aData1 = "http://www.parentfinder.com/modules/boonex/avatar/data/images/".$row['Avatar'].".".'jpg';
