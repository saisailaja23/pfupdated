...
$data = file_get_contents($site['url'].'test.php?profile='.$_GET['profile']);
 $aData = (explode(";-",$data));
$datap = file_get_contents($site['url'].'test.php?profile='.$_GET['profile']);
...
$privatekey = "6LcNhvASAAAAAGEbf-qrwRtWivwArCk8k6p_u-IW";
...
$msg_captcha = "<font color='red'><b>Captcha error. Please, type the check-words again.</b></font>";
}
...
$datas = file_get_contents($site['url'].'achose1.php?profile='.$_GET['profile']);
...
$to = "parentfinder@adoptionstogether.org, sogden@adoptionstogether.org, lteeter@adoptionstogether.org, jsullivan@adoptionstogether.org, jtaylor@adoptionstogether.org, bstahl@adoptionstogether.org, jsimpson@adoptionstogether.org";
//$to = "prashanth.adkathbail@mediaus.com";
$subject = "Mail for Adoptions together  profile - ".$fullName;
$txt = <<<EOF
...
EOF;
$headers = "From: info@parentfinder.com" . "\r\n" .
"CC: ahurd@adoptionstogether.org";


//$result = mail($to,$subject,$txt,$headers);

//if ($result && $resp->is_valid)
//if (($resp->is_valid) && (isset($result))) 
if ($resp->is_valid) 
{     
...
<head id="Head"><meta content="text/html; charset=UTF-8" http-equiv="Content-Type" />
    ...
    <title>Contact - <?=$aData1[3]?><?php if($aData1[4] != '') { ?> & <?php } ?><?=$aData1[4]?></title>
...
if(isset($_GET['pstart']) && $_GET['pstart'] != 0){
	...
	$spage = $_GET['page'] - 1;
	echo '<a href="contact.php?pstart='.$lpstart.'&pend='.$lpend.'&page='.$spage.'&profile='.$_GET['profile'].'" class="more">Prev</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
if(isset($_GET['search'])){
		...
		if($aData[$p] != ''){
			...
			if($_GET['page'] == $i){
				...
				echo '<div style=margin-left:30px;margin-top:10px;><b>'.$i.'</b></div> &nbsp;';
					...
					echo '<a href="contact.php?pstart='.$pstart.'&pend=12&page='.$i.'&profile='.$_GET['profile'].'">'.$i.'</a> &nbsp;';
			}
			$pstart = $pend + 0;
			$pend = $pend + 12;
		}
	...
	if($currentpage != $i){
		...
		echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="contact.php?pstart='.$lpstart.'&pend='.$lpend.'&page='.$page.'&profile='.$_GET['profile'].'" class="next">Next</a>';
...
if(isset($_GET['pstart']) && $_GET['pstart'] != 0){
  ...
  echo '<a href="contact.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$lpstart.'&pend='.$lpend.'&page='.$spage.'&button=Submit+Search" class="more">Prev</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			...
			if($_GET['page'] == $i){
				...
				echo '<b>'.$i.'</b> &nbsp;';
					...
					//echo '<a href="index.php?max_race_id=7&beth[]='.$bethz.'&pareth='.$parethz.'&famreg='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&page=family_profile%2Fbrowse&button=Submit+Search">'.$i.'</a> &nbsp;'; pstart='.$pstart.'&pend=12&page='.$i.'"
                                   echo '<a href="contact.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$pstart.'&pend=12&page='.$i.'&button=Submit+Search">'.$i.'</a> &nbsp;';
	...
	if($currentpage != $i){
		if(isset($_GET['pend'])){
			...
			$page = 2;
               ...
               if($i !=''){
                echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="contact.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$lpstart.'&pend='.$lpend.'&page='.$page.'&button=Submit+Search" class="next">Next</a>';
                   }
...
<div class="left_body_box">
   ...
   <div style="text-align: center;margin-top:10px;"><a href="mainProfile.php?profile=<?=$_GET['profile']?>" class="black"><?php echo $getAboutUsArray[0]; ?> <?php if($getAboutUsArray[1] != ''){ ?> &amp; <?php echo $getAboutUsArray[1] ?> <?php } ?></a></div>
           ...
           <div style="margin-top:10px;margin-left:15px;"><a href="contact.php?profile=<?=$_GET['profile']?>&v=watch" >  <img class="mainLogo" alt="logo" src="images/contactus_btn.png"></a></div>
...
<div style="margin-top:10px;margin-bottom:10px">
   If you'd like to learn more about us, please complete the following form.  Our Family Specialist at Adoptions Together will be in touch with you shortly.
</div>
...
<?php
					...
					$publickey = "6LcNhvASAAAAAEcomgJiUsRuMGci7hA7KutjXZKK";
