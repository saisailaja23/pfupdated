...
<?php
 ...
 //$data = file_get_contents('http://ctstagepf.parentfinder.com/test.php?profile='.$_GET[profile].'&v=watch');
 //$aData = (explode(";-",$data));
//print_r($aData);
 $data = file_get_contents($site['url'].'test.php?profile='.$_GET['profile']);
 $aData = (explode(";-",$data));
$data = file_get_contents($site['url'].'getphotosvideos.php?profile='.$_GET[profile].'&v=watch');
$videos = (explode(";-",$data));
array_pop($videos);
$profile = $_GET['profile'];
?>
...
<head id="Head"><meta content="text/html; charset=UTF-8" http-equiv="Content-Type" />
    ...
    <title>Video - <?=$aData1[3]?><?php if($aData1[4] != '') { ?> & <?php } ?><?=$aData1[4]?></title>
     		...
     		<div class="oneside1">
                ...
                <div style="text-align: center;margin-top:10px;"><a href="mainProfile.php?profile=<?=$profile?>" class="black"><?=$familyData[1]?> <? if($familyData[2] != ''){ ?> &amp; <?=$familyData[2]?> <? }?></a></div>
                	...
                	<?php }
                     ...
                     <div class="oneside1">
			<div class="oneside">
                            <img src="images/noimage.jpg" width="64" height="64" height="64 "border="0"/></div>
                            <div style="text-align: center;margin-top:10px;"><a href="mainProfile.php?profile=<?=$profile?>" class="black"><?=$familyData[1]?> <? if($familyData[2] != ''){ ?> &amp; <?=$familyData[2]?> <? }?></a></div>
...
if(isset($_GET['pstart']) && $_GET['pstart'] != 0){
	...
	$spage = $_GET['page'] - 1;
	echo '<a href="video.php?pstart='.$lpstart.'&pend='.$lpend.'&page='.$spage.'&profile='.$_GET['profile'].'" class="more">Prev</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
if(isset($_GET['search'])){
		...
		if($aData[$p] != ''){
			...
			if($_GET['page'] == $i){
				...
				echo '<div style=margin-left:30px;margin-top:10px;><b>'.$i.'</b></div> &nbsp;';
					...
					echo '<a href="video.php?pstart='.$pstart.'&pend=12&page='.$i.'&profile='.$_GET['profile'].'">'.$i.'</a> &nbsp;';
			}
			$pstart = $pend + 0;
			$pend = $pend + 12;
		}
	...
	if($currentpage != $i){
		...
		echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="video.php?pstart='.$lpstart.'&pend='.$lpend.'&page='.$page.'&profile='.$_GET['profile'].'" class="next">Next</a>';
...
if(isset($_GET['pstart']) && $_GET['pstart'] != 0){
  ...
  echo '<a href="video.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$lpstart.'&pend='.$lpend.'&page='.$spage.'&button=Submit+Search" class="more">Prev</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			...
			if($_GET['page'] == $i){
				...
				echo '<b>'.$i.'</b> &nbsp;';
					...
					//echo '<a href="index.php?max_race_id=7&beth[]='.$bethz.'&pareth='.$parethz.'&famreg='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&page=family_profile%2Fbrowse&button=Submit+Search">'.$i.'</a> &nbsp;'; pstart='.$pstart.'&pend=12&page='.$i.'"
                                   echo '<a href="video.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$pstart.'&pend=12&page='.$i.'&button=Submit+Search">'.$i.'</a> &nbsp;';
	...
	if($currentpage != $i){
		if(isset($_GET['pend'])){
			...
			$page = 2;
               ...
               if($i !=''){
                echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="video.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$lpstart.'&pend='.$lpend.'&page='.$page.'&button=Submit+Search" class="next">Next</a>';
                   }
