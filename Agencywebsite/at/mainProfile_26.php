...
$getaboutus = file_get_contents('http://ctstagepf.parentfinder.com/getaboutus_exe.php?profile='.$_GET['profile']);
$getAboutUsArray = (explode(";-",$getaboutus));
array_pop($getAboutUsArray);
//print_r($getAboutUsArray);

$aDataBlock = file_get_contents('http://ctstagepf.parentfinder.com/getblockshoa_exe.php?profile='.$_GET['profile'].'');
$aDataBlocks = (explode(";-",$aDataBlock));
//print_r($aDataBlocks);
array_pop($aDataBlocks);

$achildata = file_get_contents('http://ctstagepf.parentfinder.com/getchild.php?profile='.$_GET['profile'].'');
$achildatas = (explode(";-",$achildata));
array_pop($achildatas);

$data = file_get_contents('http://ctstagepf.parentfinder.com/test.php?profile='.$_GET['profile']);
$aData1 = (explode(";-",$data));
//print_r($aData);
...
if(!isset($_GET['max_race_id'])){
    ...
    $data = file_get_contents('http://ctstagepf.parentfinder.com/badgewaitingfamiliesat.php?pstart='.$_GET[pstart].'&pend='.$_GET['pend']);
   }

else {

  $data = file_get_contents('http://ctstagepf.parentfinder.com/badgewaitingfamiliesat.php?famreg='.$famregz.'&pareth='.$parethz.'&beth='.$bethz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$_GET['pstart'].'&pend='.$_GET['pend']);

   }
}
elseif(isset($_GET['search']) && !isset($_GET['max_race_id'])){
$data = file_get_contents('http://ctstagepf.parentfinder.com/badgewaitingfamiliesat.php?search='.$_GET[search]);

}
elseif(isset($_GET['max_race_id'])){

$data = file_get_contents('http://ctstagepf.parentfinder.com/badgewaitingfamiliesat.php?famreg='.$famregz.'&pareth='.$parethz.'&beth='.$bethz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state]);

}
else {

    $data = file_get_contents('http://ctstagepf.parentfinder.com//badgewaitingfamiliesat.php');
...
<a class="SkinObject" href="http://www.adoptionstogether.org/Pregnant.aspx">Pregnant?</a>
&gt;
<a class="SkinObject" href="http://www.adoptionstogether.org/Pregnant/FamiliesWaitingtoAdopt.aspx">Families Waiting to Adopt</a>
</span></div>
...
if(isset($_GET['pstart']) && $_GET['pstart'] != 0){
	...
	$spage = $_GET['page'] - 1;
	echo '<a href="mainProfile.php?pstart='.$lpstart.'&pend='.$lpend.'&page='.$spage.'" class="more">Prev</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
if(isset($_GET['search'])){
		...
		if($aData[$p] != ''){
			...
			if($_GET['page'] == $i){
				...
				echo '<div style=margin-left:30px;margin-top:10px;><b>'.$i.'</b></div> &nbsp;';
					...
					echo '<a href="mainProfile.php?pstart='.$pstart.'&pend=12&page='.$i.'">'.$i.'</a> &nbsp;';
			}
			$pstart = $pend + 0;
			$pend = $pend + 12;
		}
	...
	if($currentpage != $i){
		...
		echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="mainProfile.php?pstart='.$lpstart.'&pend='.$lpend.'&page='.$page.'" class="next">Next</a>';
...
$data = file_get_contents('http://ctstagepf.parentfinder.com/badgewaitingfamiliesat.php?getpa=true&max_race_id=7&famreg='.$famregz.'&pareth='.$parethz.'&beth='.$bethz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state]);
...
<?php } ?>           </div>
            ...
            <div class="art-content">
                <div style="margin-left:10px;margin-top:15px;"><a href="http://ctstagepf.parentfinder.com/">
<img class="mainLogo" alt="logo" src="images/logo.png"></div>
</a><div>&nbsp;</div><div>&nbsp;</div>
<div class="left_body_box">

    <div class="one1">
   <div class="one" ><img src="<?php if ($aData1[9] == '') echo 'images/noimage.jpg'; else echo $aData1[9];?>" width="100" height="75" title="Adoptive Family <?=$aData1[3]?><?php if($aData1[4] != '') { ?> & <?php } ?><?=$aData1[4]?>" width="100" height="75" alt="<?=$aData1[3]?><?php if($aData1[4] != '') { ?> & <?php } ?><?=$aData1[4]?>"><!--<img src="<?=$aData1[9]?>" title="Adoptive Family Dave &amp; Krissy" width="300" height="422">--></div>
   <div style="text-align: center;margin-top:10px;"><a href="mainProfile.php?profile=<?=$profile?>" class="black"><?php echo $getAboutUsArray[0]; ?> <?php if($getAboutUsArray[1] != ''){ ?> &amp; <?php echo $getAboutUsArray[1] ?> <?php } ?></a></div>
   ...
   <li class="internalli"><a href="video.php?profile=<?=$_GET['profile']?>&v=watch" >  <img class="mainLogo" alt="logo" src="images/video_btn.png"></a></li>
