...
if(!isset($_GET['max_race_id'])){
    ...
    $data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php?pstart='.$_GET[pstart].'&pend='.$_GET['pend']);
   }

else {

  $data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php?famreg='.$famregz.'&pareth='.$parethz.'&beth='.$bethz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$_GET['pstart'].'&pend='.$_GET['pend']);

   }
}
elseif(isset($_GET['search']) && !isset($_GET['max_race_id'])){
$data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php?search='.$_GET[search]);

}
elseif(isset($_GET['max_race_id'])){

$data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php?famreg='.$famregz.'&pareth='.$parethz.'&beth='.$bethz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state]);

}
else {

    $data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php');
...
<img id="dnn_ctr892_ViewImage_imgImage" src="Portals/0/images/YoungInterracialCouple2.jpg" style="border-width:0px;" />
	...
	<ul class="" id="dnn_dnnNAV_ctldnnNAV">
		<li id="dnn_dnnNAV_ctldnnNAVctr167"><a href="http://www.adoptionstogether.org/Pregnant/MakinganAdoptionPlan.aspx"><span>Making an Adoption Plan</span></a></li><li id="dnn_dnnNAV_ctldnnNAVctr173"><a href="http://www.adoptionstogether.org/Pregnant/RightsandResponsibilitiesoftheBirthFather.aspx"><span>Rights and Responsibilities of the Birth Father</span></a></li><li id="dnn_dnnNAV_ctldnnNAVctr174"><a href="http://www.adoptionstogether.org/Pregnant/WhatifmyFriendorFamilyMemberisPregnant.aspx"><span>What if my Friend or Family Member is Pregnant?</span></a></li><li id="dnn_dnnNAV_ctldnnNAVctr65"><a href="index.php"><span>Families Waiting to Adopt</span></a></li><li id="dnn_dnnNAV_ctldnnNAVctr170"><a href="http://www.adoptionstogether.org/Pregnant/Resources.aspx"><span>Resources</span></a></li><li id="dnn_dnnNAV_ctldnnNAVctr79"><a href="http://www.adoptionstogether.org/Pregnant/SpeaktoaCounselor.aspx"><span>Speak to a Counselor</span></a></li><li id="dnn_dnnNAV_ctldnnNAVctr175"><a href="http://www.adoptionstogether.org/Pregnant/BirthMotherStories.aspx"><span>Birth Mother Stories</span></a></li><li id="dnn_dnnNAV_ctldnnNAVctr273"><a href="http://www.birthparentblog.com/"><span>Birth Parent Blog</span></a></li>
                    ...
                    <div>&nbsp;</div>
<div style="margin-left:30px;"><a href="advsearch.php" ><img class="mainLogo" alt="logo" src="images/advanced_btn.png"></a></div>
                </div>
            ...
            <div class="art-content">
                ...
                </div><div>&nbsp;</div><a href="<?php $site['url']; ?>">
...
?>
			<?php if($familyData[3]!='') { ?>
     		...
     		<div class="onesidemain1">
                    ...
                    <div class="onesidemain"><a href="photo.php?profile=<?=$profile?>" class="photob"><img src="<?=$familyData[3]?>" width="150" height="150" border="0"/></a></div>
		...
		<?php }
			...
			<div class="onesidemain1">
                        ...
                        <div class="onesidemain"><img src="images/noimage.jpg" width="150" height="150" border="0"/></div>
                        <div style="text-align: center;margin-top:10px;"><a href="mainProfile.php?profile=<?=$profile?>" class="black"><?=$familyData[1]?> <? if($familyData[2] != ''){ ?> &amp; <?=$familyData[2]?> <? } ?></a></div>
     		...
     		<div class="onesidemain1">
                ...
                <div style="text-align: center;margin-top:10px;"><a href="mainProfile.php?profile=<?=$profile?>" class="black"><?=$familyData[1]?> <? if($familyData[2] != ''){ ?> &amp; <?=$familyData[2]?> <? }?></a></div>
                	...
                	<?php }
                     ...
                     <div class="onesidemain1">
			<div class="onesidemain">
                            <img src="images/noimage.jpg" width="150" height="150" border="0"/></div>
                            <div style="text-align: center;margin-top:10px;"><a href="mainProfile.php?profile=<?=$profile?>" class="black"><?=$familyData[1]?> <? if($familyData[2] != ''){ ?> &amp; <?=$familyData[2]?> <? }?></a></div>
...
<!-- -->
<?php
echo '<div style="margin-top:1700px;text-align:center;">';
...
if(isset($_GET['search'])){
	$data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php?getpa=true&search='.$_GET[search]);
}else{
	$data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php?getpa=true');
	$aData = (explode("#####",$data));
	//print_r($aData);
	$i=0;
	$p = 0;
		...
		if($aData[$p] != ''){
			...
			if($_GET['page'] == $i){
				...
				echo '<b>'.$i.'</b> &nbsp;';
					...
					echo '<a href="index.php?pstart='.$pstart.'&pend=24&page='.$i.'">'.$i.'</a> &nbsp;';
			}
			$pstart = $pend + 0;
			$pend = $pend + 24;
		}
	...
	if($currentpage != $i){
		...
		echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="index.php?pstart='.$lpstart.'&pend='.$lpend.'&page='.$page.'" class="next1">Next</a>';
...
if(isset($_GET['search'])){
	...
	//$data = file_get_contents('http://ctstagepf.parentfinder.com/badgewaitingfamiliess.php?getpa=true');

$data = file_get_contents($site['url'].'badgewaitingfamiliesatmain.php?getpa=true&max_race_id=7&famreg='.$famregz.'&pareth='.$parethz.'&beth='.$bethz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state]);
		...
		if($aData[$p] != ''){
			...
			echo '<b>'.$i.'</b> &nbsp;';
					...
					//echo '<a href="index.php?max_race_id=7&beth[]='.$bethz.'&pareth='.$parethz.'&famreg='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&page=family_profile%2Fbrowse&button=Submit+Search">'.$i.'</a> &nbsp;'; pstart='.$pstart.'&pend=12&page='.$i.'"
                                   echo '<a href="index.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$pstart.'&pend=24&page='.$i.'&button=Submit+Search">'.$i.'</a> &nbsp;';
	...
	if($currentpage != $i){
		if(isset($_GET['pend'])){
			...
			$page = 2;
               ...
               if($i !=''){
                echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="index.php?max_race_id=7&beth[]='.$bethz.'&pareth[]='.$parethz.'&famreg[]='.$famregz.'&family_num_children='.$_GET[family_num_children].'&family_state='.$_GET[family_state].'&pstart='.$lpstart.'&pend='.$lpend.'&page='.$page.'&button=Submit+Search" class="next1">Next</a>';
                   }
