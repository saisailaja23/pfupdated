...
$searchFilter = ($_REQUEST['searchFilter'])?$_REQUEST['searchFilter']:'';
...
/*if($searchtype == 'Familysize' && $_REQUEST['sortvalue'] == '3 ') {
   $searchvalues == '3+';
    ...
    $AgencyLike_List = "SELECT `Profiles`. `ID`,`Profiles`. `Region`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles` WHERE  `Profiles`.`Status` = 'Active' and ($searchOption and Profiles.noofchildren is not null and Profiles.noofchildren <> '') and `Profiles`.`ProfileType` = 2 and `Profiles`.`Avatar` != '0'  AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter limit $from , $count";  
    if($from == 0){
  ...
  // $AgencyLike_List = "SELECT `Profiles`. `ID`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles` WHERE  `Profiles`.`Status` = 'Active' and `Profiles`.`ProfileType` = 2 and `Profiles`.`Avatar` != '0' AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter ORDER BY `Profiles`.`DateReg` , RAND() DESC limit $from , $count";
      $AgencyLike_List = "SELECT `Profiles`. `ID`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles` WHERE  `Profiles`.`Status` = 'Active' and `Profiles`.`ProfileType` = 2 and `Profiles`.`Avatar` != '0' AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) $sql_getAgencyFilter limit $from , $count";
...
//$columns = '';
$Agencydetails = db_res("SELECT ID,FirstName,Age,State,waiting,(SELECT config_display FROM sys_configuration WHERE config_value = (CASE WHEN noofchildren = '' then NULL WHEN noofchildren >6 THEN 6  ELSE noofchildren END)) AS children,faith,ChildEthnicity,ChildAge,Adoptiontype,Avatar,NickName,childrenType from Profiles where ID = " . $agency_id . "");
...
$profile_link = $site['url'].'extra_profile_view_17.php?id='.$row_agency['ID'];
...
$photo = '<a href= "'.$profile_link.'" /><img style="width:'.$width.'px; height:'.$height.'px;margin-left:'.$margin_left.'px;margin-top:'.$margin_top.'px; background-color: #EDEDED;" src="' . $sImage . '"></a>';
...
$children      = (trim($row_agency[5]))?$row_agency[5]:"Not Disclosed";
$children_type      = (trim($row_agency[12]))?$row_agency[12]:"Not given";

if($children !=  'No Children' && $children != 'Not Disclosed') {      
if($children == '1')
{
    
  $fieldlabel = 'TYPE OF CHILD:';       
}
else 
{
$fieldlabel = 'TYPE OF CHILDREN:';  
}
}
else 
{
...
<profile_children_type>'.$children_type.'</profile_children_type>
<profile_fieldlabel>'.$fieldlabel.'</profile_fieldlabel>   
