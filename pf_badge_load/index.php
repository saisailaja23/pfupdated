...
$pid =$_GET['pid'];
//$pid =6778;
/*$gsiteName='http://www.parentfinder.com/';*/
...
<!--<script type="text/javascript"  src="https://www.parentfinder.com/pf_badge_load/js/handlebars.js"></script>-->
...
//jqueryNoConflict.getScript( "https://www.parentfinder.com/pf_badge_load/js/handlebars.js");
...
jqueryNoConflict('#searchInput').val('');
...
jqueryNoConflict(document).on('click','.backb_inner',function(){
...
jqueryNoConflict(document).on('click','.pf_more_cl014_jour',function(){
  var jouid = jqueryNoConflict(this).attr('jid');
   jqueryNoConflict("html, body").animate({
            scrollTop: 0
            }, 600);
pageDataChild("Journalcontent","Template_<?php echo $templateID; ?>",this.id,jouid);
});

jqueryNoConflict(document).on('click','.pf_more_cl07',function(){
pageData_new("Letter","Template_<?php echo $templateID; ?>",this.id);
});

jqueryNoConflict(document).on('click','.pf_more_cl06',function(){
pageData_new("OurHome","Template_<?php echo $templateID; ?>",this.id);
});
...
if(url=='javascript:void(0)'){
 ...
 jqueryNoConflict(document).on('click','.print_more_home a',function(){
            jqueryNoConflict('.print_more').toggle();
            jqueryNoConflict('.printClick').hide();
        });
 jqueryNoConflict(document).on('click','.print_more a',function(){
            jqueryNoConflict('.print_more').toggle();
            jqueryNoConflict('.printClick').hide();
        });
...
function pageData(page,templateid,dataid,jod) {
       jod = jod || 0; 
      jqueryNoConflict('#pf_search').hide(); 
      jqueryNoConflict('#data-detailsAbout').empty();
      jqueryNoConflict('#pf_loading').show();
        jqueryNoConflict.getJSON('<?php echo $gsiteName; ?>pf_badge_load/json-data.php?page='+page+'&uid='+dataid+'&start=0&end=1&jod='+jod, function(data) { 
...
function pageData_new(page,templateid,dataid) {
      jqueryNoConflict('#pf_search').hide(); 
      jqueryNoConflict('.pf_more_cl02').empty();
      jqueryNoConflict('#pf_page_home_new').show();
        jqueryNoConflict.getJSON('<?php echo $gsiteName; ?>pf_badge_load/json-data.php?page='+page+'&uid='+dataid+'&start=0&end=1', function(data) { 
          ...
          console.log(template);
            jqueryNoConflict('#data-detailsAbout').html(template(data));
            jqueryNoConflict('#pf_page_home_new').hide();
          });

      });
};

function pageDataChild(page,templateid,dataid,jod) {
		jod = jod || 0; 
      jqueryNoConflict('#pf_search').hide(); 
      //jqueryNoConflict('#data-detailsAbout').empty();
...
function handlebarsDebugHelper(){
    Handlebars.registerHelper("debug", function(optionalValue) {
       ...
       <?php include('header.php');?>
        ...
        <div style="display:none;" class="pf_loader_home_new" id="pf_page_home_new">
<img src="<?php echo $gsiteName; ?>/pf_badge_load/templates/Template_<?php echo $templateID; ?>/images/loader.gif" class="pf_loader_new_home_img" title="Loading.." /></div>
