...
break;
case 'Journalcontent':foo::journalcontent();
break;
...
function getSiteUrl()
{
 return $gsiteName='http://ctpf01.parentfinder.com';
}
	...
	while (($row_agency = mysql_fetch_array($Agencydetails, MYSQL_BOTH))) {
		...
		$Couple_age = $row_couplename['DateOfBirth'];
            ...
            $Profile_pth = (trim(str_replace(foo::getSiteUrl(), '/', $ProfileQ[template_file_path])) != '') ? trim(str_replace(foo::getSiteUrl(), '/', $ProfileQ[template_file_path])) : 'javascript:void(0)';
             $pdf_new_url =foo::getSiteUrl().'/'."ProfilebuilderComponent/pdf.php?id=".$row_agency[0];
            ...
            if($Profile_pth == 'javascript:void(0)'){
            	...
            	$pdf_atag = '<a class="pf_view_cl013" href="'.$pdf_new_url.'"></a>';
            }
            else{
            	$pdf_new_path = '<div class="print_more_home" data-profileId="'.$row_agency[0].'" id="printmore'.$row_agency[0].'">
                                <p style="margin:10px;height:25px;text-decoration: underline; font-size: 14px;font-weight: bold;"><a class="print_more_atag" href="'.$pdf_new_url.'" id="viewHtml" >Single Page Profile</a></p>
                                <p style="margin:10px;height:25px;text-decoration: underline; font-size: 14px;font-weight: bold"><a class="print_more_atag" href="'.$pdf_output.'" id="viewPdf" target="_blank">Multi Page Profile</a></p>
                                   
                            </div>';
                $pdf_atag = '<a class="pf_view_cl013"  id="pdf_multi_home"></a>';              
            }
...
while (( $row = mysql_fetch_array($query, MYSQL_BOTH))) {
            ...
            $pdf_new_url =foo::getSiteUrl().'/'."ProfilebuilderComponent/pdf.php?id=".$uid;
            $parsed = parse_url($Profile_pth);
				$path = $parsed['path'];
				$path_parts = explode('/', $path);
				if($path_parts[7]){
            ...
            if($Profile_pth == 'javascript:void(0)'){
            	...
            	$pdf_new_path = '<div class="print_more" >
                                <p style="margin:25px;text-decoration: underline; font-size: 14px;font-weight: bold;"><a  href="'.$pdf_new_url.'" id="viewHtml" >Single Page Profile</a></p>
                                <p style="margin:25px;text-decoration: underline; font-size: 14px;font-weight: bold"><a href="'.$pdf_output.'" id="viewPdf" target="_blank">Multi Page Profile</a></p>
                                   
                            </div>';
                $pdf_atag = '<a class="pf_more_cl09"  id="pdf_multi">';              
            }
...
function getPic($picID){
	...
	while($rw = mysql_fetch_array($r2)){
		$bImage=$rw["img"];
		$img2[] ='<img src=http://ctpf01.parentfinder.com/m/photos/get_image/thumb/'.$bImage.' />&nbsp;';	
	}
	$ret =$img2;
	return $ret;
}
...
$rowhh = mysql_fetch_array($query);
	...
	if($rowhh['img_her']!=''){
		$ff12 = foo::getPic2($rowhh['img_her']);
	...
	if($rowhh['img_agency']!=''){
		...
		$ff1 = foo::getPic2($rowhh['img_agency']);
	...
	if($rowhh['img_them']!=''){
		$ff2 = foo::getPic2($rowhh['img_them']);
	...
	if($rowhh['img_mother']!=''){
		 $moth = foo::getPic2($rowhh['img_mother']);
...
while ( ( $rowgg = mysql_fetch_array( $query, MYSQL_BOTH ) ) ) {
    ...
    $newval .= '<h3><a href="javascript:void(0)">'.$header.'</a></h3>
              <div style="padding-left: 30px; height:auto!important">'.$desc_lee.'<div style="padding-left: 30px; height:auto!important">'.$lettimg.'</div></div>';
}
}
else{
	...
	$stringletterelse = "SELECT img,label,description FROM `letter` WHERE profile_id= '$logid'";
		...
		$newval .= '<h3><a href="javascript:void(0)">'.$headerl.'</a></h3>
              <div style="padding-left: 30px; height:auto!important">'.$desc_leel.'<div style="padding-left: 30px; height:auto!important">'.$lettimgl.'</div></div>';
		}
...
function getPic2($picID){
	...
	while($rw = mysql_fetch_array($r2)){
		$bImage=$rw["img"];
		 if(getimagesize('http://ctpf01.parentfinder.com/m/photos/get_image/thumb/'.$bImage)!= false)
		 {
		 	$dd = $bImage;
		 	$img2[] ='<img src="http://ctpf01.parentfinder.com/m/photos/get_image/thumb/'.$bImage.'" />&nbsp;';	
		 }
