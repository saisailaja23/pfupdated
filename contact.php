...
//print_r($_GET);
//echo getLoggedId();
$sql = "SELECT p.email as userEmail,p.show_contact as contactSettings, p2.email as agencyEmail
            from Profiles p, bx_groups_main agen, Profiles p2
            where agen.id=p.adoptionagency
            and agen.author_id=p2.id
            and p.id=" . $id;

if ($id) {
	$EmailDetails = db_arr($sql);
	if ($EmailDetails['contactSettings'] == 0) {
		$recipientEmailId = $EmailDetails['agencyEmail'];
	} else {
		$recipientEmailId = $EmailDetails['userEmail'];
	}    
        $GLOBALS['recipientEmailId'] = $recipientEmailId;
}
check_logged();
...
function PageCompPageMainCodeWithForm() {
	...
	if ($oForm->isSubmittedAndValid()) {
		...
		if (!$GLOBALS['recipientEmailId']) {
                    $recipientEmail = $site['email'];
		}
                else{
                    $recipientEmail = $GLOBALS['recipientEmailId'];
                }

		if (sendMail($recipientEmail, $sLetterSubject, $sLetterBody)) {
