...
//$searchvaluess = ' ORDER BY Profiles.'.$searchvalues.' DESC';;

if($_REQUEST['sortvalue'] == 'sortby'){
 $familiesLike_List = "SELECT like_list_family.FamilyLiked,Profiles.NickName  
 FROM `like_list_family`,
 Profiles WHERE like_list_family.FamilyLiked = Profiles.ID and  `LikedBy` =" . $logid . " ORDER BY `Profiles`.`NickName` ASC";
}
else
{   
...
$cmdtuples = mysql_num_rows($family_query);
$arrColumns = explode(",", $columns);
$arrRows_family_list1 = array();

while (($row = mysql_fetch_array($family_query, MYSQL_BOTH)))
{
    
  //  if($sortvalues == 'favourites')
   $agency_id = $row['FamilyLiked'];
   // else
      //  $agency_id = $row['FamilyLiked'];
    ...
    //if($searchvalues){
      //  if($sortvalues == 'sortby') {
       //     $Agencydetails = "SELECT `Profiles`. `ID`, `Profiles`. `NickName`, `Profiles`. `Couple`, `Profiles`. `Sex`, if(`DateLastNav` > SUBDATE(NOW(), INTERVAL 30 MINUTE ), 1, 0) AS `is_online`,Avatar FROM `Profiles`,sys_acl_levels_members WHERE sys_acl_levels_members.IDLevel IN('14','15','18','20','24','23') AND `Profiles`.`Status` = 'Active' and `Profiles`.`ProfileType` = 2 and `Profiles`.`Avatar` != '0' AND sys_acl_levels_members.IDMember = `Profiles`. `ID` AND (DateExpires >= NOW() OR DateExpires IS NULL) AND (`Profiles`.`Couple` = 0 or `Profiles`.`Couple` > `Profiles`.`ID`) AND where ID = " . $agency_id . " Group By sys_acl_levels_members.IDMember ORDER BY `Profiles`.`DateLastLogin` DESC";
     //   }
     /*   elseif($searchvalues == 'favourites'){
            $fav_qryList = "SELECT favouredfamily    FROM `family_favourite`  WHERE `favouredby` = " . $logid . "";
            $fav_qry = mysql_query($fav_qryList);
            while (($row = mysql_fetch_array($fav_qry, MYSQL_BOTH))){
                $agency_id = $row['favouredfamily'];
                $Agencydetails = db_res("SELECT  ID,FirstName,Age,State,waiting,noofchildren,faith,ChildEthnicity,ChildAge,Adoptiontype,Avatar from profiles where ID = " . $agency_id . "");
            }
...
$Childages = sizeof($Childages_ary) >= 7 ? implode(", ",$Childages_sub) . '<a class = \'tooltip\' title= "'.str_replace(',',', ',$row_agency[8]) .'"  href="javascript:void(0)" ><span title=" ">....More</span></a>' :implode(", ",$Childages_ary);
        ...
        $arrValues = array();
        array_push($arrRows_family_list1, array(
            'id'=>$row_agency[0],
          ...
          //  'profile_childethnicity' => $row_agency[7],
            ...
            'home' => 'extra_profile_view_18.php?id='+profile_id, 
...
if ($cmdtuples > 0)
{
echo json_encode($arrRows_family_list1);
}
  else
{
...
function avatarimages($iId) {
 ...
 //   echo $sql_avt;exit();
   // $aData1='';
    $row_avt = mysql_fetch_array($result_avt);
     ...
     $filename = $dir['root'].'modules/boonex/avatar/data/familieslike/'.$row_avt[id].'.jpg'; 
    

   if (file_exists($filename)) {
   $aData1   =   $site['url'].'modules/boonex/avatar/data/familieslike/'.$row_avt[id].'.jpg';

     }
   else {

   $filename_new = $dir['root'].'modules/boonex/avatar/data/familieslike/'.$row_avt[author_id].'.jpg';
   $sNewFilePath = $dir['root'].'modules/boonex/avatar/data/familieslike/' . $row_avt[id] . '.jpg';
 ...
 if(!file_exists($filename_new)) {
    ...
    //$sNewFilePath_s2 = '/var/www/html/pf/modules/boonex/avatar/data/slider1/' . $row_avt[id] . '.jpg';
     $sNewFilePath1 = $dir['root'].'modules/boonex/avatar/data/familieslike/' . $row_avt[id] . '.jpg';
      } 
