    ...
    class BxBaseMailBox extends BxDolMailBox
    {
        ...
        function getBlockCode_ComposeMessage()
        {
            ...
            $sRecipientName      = ( !empty($aMemberInfo) ) ? $aMemberInfo['NickName'] : null;
              ...
              // Profile view link change by prashanth                                                                                  
                 ...
                 $profileDetails = getProfileInfo($aMemberInfo['ID']);
                            
                 if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
                 $ProfileLink = $site['url'].'extra_profile_view_17.php?id='.$aMemberInfo['ID']; 
                  }
                 else {
                 $ProfileLink = $site['url'].'extra_agency_view_29.php?id='.$aMemberInfo['ID'];  
                 }                           
   
             //  $sMemberLocation     = ( !empty($aMemberInfo) ) ? getProfileLink($aMemberInfo['ID']) : null;
                 $sMemberLocation     = ( !empty($aMemberInfo) ) ? $ProfileLink : null;               
        ...
        function getBlockCode_ViewMessage()
        {
            ...
            while( true == ($aRow = mysql_fetch_assoc($rResult)) ) {
               ...
               // Profile view link change by prashanth                                                                                  
                ...
                if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
                 $sMemberLocation = $site['url'].'extra_profile_view_17.php?id='.$aMemberInfo['ID']; 
                  }
                else {
                 $sMemberLocation = $site['url'].'extra_agency_view_29.php?id='.$aMemberInfo['ID'];  
                 }                           
            ...
            if (method_exists($this, $sFunctionName)) {
                ...
                if ( is_array($aMembersList) and !empty($aMembersList) ) {
                        ...
                        foreach($aMembersList AS $sKey => $aProfileInfo ) {
                            ...
                            $profileDetails = getProfileInfo($aProfileInfo['ID']);
                            
                            if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
                            $sMemberLocation = $site['url'].'extra_profile_view_17.php?id='.$aProfileInfo['ID']; 
                            }
                            else {
                            $sMemberLocation = $site['url'].'extra_agency_view_29.php?id='.$aProfileInfo['ID'];  
                            }                            
        ...
        function genMessagesRows()
        {
            ...
            if ( is_array($aMessages) and !empty($aMessages) ) {
                  ...
                  if($this -> aMailBoxSettings['mailbox_mode'] != 'outbox') {  
                  $profileDetails = getProfileInfo($aItems['Sender']);
                  if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
                  $sMemberLocation = $site['url'].'extra_profile_view_17.php?id='.$aItems['Sender']; 
                   }
                  else {
                  $sMemberLocation = $site['url'].'extra_agency_view_29.php?id='.$aItems['Sender'];  
                  }                           
                  }
                  else {
                  $profileDetails = getProfileInfo($aItems['Recipient']);
                  if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
                  $sMemberLocation = $site['url'].'extra_profile_view_17.php?id='.$aItems['Recipient']; 
                  }
                  else {
                  $sMemberLocation = $site['url'].'extra_agency_view_29.php?id='.$aItems['Recipient'];  
                  }    
