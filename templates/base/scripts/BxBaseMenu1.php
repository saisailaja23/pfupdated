...
class BxBaseMenu extends BxDolMenu {
	...
	* Sample is:
		$aCustomActions = array(
			1 => array(
				'title' => 'Contact',
				'url' => 'mail.php?mode=compose&amp;recipient_id=2',
				'icon' => 'action_send.png',
			),
			2 => array(
				'title' => 'Befriend',
				'url' => 'list_pop.php?action=friend&amp;ID=2',
				'icon' => 'action_friends.png',
			),
			3 => array(
				'title' => 'Befriend',
				'url' => 'mail.php?mode=compose&amp;recipient_id=2',
...
<?
		...
		return array (
		    'id'=> 'searchicon',
		    'icon_url' => getTemplateIcon('help-icon.png'),
		    'element_content' => '<script>document.getElementById("searchicon").style.display = "none";</script>',
		    'url' => "window.open('http://help.cairsolutions.com/index.php?a=add&catid=4')",
		);
    }	
	...
	*/
	function genMoreTemplatesElement(){
		...
		foreach ($aExistedTemplates as $sTemplateID => $sTemplateVal) {
			//$sIActiveStyles = ($sCurTemplate == $sTemplateID) ? 'color:#333;font-weight:bold;' : '';
			...
			if ($sCurTemplate == $sTemplateID) {
                ...
                if (defined('BX_PROFILE_PAGE')) {
    				...
    				foreach ($_GET as $sParamKey => $sParamValue) {
    					if ('skin' != $sParamKey)
    						$sGetTransfer .= "&{$sParamKey}={$sParamValue}";
    				}
    				$sTemplateUrl = bx_html_attribute($_SERVER['PHP_SELF']) . '?skin='. $sTemplateID . $sGetTransfer;
                }
			}
	...
	*/
	function genMoreLanguagesElement(){
		...
		foreach ($aExistedLanguages as $sLanguageID => $sLanguageVal) {
			//$sIActiveStyles = ($sCurLanguage == $sLanguageID) ? 'color:#333;font-weight:bold;' : '';
			...
			if ($sCurLanguage == $sLanguageID) {
				...
				$sLanguageUrl = $this->oPermalinks->permalink(bx_html_attribute($_SERVER['PHP_SELF']) . '?' . $sGetTransfer . 'lang='. $sLanguageID);
			}
...
EOF;
    ...
    <div class="actionsBlock">
    	<table cellspacing="0" cellpadding="3px" width="100%" >
            <tbody>
                ...
                <tr>
                <td width="50%">
                    <div onclick="AqbPCMain.showPopup('modules/?r=aqb_pcomposer/get_create_block_form/{$this->aMenuInfo['profileID']}');" style="width: 100%; margin: 0pt;" class="button_wrapper">
	<div class="button_wrapper_close"></div>
    <div class="button_input_wrapper">
    	<button class="form_input_submit" type="button">
            <img src="{$site['url']}modules/aqb/pcomposer/templates/base/images/icons/new_block.png" alt="Add my block">
            Add Section
        </button>
    </div>
</div>
                ...
                <td width="50%">
                   <div onclick="$('#profile_customize_page').fadeIn('slow', function() {dbTopMenuLoad('profile_customizer');});" style="width: 100%; margin: 0pt;" class="button_wrapper">
	<div class="button_wrapper_close"></div>
    <div class="button_input_wrapper">
    	<button class="form_input_submit" type="button">
            <img src="{$site['url']}modules/boonex/profile_customize/templates/base/images/icons/bx_action_customize.png" alt="Customize">
            Customize Profile
        </button>
    </div>
</div>
 ...
 <td>
                 ...
                 <script type="text/javascript" src="{$site['url']}/lightbox/thickbox-compressed.js"></script>
                 <link rel="stylesheet" href="{$site['url']}/lightbox/thickbox.css" type="text/css" media="screen"/>
               <div  style="width: 100%; margin: 0pt;" class="button_wrapper">
					<div class="button_wrapper_close"></div>
    					<div class="button_input_wrapper">
                                  <img src="{$site['url']}modules/aqb/pcomposer/templates/base/images/icons/new_block.png" alt="Upload Profile">
    					<a href="file_upload.php?TB_iframe=true&height=195&width=490&modal=true" class="thickbox" ><input type='button' name='contact' value='Upload Profile' class='form_input_submit'/></a>
    					</div>
				</div>
              </td> 
		...
		<tr>
			<td width="50%"></td>
					...
					<div class="button_wrapper_close"></div>
    					<div class="button_input_wrapper">
    						<a href="http://pdfcrowd.com/url_to_pdf/" style="height:25px;">	
							<button class="form_input_submit" type="button" >
           							 <img src="{$site['url']}modules/aqb/pcomposer/templates/base/images/icons/print.jpeg" alt="Save to PDF">
            											Save as PDF
        						</button>
						</a>
    					</div>
...
<tr><td width="50%"></td><td width="50%"></td></tr>
	...
	<div class="button_wrapper_close"></div>
    <div class="button_input_wrapper">
    <a href="http://pdfcrowd.com/url_to_pdf/" style="height:25px;">	<button class="form_input_submit" type="button" >
            <img src="{$site['url']}modules/aqb/pcomposer/templates/base/images/icons/print.jpeg" alt="Save to PDF">
            Save as PDF
        </button></a>
    </div>
...
EOH;
		...
		/////Picture////////
		if ($this->sCustomSubHeader == '' && $this->aMenuInfo['profileID']>0) {
...
EOF;
        ...
        if (true) { // $sSubElementCaption != '') {
			...
			$sSubMainOnclick = $sSubMainOnclick ? ( ' onclick="' . $sSubMainOnclick . '"' ) : '';
...
EOF;
		...
		if (defined('PF_VIEW_PROFILE')) {
			$aProfInfo = getProfileInfo(PF_VIEW_PROFILE);
			$sCaptionWL = $aProfInfo['FirstName'];
			if ($aProfInfo['Couple'] > 0) {
				$aProf2Info = getProfileInfo($aProfInfo['Couple']);
				$sCaptionWL .= '&nbsp;&amp;&nbsp;' . $aProf2Info['FirstName'];
			}
			$sCaptionWL = '<a href="' . getProfileLink(PF_VIEW_PROFILE) . '">' . $sCaptionWL . '</a>';
		}
		...
		if( !$bActive ) {
			...
			if ( strpos( $sLink, 'http://' ) === false && !strlen($sOnclick) )
				$sLink = $this->sSiteUrl . $sLink;
                     ...
                     $style=  "<style>
                      .subMenu table td.usual div #style
                            {
                            color:blue
                            }
                             </style>" ;
...
if($sCaption=='Photos' || $sCaption=='Videos' || $sCaption=='Journal' || $sCaption=='Sounds' || $sCaption=='Profile'){
				$sSubItems .= <<<EOF
<td class="usual" >{$style}
	<div><a class="sublinks" id="style"  href="{$sLink}" {$sTarget} {$sOnclick}>{$sCaption}</a></div>
</td>
EOF;
}
else{
...
EOF;
         ...
         $style=  "<style>
                  .subMenu table td.tabbed div.Photos,
                  .subMenu table td.tabbed div.Journal,
                  .subMenu table td.tabbed div.Videos,
                  .subMenu table td.tabbed div.Profile{
                   color:blue
                  }
                  </style>" ;
...
if($sCaption=='Photos' || $sCaption=='Videos' || $sCaption=='Journal' || $sCaption=='Sounds'|| $sCaption=='Profile'){
	...
	<td ><div style="background:url({$sIcon1_a_c}) repeat-x top center;" class={$sCaption}>{$sCaption}</div></td>
