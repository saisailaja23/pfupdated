...
class BxBaseCmtsView extends BxDolCmts
{
    ...
    function _getCommentHeadBox (&$a)
    {
        ...
        // Profile view link change by prashanth                                                                                  
         ...
         $profileDetails = getProfileInfo($a['cmt_author_id']);
                            
         if($profileDetails['ProfileType'] == 2 || $profileDetails['ProfileType'] == 4){                         
         $ProfileLink = $site['url'].'extra_profile_view_17.php?id='.$a['cmt_author_id']; 
          }
         else {
         $ProfileLink = $site['url'].'extra_agency_view_29.php?id='.$a['cmt_author_id'];  
          }                           
   ...
   //   $sAuthor = '<a href="' . getProfileLink($a['cmt_author_id']) . '" class="cmt-author">' . getNickName($a['cmt_author_id']) . '</a>';
         $sAuthor = '<a href="' . $ProfileLink . '" class="cmt-author">' . getNickName($a['cmt_author_id']) . '</a>';
        else
