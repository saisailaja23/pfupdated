    ...
    function genQuickLinks()
    {
        ...
        foreach ($this->aTopMenu as $iItemID => $aItem) {
            ...
            $aItem['Caption'] = $this->replaceMetas($aItem['Caption']);
			 ...
			 if($aItem['Caption']=='_bx_groups_root')
                        {
                           $_sProfInfo=getProfileInfo($this->aMenuInfo['memberID']);
                            $_sURI=db_arr("SELECT uri FROM bx_groups_main WHERE  id=". $_sProfInfo['AdoptionAgency']." LIMIT 0,1");
                            $aItem['Link'] = 'm/groups/view/'.$_sURI['uri'];
                            $aItem['Caption']='_bx_groups_root';
                        }
        ...
        if(!$GLOBALS['logged']['admin']){
                $aQlinksUnits['Agency Home'] = '';  
                }


                $aQlinksUnits['Events Home']= '';  
                $aQlinksUnits['Knowledge Base']= ''; 
        ...
        $membership_info = getMemberMembershipInfo(getLoggedId());
             ...
             if($membership_info['ID'] == '23' || $membership_info['ID'] == '2' || $membership_info['ID'] == '25')
             {
               unset($aQlinksUnits['Manage Print Profile']);
               unset($aQlinksUnits['Create PDF Profile']);
               unset($aQlinksUnits['Upload PDF Profile']);
