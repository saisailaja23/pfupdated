    ...
    */
    ...
    function getMemberThumbnail($iId, $sFloat = 'none', $bGenProfLink = false, $sForceSex = 'visitor', $isAutoCouple = true, $sType = 'medium', $aOnline = array())
    {
       ...
       // Profile view link change by prashanth       
        ...
        $sLink = $site['url'].'extra_profile_view_17.php?id='.$iId; 
         }
        else {
        $sLink = $site['url'].'extra_agency_view_29.php?id='.$iId;  
        }                           
       // Profile view link change by prashanth       
...
function getMemberThumbnail_larger($iId, $sFloat = 'none', $bGenProfLink = false, $sForceSex = 'visitor', $isAutoCouple = true, $sType = 'medium', $aOnline = array()) {
    global $baseurl,$basepath;

    $baseurl        = $site['url'];
    $basepath       = $dir['root'];
        
   //echo $imagepath  = $this->getSliderImage($iId);exit();
...
$row_avt = mysql_fetch_array($result_avt);
    ...
    $filename = '/var/www/html/pf/modules/boonex/avatar/data/slider/'.$row_avt[id].'.jpg'; 
    

   if (file_exists($filename)) {
    $aData1   =   'http://www.parentfinder.com/modules/boonex/avatar/data/slider/'.$row_avt[id].'.jpg';

     }
   else {

    $filename_new = '/var/www/html/pf/modules/boonex/avatar/data/avatarphotos/'.$row_avt[author_id].'.jpg';
    $sNewFilePath = '/var/www/html/pf/modules/boonex/avatar/data/slider/' . $row_avt[id] . '.jpg';
 ...
 if(!file_exists($filename_new)) {
    ...
    //$sNewFilePath_s2 = '/var/www/html/pf/modules/boonex/avatar/data/slider1/' . $row_avt[id] . '.jpg';
     $sNewFilePath1 = '/var/www/html/pf/modules/boonex/avatar/data/slider/' . $row_avt[id] . '.jpg';
      } 
      ...
      //  imageResizee($filename_new1, $sNewFilePath1, $iWidth = 200, $iHeight = 200, true);
        ...
        $aVariables = array(
			...
			'sys_status_title' => $oUserStatusView->getStatus($iId),
            'usr_profile_url' => $sLink,
						'usr_thumb_url0' => $aData1 ? $aData1 : $this->getSexPic($aProfile['Sex'], $sType),
...
$bCouple = ((int)$aProfile['Couple'] > 0) && $isAutoCouple ? true : false;
        if ($bCouple) {
          ...
          //  $aVariables['usr_thumb_url1'] = $aProfileCouple['Avatar'] ? BX_AVA_URL_USER_AVATARS . $aProfileCouple['Avatar'] . 'i' . BX_AVA_EXT : $this->getSexPic($aProfileCouple['Sex'], 'small'); 
           ...
           $aVariables['bx_if:profileLink']['content']['personOne']=$aProfile['FirstName'];
            $aVariables['bx_if:profileLink']['content']['personTwo']=$aProfileCouple['FirstName'];
        }else{
            $aVariables['bx_if:profileLink']['content']['personOne']=$aProfile['FirstName'];
        }
        
        return $GLOBALS['oSysTemplate']->parseHtmlByName($bCouple ? "thumbnail_couple_larger.html" : "thumbnail_single_larger.html", $aVariables);
    }
        ...
        if (null === $aActions || empty($aActions[$sActionsType]) ) {
            ...
            $sQuery  = 	"
                ...
                FROM
                    `sys_objects_actions`
                WHERE
                    `Type` = '{$sActionsType}'  AND Caption NOT IN('{cpt_befriend}','{cpt_fave}')
                ORDER BY
                    `Order`
            ";
