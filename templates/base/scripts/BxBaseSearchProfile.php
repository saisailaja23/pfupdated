...
//--- AQB Profile Types Splitter ---//
		...
		$bExtMode = (!isset($_GET['mode']) || $_GET['mode']=='extended' || $_GET['search_result_mode'] == 'ext') ? true : false;
		...
		$sProfileThumb = get_member_thumbnail( $aProfileInfo['ID'], 'none', false, 'visitor', $aOnline );
		...
		$country = _t("_Country").": "._t($aPreValues['Country'][$aProfileInfo['Country']]['LKey']).'&nbsp;<img src="'. ($site['flags'].strtolower($aProfileInfo['Country'])) .'.gif" alt="flag" />';
		...
		// description
		$i_am = $i_am2 = _t("_I am");
		if($aProfileInfo['DearBirthParent'])
		$i_am_desc = trim( strip_tags($aProfileInfo['DearBirthParent']) );
                else
		...
		if ( $bExtMode && isLogged()) {
			$sReportC = _t('_Report');
			$al_report = <<<EOF
<a class="menuLink" onclick="getHtmlData( 'ajaxy_popup_result_div', 'list_pop.php?action=block&ID={$aProfileInfo['ID']}&mode=ajax');return false;" href="javascript:void(0)">{$sReportC}</a>
EOF;
		}
		...
		$sNickName = $aCoupleInfo ? $aProfileInfo['FirstName'] . '&nbsp;&amp;&nbsp;' . $aCoupleInfo['FirstName'] : $aProfileInfo['FirstName'];
		$Link = getProfileLink($aProfileInfo['ID']);
                $sStatus=$aProfileInfo['Status'];
		  $_profileDraft=db_arr("SELECT Status FROM Profiles_draft WHERE ID=".$aProfileInfo['ID']);
                if($sStatus != 'Active'){

                    if($aProfileInfo['FirstName']&&$aCoupleInfo['FirstName'])
                    {    
                        $sProfileNickname =  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" style="color:red;">' . $aProfileInfo['FirstName'] . '&nbsp;&amp;&nbsp;' . '</a></div>' ;
                        $sProfileNickname .=  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" style="color:red;">' . $aCoupleInfo['FirstName'] . '</a></div>' ;
                    }else{
                        $sProfileNickname =  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" style="color:red;">' . $aProfileInfo['FirstName'] . '</a></div>' ;
                    }
                }elseif($sStatus=='Active' && $_profileDraft['Status']=='Approval' && @mysql_num_rows(mysql_query("SELECT id FROM bx_groups_main WHERE author_id=".$_COOKIE['memberID'])) || $GLOBALS['logged']['admin']){
                    if($aProfileInfo['FirstName']&&$aCoupleInfo['FirstName'])
                    {
                        $sProfileNickname =  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" style="color:Green;font-weight: 600;">' . $aProfileInfo['FirstName'] . '&nbsp;&amp;&nbsp;' . '</a></div>' ;
                        $sProfileNickname .=  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" style="color:Green;font-weight: 600;">' . $aCoupleInfo['FirstName'] . '</a></div>' ;
                    }else{
                        $sProfileNickname =  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" style="color:Green;font-weight: 600;">' . $aProfileInfo['FirstName'] . '</a></div>' ;
                    }
                }else{
                    if($aProfileInfo['FirstName']&&$aCoupleInfo['FirstName'])
                    {
                        $sProfileNickname =  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'">' . $aProfileInfo['FirstName'] . '&nbsp;&amp;&nbsp;' . '</a></div>' ;
                        $sProfileNickname .=  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" >' . $aCoupleInfo['FirstName'] . '</a></div>' ;
                    }else{
                        $sProfileNickname =  '<div style="width: 70px;" class="thumb_username"><a href="'.$Link.'" >' . $aProfileInfo['FirstName'] . '</a></div>' ;
                    }
                }
		...
		if ($bExtMode && isLogged()) {
			...
			$oProfileSR = new BxBaseProfileGenerator( $aProfileInfo['ID'] );
			$sActions = $oProfileSR->GenActionsMenuBlock();
		}
        ...
        $aKeys = array(
			...
			'al_im' => $al_im,
			'al_report' => $al_report,
			'ai_viewprof' => $ai_viewprof,
			'al_viewprof' => $al_viewprof,
			'ai_kiss' => $sAiKiss,
			'al_kiss' => $al_kiss,
			'ai_sendmsg' => $ai_sendmsg,
